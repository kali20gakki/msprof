...
function add_gcov_excl_line_for_collector() {
    find ${TOP_DIR}/collector/dvvp -name "*.cpp" -type f -exec sed -i -e 's/^[[:blank:]]*MSPROF_.*;$/& \/\/ LCOV_EXCL_LINE/g' -e '/^[[:blank:]]*MSPROF_.*[,"]$/,/.*;$/ s/;$/& \/\/ LCOV_EXCL_LINE/g' {} \;
    find ${TOP_DIR}/collector/dvvp -name "*.cpp" -type f -exec sed -i -e 's/^[[:blank:]]*MSVP_.*;$/& \/\/ LCOV_EXCL_LINE/g' -e '/^[[:blank:]]*MSVP_.*[,"]$/,/.*;$/ s/;$/& \/\/ LCOV_EXCL_LINE/g' {} \;
}
