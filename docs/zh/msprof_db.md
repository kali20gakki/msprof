...
| 2025/9/29 | 修正5500指代的level层级内容为communication |
...
| 2025/3/21  | 76 77枚举值仅资料变更和实际数据内容保持一致                              |
...
| 2025/9/28 | 无device目录时，会生成一条deviceId为-1的记录 |
...
| 字段名         | 类型      | 索引  | 含义       |
|-------------|---------|-----|----------|
| timestampNs | NUMERIC |  | 采样时的本地时间，单位 ns |
| cpuId       | NUMERIC |     | cpu编号    |
| usage       | NUMERIC |    | 利用率(%)   |
...
|-------------|---------|-----|-----------------|
| timestampNs | NUMERIC |  | 采样时的本地时间，单位 ns  |
| readRate    | NUMERIC |     | 磁盘读速率， 单位 B / s |
| writeRate   | NUMERIC |     | 磁盘写速率， 单位 B / s |
| usage       | NUMERIC |    | 利用率(%)          |
...
| 2025/9/28 | 无device目录时，也生成一条deviceId为-1的记录            |
