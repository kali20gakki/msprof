...
class TestCalculateTaskScheduler(unittest.TestCase):
    ...
    def test_create_task_time(self):
        ...
        res = db_manager.create_table("runtime.db", create_sql, insert_sql, data)
        runtime_conn = res[0]
        with mock.patch(NAMESPACE + '.CalculateTaskScheduler._get_timeline_data', side_effect=OSError), \
    ...
    def test_update_timeline_api(self):
        ...
        with mock.patch(NAMESPACE + '.DBManager.judge_table_exist', return_value=True), \
                mock.patch(NAMESPACE + '.CalculateTaskScheduler.update', side_effect=TypeError), \
                mock.patch(NAMESPACE + '.logging.error'):
            key = CalculateTaskScheduler({"result_dir": 123})
            key.update_timeline_api(runtime_conn)
    ...
    def test_pre_mini_task_data(self):
        ...
        with mock.patch(NAMESPACE + '.DBManager.check_connect_db_path', return_value=(None, None)), \
                mock.patch(NAMESPACE + '.DBManager.destroy_db_connect'):
            key = CalculateTaskScheduler({"result_dir": 123})
            key._CalculateTaskScheduler__pre_mini_task_data(project_path, device_id, iter_time_range)
        res = db_manager.create_table("runtime.db")
        with mock.patch(NAMESPACE + '.DBManager.check_connect_db_path', return_value=(res[0], res[1])), \
                mock.patch(NAMESPACE + '.CalculateTaskScheduler.create_task_time'), \
                mock.patch(NAMESPACE + '.CalculateTaskScheduler.update_timeline_api'), \
                mock.patch(NAMESPACE + '.DBManager.destroy_db_connect'):
            key = CalculateTaskScheduler({"result_dir": 123})
            key._CalculateTaskScheduler__pre_mini_task_data(project_path, device_id, iter_time_range)
        with mock.patch(NAMESPACE + '.CalculateTaskScheduler.create_task_time', side_effect=TypeError), \
                mock.patch(NAMESPACE + '.logging.error'), \
                mock.patch(NAMESPACE + '.DBManager.destroy_db_connect'):
            key = CalculateTaskScheduler({"result_dir": 123})
            key._CalculateTaskScheduler__pre_mini_task_data(project_path, device_id, iter_time_range)
    ...
    def test_ms_run(self):
        ...
        with mock.patch(NAMESPACE + '.CalculateTaskScheduler.generate_report_data'):
            key = CalculateTaskScheduler({"result_dir": 123})
            key.run()
        with mock.patch(NAMESPACE + '.logging.error'):
            key = CalculateTaskScheduler({"result_dir": 123})
    ...
    def test_generate_report_data(self):
        ...
        with mock.patch(NAMESPACE + '.CalculateTaskScheduler._CalculateTaskScheduler__get_iter_time_range',
                ...
                mock.patch(NAMESPACE + '.CalculateTaskScheduler._CalculateTaskScheduler__pre_mini_task_data'), \
                mock.patch(NAMESPACE + '.CalculateTaskScheduler.insert_report_data'):
            check = CalculateTaskScheduler(CONFIG)
