...
NAMESPACE = 'mscalculate.runtime.calculate_op_task_scheduler'
...
class TestCalculateOpTaskScheduler(unittest.TestCase):
    ...
    def test_update(self):
        ...
        expect_result = [(25, 1, 65535, '0', '0'),
                         ...
                         (6, 6, 5, '0', '0'),
                         (6, 6, 5, '0', '0')]
        check = CalculateOpTaskScheduler(CONFIG)
        result = check._update(api_data)
    ...
    def test_op_create_task_time(self):
        ...
        with mock.patch(NAMESPACE + '.logging.info'):
            ...
            with mock.patch(NAMESPACE + '.CalculateOpTaskScheduler._get_timeline_data', side_effect=SystemError):
                check = CalculateOpTaskScheduler(CONFIG)
                check.index_id = 1
                check.op_create_task_time(runtime_conn, device)
    ...
    def test_op_update_timeline_api(self):
        ...
        with mock.patch(NAMESPACE + '.logging.info'), \
            ...
            with mock.patch(NAMESPACE + '.DBManager.judge_table_exist', return_value=True):
                ...
                with mock.patch(NAMESPACE + '.CalculateOpTaskScheduler._update', side_effect=OSError):
                    check = CalculateOpTaskScheduler(CONFIG)
                    check.op_update_timeline_api(runtime_conn)
    ...
    def test_op_pre_mini_task_data(self):
        ...
        with mock.patch(NAMESPACE + '.logging.error'), \
            ...
            with mock.patch(NAMESPACE + '.DBManager.check_connect_db_path',
                    ...
                    mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_create_task_time'), \
                    mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_update_timeline_api'):
                check = CalculateOpTaskScheduler(CONFIG)
    ...
    def test_op_generate_report_data(self):
        with mock.patch(NAMESPACE + '.PathManager.get_db_path', return_value=True):
            ...
            with mock.patch(NAMESPACE + '.DBManager.check_tables_in_db', return_value=False), \
                    mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_pre_mini_task_data'), \
                    mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_insert_report_data'):
                check = CalculateOpTaskScheduler(CONFIG)
    ...
    def test_run(self):
        ...
        with mock.patch('analyzer.scene_base.profiling_scene.Utils.get_scene',
            ...
            with mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_generate_report_data'):
                check = CalculateOpTaskScheduler(CONFIG)
                check.run()
        with mock.patch('analyzer.scene_base.profiling_scene.Utils.get_scene',
                           return_value="single_op"):
            with mock.patch(NAMESPACE + '.CalculateOpTaskScheduler.op_generate_report_data', side_effect=OSError), \
                    mock.patch(NAMESPACE + '.logging.error'):
                check = CalculateOpTaskScheduler(CONFIG)
                check.run()
