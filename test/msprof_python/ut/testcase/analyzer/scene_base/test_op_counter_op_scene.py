...
NAMESPACE = 'analyzer.scene_base.op_counter_op_scene'


class TestOpCounterOpScene(unittest.TestCase):
    ...
    def test_create_db(self):
        ...
        merge_conn, merge_curs = res[0], res[1]
    ...
    def test_get_op_report_sql(self):
        ...
        expected_sql = "select op_type, ge_task_merge.task_type, count(op_type), sum(duration) as total_time," \
                       ...
                       "and ge_task_merge.task_type=rts_task.task_type group by op_type,ge_task_merge.task_type"
    ...
    def test_create_report(self):
        ...
        with mock.patch(NAMESPACE + '.OpCounterOpScene._get_op_report_sql', return_value=sql):
            check = OpCounterOpScene(CONFIG)
            check.create_report(res[0], merge_curs)
        merge_curs.execute("insert into ge_task_merge values (1,'pool1','Pooling',5,5,0,10)")
        merge_curs.execute("insert into rts_task values (1,5,5,0,1,0,10)")
        res[0].commit()
        with mock.patch(NAMESPACE + '.OpCounterOpScene._get_op_report_sql', return_value=sql), \
                mock.patch(NAMESPACE + '.OpCounterOpScene._cal_total', return_value=10000):
            check = OpCounterOpScene(CONFIG)
            check.create_report(res[0], merge_curs)
    ...
    def test_cal_total(self):
        ...
        check = OpCounterOpScene(CONFIG)
        result = check._cal_total(task_time)
    ...
    def test_run(self):
        ...
        with mock.patch(NAMESPACE + '.PathManager.get_db_path', return_value=True), \
            ...
            with mock.patch(NAMESPACE + '.OpCounterOpScene._is_db_need_to_create', return_value=False), \
                    mock.patch(NAMESPACE + '.logging.warning'):
                check = OpCounterOpScene(CONFIG)
                check.run()
            with mock.patch(NAMESPACE + '.OpCounterOpScene._is_db_need_to_create', return_value=True), \
                    mock.patch(NAMESPACE + '.logging.error'), \
                    mock.patch(NAMESPACE + '.logging.warning'), \
                    mock.patch(NAMESPACE + '.DBManager.destroy_db_connect'):
                ...
                with mock.patch(NAMESPACE + '.DBManager.create_connect_db',
                                return_value=res), \
                        mock.patch(NAMESPACE + '.OpCounterOpScene._create_db'), \
                        mock.patch(NAMESPACE + '.OpCounterOpScene.create_ge_merge'), \
                        mock.patch(NAMESPACE + '.OpCounterOpScene.create_task'), \
                        mock.patch(NAMESPACE + '.OpCounterOpScene.create_report'):
                    check = OpCounterOpScene(CONFIG)
                    check.run()
        db_manager.destroy(res)
