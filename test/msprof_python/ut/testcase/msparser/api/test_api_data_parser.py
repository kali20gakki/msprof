...
from msparser.api.api_data_parser import ApiDataParser
from profiling_bean.prof_enum.data_tag import DataTag

NAMESPACE = 'msparser.api.api_data_parser'


class TestApiDataParser(unittest.TestCase):
    file_list = {DataTag.API: ['aging.api.data.slice_0', 'unaging.api.data.slice_0']}

    def test_ms_run(self):
        with mock.patch(NAMESPACE + '.ApiDataParser.parse'), \
                mock.patch(NAMESPACE + '.ApiDataParser.save'):
            check = ApiDataParser(self.file_list, CONFIG)
            check.ms_run()
        with mock.patch(NAMESPACE + '.ApiDataParser.parse', side_effect=SystemError), \
    ...
    def test_parse(self):
        ...
        with mock.patch(NAMESPACE + '.is_valid_original_data', return_value=True), \
                mock.patch(NAMESPACE + '.ApiDataParser.get_file_path_and_check', return_value=True), \
