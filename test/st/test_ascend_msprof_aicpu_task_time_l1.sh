...
main(){
    ...
    start_time=$(date "+%s")
    cd ${cur_dir}/src/
    msprof --output=${testcase_result_dir}/result_dir python3 --task_time=l1 ${cur_dir}/src/train.py > ${testcase_result_dir}/plog.txt 2>&1
    python_exit_code=$?
    ...
    # 1. Check if plog reports any errors
    ...
    # 2. Verify the profiling data items
