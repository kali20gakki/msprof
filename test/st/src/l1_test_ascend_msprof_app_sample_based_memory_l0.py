...
class TestAscendMsprofAppSampleBasedMemoryL0(BaseAscendMsprofChecker):
    def check_ai_vector_core_utilization_data(self):
        ...
        FileChecker.check_csv_data_non_negative(ai_vector_core_utilization_path[0],
                                                ...
                                                columns=["l0a_read_bw(GB/s)", "l0a_write_bw(GB/s)",
                                                         "l0b_read_bw(GB/s)", "l0b_write_bw(GB/s)", "l0c_read_bw(GB/s)",
                                                         "l0c_write_bw(GB/s)", "l0c_read_bw_cube(GB/s)",
    ...
    def check_ai_core_utilization_data(self):
        ai_core_utilization_path = glob.glob(f"{self.prof_path}/PROF_*/mindstudio_profiler_output/"
                                             f"ai_core_utilization*.csv")
        if not ai_core_utilization_path:
            raise FileNotFoundError(f"No ai_core_utilization.csv found in {self.prof_path}")
        FileChecker.check_csv_data_non_negative(ai_core_utilization_path[0],
                                                comparison_func=self._is_non_negative,
                                                columns=["l0a_read_bw(GB/s)", "l0a_write_bw(GB/s)",
                                                         "l0b_read_bw(GB/s)", "l0b_write_bw(GB/s)", "l0c_read_bw(GB/s)",
                                                         "l0c_write_bw(GB/s)", "l0c_read_bw_cube(GB/s)",
...
def test_ascend_msprof_app_sample_based_memory_l0(prof_path):
    expect_db_tables = ["ACC_PMU", "AICORE_FREQ", "CANN_API", "COMPUTE_TASK_INFO", "ENUM_API_TYPE",
                        ...
                        "STRING_IDS", "TASK"]
    test_ascend_msprof = TestAscendMsprofAppSampleBasedMemoryL0(prof_path, expect_db_tables)
