...
TEST_F(DRIVER_AI_DRV_API_TEST, DrvGetChannels)
{
    GlobalMockObject::verify();
    struct DrvProfChannelsInfo channels;
    channels.deviceId = -1;
    EXPECT_EQ(PROFILING_FAILED, analysis::dvvp::driver::DrvGetChannels(channels));
...
TEST_F(DRIVER_AI_DRV_API_TEST, GetAllChannels)
{
    GlobalMockObject::verify();
    auto entry = analysis::dvvp::driver::DrvChannelsMgr::instance();

    int indexId = -1;
    EXPECT_EQ(PROFILING_FAILED, entry->GetAllChannels(indexId));
    indexId = 1;
    EXPECT_EQ(PROFILING_SUCCESS, entry->GetAllChannels(indexId));
}

TEST_F(DRIVER_AI_DRV_API_TEST, ChannelIsValid)
{
    GlobalMockObject::verify();
    auto entry = analysis::dvvp::driver::DrvChannelsMgr::instance();

    struct DrvProfChannelInfo channelInfo;
    channelInfo.channelId = PROF_CHANNEL_HBM;
    struct DrvProfChannelsInfo ChannelsInfo;
    channelsInfo.channels = {
        channelInfo
    };
    int devId = -1;
    AI_DRV_CHANNEL channelId = PROF_CHANNEL_UNKNOWN;
    entry->devIdChannelsMap_ = {
        {1, channelsInfo},
    };
    EXPECT_EQ(false, entry->ChannelIsValid(devId, channelId));
    devId = 1;
    EXPECT_EQ(false, entry->ChannelIsValid(devId, channelId));
    channelId = PROF_CHANNEL_BUS;
    EXPECT_EQ(false, entry->ChannelIsValid(devId, channelId));
    channelId = PROF_CHANNEL_HBM;
    EXPECT_EQ(true, entry->ChannelIsValid(devId, channelId));
}
