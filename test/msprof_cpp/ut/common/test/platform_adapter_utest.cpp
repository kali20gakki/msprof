...
TEST_F(PlatformAdapterUtest, PlatformAdapterInterfaceModule1)
{
    ...
    std::string events;
    ret = PlatformAdapterInterfaceMgr->GetMetricsEvents(metricsType, events, CoreType::AI_CORE);
    ...
    ret = PlatformAdapterInterfaceMgr->GetMetricsEvents(metricsType, events, CoreType::AI_CORE);
    EXPECT_EQ(PROFILING_SUCCESS, ret);
    ret = PlatformAdapterInterfaceMgr->GetMetricsEvents(metricsType, events, CoreType::AI_VECTOR_CORE);
    EXPECT_EQ(PROFILING_SUCCESS, ret);
    metricsType = "xxx";
    ret = PlatformAdapterInterfaceMgr->GetMetricsEvents(metricsType, events, CoreType::AI_VECTOR_CORE);
    EXPECT_EQ(PROFILING_FAILED, ret);
