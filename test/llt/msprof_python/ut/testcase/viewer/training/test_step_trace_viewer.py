...
#     def test_query_top_total_2(self):
#         with mock.patch(NAMESPACE + '.TrainingTraceSaver.create_trace_conn', side_effect=sqlite3.DatabaseError), \
#              mock.patch(NAMESPACE + '.logging.error'):
#             res = StepTraceViewer.query_top_total(message)
#         self.assertEqual(res, {})
...
#         with mock.patch(NAMESPACE + '.TrainingTraceSaver.create_trace_conn', return_value=test_sql[0]):
#             res = StepTraceViewer.query_top_total(message)
...
#         db_manager = DBManager()
#         test_sql = db_manager.create_table(DBNameConstant.DB_TRACE)
#         test_sql[1].execute(create_sql)
#         with mock.patch(NAMESPACE + '.TrainingTraceSaver.create_trace_conn', return_value=test_sql[0]):
#             res = StepTraceViewer.query_top_total(message)
#         (test_sql[1]).execute("drop Table {}".format(DBNameConstant.TABLE_TRAINING_TRACE))
#         db_manager.destroy(test_sql)
...
#     # def test_get_step_trace_summary(self): XXX
#     #     message = {'job_id': 'job_default', 'device_id': '4'}

#     #     with mock.patch(NAMESPACE + '.StepTraceViewer.get_step_trace_data', side_effect=ValueError), \
#     #             mock.patch(NAMESPACE + '.logging.error'):
#     #         res = StepTraceViewer.get_step_trace_summary(message)
#     #     self.assertEqual(res, ([], [], 0))

#     #     message = {'job_id': 'job_default', 'device_id': '4', 'project_path': ''}
...
#     # def test_step_trace_timeline(self): XXX
...
#     #     with mock.patch(NAMESPACE + '.PathManager.get_sql_dir', return_value=db_manager.db_path):
#     #         res = StepTraceViewer.get_step_trace_timeline(message)

#     #     if os.path.exists(db_manager.db_name):
#     #         os.remove(db_manager.db_name)
#     #     self.assertEqual(len(res), 2153)

#     # def test_get_one_iter_timeline_data(self): XXX
#     #     db_manager, conn, curs = create_trace_db()
#     #     conn.close()
#     #     InfoConfReader()._info_json = {'DeviceInfo': [{'hwts_frequency': 100}]}
