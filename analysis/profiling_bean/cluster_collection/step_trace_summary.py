...
class StepTraceSummay:
    ...
    def _export_summary_data(self: any) -> None:
        output_file_name = "step_trace_{}_{}_{}.json".format(self.npu_id, self.model_id, self.iteration_id)
        output_file_path = os.path.join(self.collection_path, MsprofQuerySummaryManager.FOLDER_NAME_TO_STORAGE,
                                        output_file_name)
    ...
    def _process_in_cluster_scene(self: any) -> None:
        if not self._check_cluster_db_valid():
            return
        self._check_query_all_devices_scene()
        rank_id_collection = list(self._get_all_rank_ids())
    ...
    def _check_query_all_devices_scene(self: any) -> None:
    ...
    def _query_in_cluster_scene(self: any, rank_id_collection: list) -> list:
        ...
        if not (conn and curs):
            return data
        if self.all_devices:
            data = self._get_data_from_all_device(curs, rank_id_collection)
        else:
            if self.npu_id not in rank_id_collection:
                return data
            data = self._get_data_from_single_device(curs)
    ...
    def _get_data_from_all_device(self: any, curs: any, rank_id_collection: list) -> list:
        data_colleciton = []
    ...
    def _get_data_from_single_device(self, curs: any) -> list:
    ...
    def _sql_for_query_all_iteration(self: any, table_name: str) -> str:
        sql = "select (case when model_id={0} then 'N/A' else model_id end), " \
              ...
              "(case when bp_end={1} then 'N/A' else iteration_end - bp_end end), " \
              "(case when iteration_time={1} or iteration_end={1} then 'N/A' else " \
              "iteration_end - iteration_time end), " \
              "(case when fp_start={1} then 'N/A' else FP_start end), " \
              "(case when bp_end={1} then 'N/A' else BP_end end), " \
              "(case when iteration_end={1} then 'N/A' else iteration_end end) " \
              "from {2} where model_id={3}".format(
    ...
    def _process_in_non_cluster_scene(self: any) -> None:
        ...
        if project_dir:
            data = self._query_in_non_cluster_scene(project_dir)
    ...
    def _find_info_file_dir(self: any) -> str:
        ...
        for sub_dir in sub_dirs:
            ...
            for child_sub_dir in child_sub_dirs:
                ...
                if InfoConfReader().get_rank_id() != -1:
    ...
    def _sql_for_non_cluster(self: any, table_name: str) -> str:
        sql = "select device_id, " \
              ...
              "iteration_id, " \
              "(case when iteration_time={1} then 'N/A' else iteration_time*{2} end), " \
              "(case when fp_bp_time={1} then 'N/A' else fp_bp_time*{2} end), " \
              "(case when data_aug_bound={1} then 'N/A' else data_aug_bound*{2} end), " \
              "(case when bp_end={1} then 'N/A' else iteration_end - bp_end end), " \
              "(case when iteration_time={1} or iteration_end={1} then 'N/A' else " \
              "(iteration_end - iteration_time)*{2} end), " \
              "(case when FP_start={1} then 'N/A' else FP_start*{2} end), " \
              "(case when BP_end={1} then 'N/A' else BP_end*{2} end), " \
              "(case when iteration_end={1} then 'N/A' else iteration_end*{2} end) " \
