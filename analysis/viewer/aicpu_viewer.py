...
class ParseAiCpuData:
    ...
    def analysis_aicpu(project_path: str, iteration_id: int, model_id: int) -> tuple:
        ...
        ai_cpu_results = DBManager.fetch_all_data(
            ai_cpu_curs, ParseAiCpuData.get_ai_cpu_sql(project_path, model_id, iteration_id, ai_cpu_conn))
        DBManager.destroy_db_connect(ai_cpu_conn, ai_cpu_curs)
        return headers, ai_cpu_results

    @staticmethod
    def get_ai_cpu_sql(project_path: str, model_id: int, iteration_id: int, ai_cpu_conn: any) -> str:
        """
        ...
        if not ProfilingScene().is_operator():
            ...
            if iter_time:
                sql = "select sys_start,op_name,compute_time,memcpy_time,task_time,dispatch_time, total_time " \
                      ...
                      "and {3}.model_id={model_id} and ({3}.index_id={index_id} or {3}.index_id=0) " \
                      "and {3}.task_type='{task_type}'" \
                    .format(DBNameConstant.TABLE_AI_CPU, iter_time[0][0] / NumberConstant.MS_TO_US,
                            iter_time[0][1] / NumberConstant.MS_TO_US, DBNameConstant.TABLE_GE_TASK,
