...
public:
    DPUTrackTraceEvent(uint32_t pid, int tid, double dur, const std::string &ts, const std::string &name,
            ...
            : DurationEvent(pid, tid, dur, ts, name), threadId_(threadId), streamId_(streamId),
              taskId_(taskId), taskType_(taskType) {}
private:
...
public:
    DPUHcclTraceEvent(uint32_t pid, int tid, double dur, const std::string &ts, const std::string &name,
                      ...
                      uint16_t npuDeviceId, uint32_t aicpuTaskId, uint16_t planeId, const std::string &notifyId,
                      ...
                      const std::string &role, const std::string &cclTag, const std::string &workFlowMode,
                      const std::string &stage)
            : DurationEvent(pid, tid, dur, ts, name), threadId_(threadId), streamId_(streamId),
              taskId_(taskId), opType_(opType), npuDeviceId_(npuDeviceId), aicpuTaskId_(aicpuTaskId),
              planeId_(planeId), notifyId_(notifyId), durationEstimated_(durationEstimated),
              localRank_(localRank), remoteRank_(remoteRank), transportType_(transportType),
              dataSize_(dataSize), bandwidth_(bandwidth), dataType_(dataType), linkType_(linkType),
              rdmaType_(rdmaType), role_(role), cclTag_(cclTag), workFlowMode_(workFlowMode), stage_(stage) {}
private:
    void ProcessArgs(JsonWriter &ostream) override;
private:
    uint32_t threadId_;
