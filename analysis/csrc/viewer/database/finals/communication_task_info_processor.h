...
****************************************************************************** */
/* ******************************************************************************
 * File Name          : communication_task_info_processor.h
 * Description        : communication_task_info_processor，处理HCCLSingleDevice表相关数据
 * Author             : msprof team
 * Creation Date      : 2023/12/16
 * *****************************************************************************
 */
#ifndef ANALYSIS_VIEWER_DATABASE_COMMUNICATION_TASK_INFO_PROCESSOR_H
#define ANALYSIS_VIEWER_DATABASE_COMMUNICATION_TASK_INFO_PROCESSOR_H
...
class CommunicationTaskInfoProcessor : public TableProcessor {
    ...
    using ProcessedDataFormat = std::vector<std::tuple<uint64_t, uint64_t, uint64_t, uint32_t, uint64_t, uint64_t,
                                                         ...
                                                         uint64_t>>;
public:
    CommunicationTaskInfoProcessor() = default;
    CommunicationTaskInfoProcessor(const std::string &reportDBPath, const std::set<std::string> &profPaths);
    virtual ~CommunicationTaskInfoProcessor() = default;
    bool Run() override;
protected:
    bool Process(const std::string &fileDir) override;
private:
    static OriDataFormat GetData(DBInfo &hcclSingleDeviceDB);
...
#endif // ANALYSIS_VIEWER_DATABASE_COMMUNICATION_TASK_INFO_PROCESSOR_H
