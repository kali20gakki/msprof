...
public:
    ...
    static uint16_t GetTaskId(const std::shared_ptr<MsprofCompactInfo> &task);
    static uint16_t GetBatchId(const std::shared_ptr<MsprofCompactInfo> &task);
    static std::shared_ptr<FlipTask> CreateFlipTask(const std::shared_ptr<MsprofCompactInfo> &taskTrack);

private:
    using CompactInfoMap = std::unordered_map<uint16_t, std::vector<std::shared_ptr<MsprofCompactInfo>>>;
    using FlipTaskMap = std::unordered_map<uint16_t, std::vector<std::shared_ptr<FlipTask>>>;
    ...
    static void SetBatchId(std::shared_ptr<MsprofCompactInfo> &task, uint32_t batchId);
};  // class Flip
}  // namespace Adapter
}  // namespace Parser
}  // namespace Analysis
