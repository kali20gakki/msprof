...
class HwtsIterModel(ParserModel):
    ...
    def get_task_offset_and_sum(self: any, iter_id: int, data_type: str) -> (int, int):
        ...
        task_count = 0
        task_offset = 0
        sql = "select {1} from {0} where iter_id=?".format(DBNameConstant.TABLE_HWTS_ITER_SYS, data_type)
        try:
            curr_num = self.cur.execute(sql, (iter_id,)).fetchone()
        except sqlite3.Error as err:
            logging.error(str(err), exc_info=Constant.TRACE_BACK_SWITCH)
            return task_offset, task_count
        if curr_num:
    ...
    def get_batch_list(self: any, iter_id: int, table_name) -> list:
        """
        get batch list from hwts batch table
        :return: batch list
        """
        sql = "select batch_id from {0} where iter_id=?".format(table_name)
