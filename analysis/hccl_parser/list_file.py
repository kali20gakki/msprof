...
def get_dir_hwts(devid, dirpath):
    for path, dir_list, file_list in os.walk(dirpath):
        for file_name in file_list:
            if ('hwts.data' in file_name and 'done' not in file_name
...
def get_filename_hccl(devfile_list):
    filename_list = []

    for devfile in devfile_list:
        filename = devfile.rsplit("/", 1)
        filename_list.append(filename[1])
