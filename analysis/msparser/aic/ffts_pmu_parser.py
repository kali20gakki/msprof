...
class FftsPmuParser(IParser, MsMultiProcess):
    ...
    def parse(self: any) -> None:
        ...
        :return: None
        """
        ffts_pmu_files = self._file_list.get(DataTag.FFTS_PMU, [])
        ffts_pmu_files.sort(key=lambda x: int(x.split("_")[-1]))
        for _file in ffts_pmu_files:
    ...
    def _parse_binary_file(self: any, file_path: str) -> None:
        ...
        with open(file_path, 'rb') as _pmu_file:
            _file_size = os.path.getsize(file_path)
            for chunk in Utils.chunks(_pmu_file.read(_file_size), self.AIC_PMU_SIZE):
    ...
    def ms_run(self: any) -> None:
        ...
        :return:None
        """
        if self._file_list.get(DataTag.FFTS_PMU, []):
            self.parse()
            self.save()
