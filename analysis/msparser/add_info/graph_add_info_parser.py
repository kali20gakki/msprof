...
from msparser.nano.nano_exeom_bean import NanoExeomBean
...
class GraphAddInfoParser(DataParser, MsMultiProcess):
    ...
    def parse(self: any) -> None:
        """
        parse ge graph data or nano's model info
        """
        ...
        nano_exeom_info_files = self._file_list.get(DataTag.NANO_MODEL_EXEOM, [])
        ...
        for file_list in nano_exeom_info_files.values():
            self._graph_info_data.extend(self.parse_bean_data(file_list, StructFmt.NANO_MODEL_EXEOM_SIZE,
                                                              NanoExeomBean, self._get_graph_info_data))
    ...
    def ms_run(self: any) -> None:
        ...
        if not self._file_list.get(DataTag.GRAPH_ADD_INFO, []) and \
                not self._file_list.get(DataTag.NANO_MODEL_EXEOM, []):
            return
        try:
            self.parse()
