...
class DPUHcclInfoParser(DataParser, MsMultiProcess):
    ...
    def reformat_data(self) -> list:
        ...
        for data in self._hccl_info_data:
            ...
            reformat.append(
                [data.npu_device_id, data.dpu_device_id, data.thread_id, data.start_time, data.timestamp,
                 hash_data.get(data.item_id, data.item_id),
                 data.group_name, data.local_rank, data.remote_rank, data.rank_size,
                 data.duration_estimated, data.src_addr, data.dst_addr, data.data_size,
                 data.stream_id, data.task_id, data.aicpu_task_id,
                 data.plane_id, op_type, data_type, link_type, transport_type, rdma_type, role,
                 data.ccl_tag, data.notify_id, data.work_flow_mode, data.stage])
        return reformat
