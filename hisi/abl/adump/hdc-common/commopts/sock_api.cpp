 ...
 */
static int32_t SockHalRead(int32_t fd, IdeBuffT readBuf, int32_t recvLen, int32_t flag)
{
...
static int32_t SockProtoRead(int32_t fd, IdeRecvBuffT readBuf, IdeI32Pt recvLen, int32_t flag)
{
    ...
    if (SockHalRead(fd, (IdeBuffT)&proto, sizeof(MsgProto), flag) != sizeof(MsgProto)) {
    ...
    IDE_LOGI("recvTotalLen: %u, sizeof(MsgProto): %u", recvTotalLen, sizeof(MsgProto));
    if (ret != EOK) {
    ...
    if (proto.totalLen != 0) {
        if ((uint32_t)SockHalRead(fd, buffer + sizeof(MsgProto), proto.totalLen, flag) != proto.totalLen) {
