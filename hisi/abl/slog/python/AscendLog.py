...
class AscendLog:
    ...
    def __init__(self: any) -> None:
        ...
        try:
            self.log = ctypes.cdll.LoadLibrary('libalog.so')
        except OSError:
            ld_path = os.getenv('LD_LIBRARY_PATH')
            if ld_path is None:
                print('[Warning]Can not find libalog.so')
                return
            path_list = ld_path.split(':')
            for path in path_list:
                target_path = os.path.join(path, 'libalog.so')
    ...
    def debug(self: any, module: any, fmt: str) -> None:
        ...
        if self.log is None:
            return
        self.log.DlogInnerForC(ctypes.c_int(module), self.level.debug,
    ...
    def info(self: any, module: any, fmt: str) -> None:
        ...
        if self.log is None:
            return
        self.log.DlogInnerForC(ctypes.c_int(module), self.level.info,
                               ctypes.c_char_p(fmt.encode("utf-8")))
    ...
    def warn(self: any, module: any, fmt: str) -> None:
        ...
        if self.log is None:
            return
        self.log.DlogInnerForC(ctypes.c_int(module), self.level.warning,
                               ctypes.c_char_p(fmt.encode("utf-8")))
    ...
    def error(self: any, module: any, fmt: str) -> None:
        ...
        if self.log is None:
            return
        self.log.DlogInnerForC(ctypes.c_int(module), self.level.error,
                               ctypes.c_char_p(fmt.encode("utf-8")))
    ...
    def event(self: any, module: any, fmt: str) -> None:
        ...
        if self.log is None:
            return
        self.log.DlogInnerForC(ctypes.c_int(module), self.level.event,
                               ctypes.c_char_p(fmt.encode("utf-8")))
