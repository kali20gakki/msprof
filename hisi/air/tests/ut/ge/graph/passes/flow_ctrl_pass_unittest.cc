...
TEST_F(UtestGraphPassesFlowCtrlPass, create_iter_ctrl_false_branch_insert_model_exit_succ) {
  ...
  Status ret = flow_ctrl_pass.CreateIterCtrlFalseBranch(graph, loop_cond, loop_reset, switch_node);
  EXPECT_EQ(ret, SUCCESS);
  EXPECT_EQ(graph->GetDirectNodesSize(), 8);
  auto model_exit = graph->FindNode("switch_node_ModelExit");
  EXPECT_NE(model_exit, nullptr);
}
}  // namespace ge
