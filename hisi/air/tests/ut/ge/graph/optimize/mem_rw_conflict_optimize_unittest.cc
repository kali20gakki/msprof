...
ComputeGraphPtr BuildGraph_ex() {
  auto builder = ut::GraphBuilder("test");
  auto id1 = builder.AddNode("id1", IDENTITY, 1, 0);
  auto rea = builder.AddNode("rea", READVARIABLEOP, 1, 0);
  auto allreduce = builder.AddNode("allreduce", HCOMALLREDUCE, 1, 1);
  auto if_node = builder.AddNode("if", IF, 1,0);
  ...
  builder.AddDataEdge(id1, 0, if_node, 0);
  builder.AddControlEdge(rea, allreduce);
...
TEST(UtestGraphPassesHcclMemcpyPass, HandleMemoryRWConflict) {
  ComputeGraphPtr graph = BuildGraph_ex();
  GraphOptimize graph_optimizer;
  EXPECT_NE(graph_optimizer.HandleMemoryRWConflict(graph), SUCCESS);
}
}  // namespace ge
