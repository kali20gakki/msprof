...
TEST_F(UtestKernelExTaskInfo, success_kernel_ex_task_release) {
  ...
  void *addr = nullptr;
  rtMalloc(&addr, 64, RT_MEMORY_HBM);
...
TEST_F(UtestKernelExTaskInfo, success_kernel_ex_task_info_copy) {
  ...
  EXPECT_EQ(kernel_ex_task_info.Init(task_def, &model), FAILED);  // if (sizeof(STR_FWK_OP_KERNEL) < args_size)
...
TEST_F(UtestKernelExTaskInfo, testUpdateArgs) {
  ...
  model.op_list_[0]->SetWorkspace({1308});   // offset
  model.op_list_[0]->SetWorkspaceBytes({150});    // length
  kernel_ex_task_info.Init(task_def, &model);
...
TEST_F(UtestKernelExTaskInfo, testHeadFile) {
  ...
  uint32_t task_id = kernel_ex_task_info.GetTaskID();
  uint32_t stream_id = kernel_ex_task_info.GetStreamId();
  uintptr_t dump_args = kernel_ex_task_info.GetDumpArgs();
