...
#define MSPROF_FUSION_DATA_RESERVE_BYTES 8
...
#define PROF_MAX_DEV_NUM 64
typedef struct MsprofCommandHandle {
  uint64_t profSwitch;
  uint64_t profSwitchHi;
  uint32_t devNums;
  uint32_t devIdList[PROF_MAX_DEV_NUM];
  uint32_t modelId;
  uint32_t type;
  struct MsprofCommandHandleParams params;
} MsprofCommandHandle_t;
#endif
