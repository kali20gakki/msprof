...
typedef enum rtEventIdType {
  RT_EVENT_AICPU_MSG = 4,
  RT_EVENT_QUEUE_ENQUEUE = 6,
...
typedef enum rtGroupType {
  ...
  RT_GRP_TYPE_BIND_DP_CPU_EXCLUSIVE,      /* Bind to a AICPU, intra-group threads are mutex awakened */
} rtGroupType_t;
...
RTS_API rtError_t rtMemQueueInitQS(int32_t devId, const char* grpName) WEAKFUC;

RTS_API rtError_t rtMemQueueInitFlowGw(int32_t devId, const rtInitFlowGwInfo_t * const initInfo) WEAKFUC;
...
RTS_API rtError_t rtMemQueueGetQidByName(int32_t device, const char *name, uint32_t *qId) WEAKFUC;
...
RTS_API rtError_t rtQueueSubscribe(int32_t devId, uint32_t qid, uint32_t groupId, int32_t type) WEAKFUC;

RTS_API rtError_t rtQueueSubF2NFEvent(int32_t devId, uint32_t qid, uint32_t groupId) WEAKFUC;
...
RTS_API rtError_t rtEschedAttachDevice(int32_t device) WEAKFUC;

RTS_API rtError_t rtEschedCreateGrp(int32_t devId, uint32_t grpId, rtGroupType_t type) WEAKFUC;

RTS_API rtError_t rtEschedSubmitEvent(int32_t devId, rtEschedEventSummary_t *event) WEAKFUC;

RTS_API rtError_t rtEschedWaitEvent(int32_t devId,
                                    uint32_t grpId,
                                    uint32_t threadId,
                                    int32_t timeout,
                                    rtEschedEventSummary_t *event) WEAKFUC;

RTS_API rtError_t rtEschedSubscribeEvent(int32_t devId,
                                         uint32_t grpId,
                                         uint32_t threadId,
                                         uint64_t eventBitmap) WEAKFUC;
