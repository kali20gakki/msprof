 ...
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtKernelLaunchWithHandle(void *handle, const void *devFunc, uint32_t blockDim, rtArgsEx_t *args,
 ...
 * @param [in] stubFunc   stub function
 * @param [in] blockDim   block dimentions
 * @param [in] arg_ex   argments address for kernel function
 * @param [in] smDesc   shared memory description
 * @param [in] stream   associated stream
 ...
 * @return RT_ERROR_INVALID_VALUE for error input
 */
RTS_API rtError_t rtKernelLaunchWithFlag(const void *stubFunc, uint32_t blockDim, rtArgsEx_t *arg_ex,
