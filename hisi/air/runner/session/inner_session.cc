...
const int32_t kDecimalSystem = 10;
...
Status InnerSession::Initialize() {
  ...
  if (GetContext().GetOption("ge.session_device_id", str_session_device_id) == SUCCESS) {
    ...
    try {
      uint32_t session_device_id = static_cast<uint32_t>(std::stoi(str_session_device_id.c_str()));
      // session device id has priority
      GetContext().SetCtxDeviceId(session_device_id);
    } catch (std::invalid_argument &) {
      GELOGW("session device id %s transform to int32_t failed.", str_session_device_id.c_str());
    } catch (std::out_of_range &) {
      GELOGW("session device id %s transform to int32_t failed.", str_session_device_id.c_str());
    }
  }

  GE_CHK_STATUS_RET(MdsUtils::SetDevice(GetContext().DeviceId()), "Set device failed.");
  ...
  const int32_t DEFAULT_DEVICE_ID = 0;
  const int32_t DEFAULT_JOB_ID = 0;
...
Status InnerSession::RunGraph(uint32_t graph_id, const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs) {
  ...
  if (mutex_.try_lock()) {
    ...
    std::vector<GeTensor> geInputs;
    for (auto &item : inputs) {
      geInputs.push_back(TensorAdapter::AsGeTensor(item));
    }
    std::vector<GeTensor> geOutputs;
...
Status InnerSession::BuildGraph(uint32_t graph_id, const std::vector<InputTensorInfo> &inputs) {
  ...
  Status ret = graph_manager_.BuildGraph(graph_id, ge_inputs, flow_model, session_id_, true);
  if (ret != SUCCESS) {
...
void InnerSession::SetTrainFlagOption() {
  ...
  GELOGI("Train flag is %d in session, run mode is %s", train_flag, run_mode.c_str());
}
}  // namespace ge
