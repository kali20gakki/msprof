...
Status SessionManager::SetRtContext(SessionId session_id, rtContext_t rt_context) const {
  ...
  GE_CHK_STATUS_RET(
      MdsUtils::CtxCreate(&rt_context, RT_CTX_NORMAL_MODE, static_cast<int32_t>(GetContext().DeviceId())));
...
Status SessionManager::DestroySession(SessionId session_id) {
  ...
  std::lock_guard<std::mutex> lock(mutex_);
  std::map<SessionId, SessionPtr>::const_iterator it = session_manager_map_.find(session_id);
  if (it == session_manager_map_.cend()) {
...
Status SessionManager::AddGraph(SessionId session_id, uint32_t graph_id, const Graph &graph,
  ...
  SessionPtr innerSession = nullptr;
  {
    ...
    for (auto sub_graph : compute_graph->GetAllSubgraphs()) {
      AttrUtils::SetStr(*sub_graph, ATTR_NAME_SESSION_GRAPH_ID, session_graph_id);
    }
  }
...
bool SessionManager::HasSession(SessionId session_id) const {
  if (!init_flag_) {
    GELOGE(GE_SESSION_MANAGER_NOT_INIT,
