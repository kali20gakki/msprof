...
void InitOptions(const std::map<std::string, std::string> &option_map, ge::Options &options) {
...
Status CheckOptionsValid(const std::map<std::string, std::string> &options) {
...
Status GEInitializeImpl(const std::map<std::string, std::string> &options) {
  ErrorManager::GetInstance().GenWorkStreamIdDefault();
  GELOGT(TRACE_INIT, "GEInitialize start");
  for (const auto &option : options) {
    GELOGI("GEInitialize option, key: %s, value: %s.", option.first.c_str(), option.second.c_str());
  }
...
void PrintOutputResult(std::vector<Tensor> &outputs) {
  ...
  std::map<ge::DataType, size_t>::const_iterator iter = CONST_OPDATA_TYPE_SIZE_MAP.find(data_type);
  if (iter == CONST_OPDATA_TYPE_SIZE_MAP.cend()) {
