...
const char *const kShapeRangeValueConvertError = "transfer from std::string to int64 error";
...
static bool StringToLongNoThrow(const std::string &str, long &val) {
  try {
    ...
    REPORT_INPUT_ERROR("E10048", std::vector<std::string>({"shape_range", "reason", "sample"}),
                       std::vector<std::string>({str, kShapeRangeValueConvertError, kInputShapeRangeSample3}));
...
static bool ParseShapeRangePair(const std::string &shape_range,
  ...
  if (range_pair_set.size() == 1) {
    ...
    if ((range_left < 0) || (range_right < 0)) {
      REPORT_INPUT_ERROR("E10048", std::vector<std::string>({"shape_range", "reason", "sample"}),
                         std::vector<std::string>({shape_range, kInputShapeRangeInvalid, kInputShapeRangeSample3}));
...
Status CheckInputFormat(const std::string &input_format) {
  ...
  if (!ge::TypeUtils::IsFormatValid(input_format.c_str())) {
    ErrorManager::GetInstance().ATCReportErrMessage("E10001", {"parameter", "value", "reason"},
                                                    {"--input_format", input_format, "input format is invalid!"});
...
Status ParseInputShapeRange(const std::string &shape_range,
  ...
  const int32_t DEFAULT_SHAPE_RANGE_PAIR_SIZE = 2;
  for (const auto &shape_range_item : shape_range_vec) {
    ...
    std::string shape_range_str = shape_range_pair_vec[1];
...
Status ParseInputShapeRange(const std::string &shape_range,
  ...
  if (shape_range_set.empty()) {
    REPORT_INPUT_ERROR("E10048", std::vector<std::string>({"shape_range", "reason", "sample"}),
                       std::vector<std::string>({shape_range, kInputShapeRangeInvalid, kInputShapeRangeSample4}));
    GELOGE(PARAM_INVALID, "[Parse][ShapeRange] str:%s invalid, reason: %s, correct sample is %s.",
...
Status CheckSparseParamValid(const std::string &sparsity) {
  if ((sparsity != "0") && (sparsity != "1")) {
    ErrorManager::GetInstance().ATCReportErrMessage("E10006", {"parameter", "value"},
                                                    {"sparsity", sparsity});
