...
static graphStatus CheckGlobalOptions(std::map<std::string, std::string> &global_options) {
  ...
  global_options[ge::OPTION_EXEC_HCCL_FLAG] = IR_OPTION_DISABLE_DEFAULT;
...
graphStatus aclgrphBuildInitializeImpl(std::map<std::string, std::string> &global_options) {
  ...
  int32_t ret = ErrorManager::GetInstance().Init(path_base);
  if (ret != 0) {
...
class Impl {
 public:
  ...
  bool IsAttrIndexSetByUser(const ComputeGraphPtr &compute_graph, size_t &data_num, std::vector<int64_t> &attr_index);
...
graphStatus Impl::UpdateDataOpAttr(const Graph &graph) {
  ...
  std::string input_shape = (options_.find(kInputShape) == options_.cend()) ? "" : options_[kInputShape];
  std::string input_shape_range =
      (options_.find(kInputShapeRange) == options_.cend()) ? "" : options_[kInputShapeRange];
...
graphStatus Impl::CheckBuildModeAndBuildStep() {
  ...
  if (it != options_.end() && !(it->second.empty())) {
    ...
    if (build_mode == BUILD_MODE_TUNING) {
      REPORT_INPUT_ERROR("E10001", std::vector<std::string>({"parameter", "value", "reason"}),
          std::vector<std::string>({BUILD_STEP, "null", "tuning must specify build step. Please check!"}));
...
graphStatus Impl::CheckOptions(const std::map<std::string, std::string> &options) {
  ...
  auto build_mode_iter = options_.find(ge::ir_option::SHAPE_GENERALIZED_BUILD_MODE);
  bool mode_is_invalid = build_mode_iter != options_.end() &&
                         build_mode_iter->second != kShapeGeneralized && build_mode_iter->second != kShapePrecise;
  if (mode_is_invalid) {
    REPORT_INNER_ERROR("E19999", "Value[%s] of SHAPE_GENERALIZED_BUILD_MODE is invalid, must be shape_generalized "
...
graphStatus aclgrphSaveModel(const std::string &output_file, const ModelBufferData &model) {
  ...
  return FileSaver::SaveToFile((output_file + ".om"), static_cast<void *>(model.data.get()),
...
graphStatus aclgrphSaveModel(const char_t *output_file, const ModelBufferData &model) {
  ...
  return FileSaver::SaveToFile((str_output_file + ".om"), static_cast<void *>(model.data.get()),
                               static_cast<uint32_t>(model.length));
}
...
graphStatus aclgrphGenerateForOp(const AscendString &op_type, const std::vector<TensorDesc> &inputs,
  ...
  if (generator.BuildSingleOpGraph(op_desc, inputs_outputs, graph_name, graph, inputs_name_type) != ge::SUCCESS) {
