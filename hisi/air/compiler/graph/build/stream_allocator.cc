...
bool IsHcclOp(const std::string &op_type) {
  const std::set<std::string> hccl_op_types({ge::HCOMBROADCAST, ge::HCOMALLGATHER,
...
Status StreamAllocator::AssignLogicalStreams(const std::map<std::string, int32_t> &max_parallel_num,
  ...
  const std::map<std::string, SchedulerConf> &scheduler_confs = gelib->DNNEngineManagerObj().GetSchedulers();
...
///      \               v
///        - - event - > N2
Status StreamAllocator::OptimizeBySendEvents(const std::map<int64_t, std::vector<NodePtr>> &stream_nodes) {
  for (const auto &one_pair : stream_nodes) {
    ...
    for (const auto &recv_node_ptr : nodes) {
      ...
      std::vector<uint32_t> recv_events;
...
///   V                  V
///   N2 - - - event - > N2
Status StreamAllocator::OptimizeByRecvEvents(const std::map<int64_t, std::vector<NodePtr>> &stream_nodes) {
  for (const auto &one_pair : stream_nodes) {
    // The nodes on a stream in order
    const std::vector<NodePtr> &nodes = one_pair.second;
    ...
    for (const auto &send_node_ptr : nodes) {
      ...
      std::vector<uint32_t> send_id_list;
      ...
      for (const auto &event_id : send_id_list) {
        ...
        std::map<NodePtr, uint32_t>::const_iterator it = recv_node_to_event_id.find(recv_node_ptr);
...
Status StreamAllocator::SplitStreams(std::vector<std::set<int64_t>> &split_streams) {
  ...
  std::vector<int64_t> stream_node_num_vec(stream_num_);
  std::vector<int64_t> added_stream_num_vec(stream_num_);
  std::vector<int64_t> new_stream_id_vec(stream_num_);
  ...
  std::vector<NodePtr> pre_node_vec(stream_num_);
  ...
  for (const auto &cur_node : whole_graph_->GetNodes(whole_graph_->GetGraphUnknownFlag())) {
    ...
    if (NeedSpiltNewStream(stream_node_num_vec[stream_id], max_node_num_one_stream, op_desc, is_stream_first_node)) {
      last_stream_id++;
      GELOGI("stream[%ld]'s node num[%ld] > max_node_num_one_stream[%ld], split stream to %ld, first node[name: %s, "
             "type: %s, owner graph: %s].", stream_id, stream_node_num_vec[stream_id], max_node_num_one_stream,
             last_stream_id, op_desc->GetName().c_str(), op_desc->GetType().c_str(),
...
Status StreamAllocator::ReuseEvent(bool send_to,
  ...
  for (const auto &node_event_id : node_to_event_id) {
    ...
    for (const auto &event_pair : node_event_id.second) {
      ...
      auto reuse_event_id = [] (std::vector<uint32_t> &event_list, uint32_t old_event, uint32_t new_event) -> void {
...
Status StreamAllocator::RefreshContinuousEvents() {
  ...
  for (auto &one_pair : node_to_send_events_) {
    ...
    for (size_t i = 0; i < send_events.size(); i++) {
      ...
      if (find_it == old_to_new_events.cend()) {
...
Status StreamAllocator::InsertSyncEventNodes() {
  unordered_map<std::string, uint32_t> sync_event_name;
  for (const auto &node : whole_graph_->GetNodes(whole_graph_->GetGraphUnknownFlag())) {
    // Add the node corresponding to the recv event
    std::vector<uint32_t> recv_event_id_list;
    ...
    for (auto &event_id : recv_event_id_list) {
      std::string recv_node_name = whole_graph_->GetName() + "_Recv_" + to_string(event_id);
    ...
    std::vector<uint32_t> send_event_id_list;
...
Status StreamAllocator::GetMaxStreamAndTask(bool huge_stream, uint32_t &max_stream_count,
  ...
  if (ret != RT_ERROR_NONE) {
    ...
    GELOGE(FAILED, "[Call][RtGetMaxStreamAndTask] Get max stream and task count by rts failed, "
           "ret:%d, stream_type:%u,", static_cast<int32_t>(ret), stream_type);
    return FAILED;
  }
...
int64_t StreamAllocator::GetMaxNodeNumPerStream(const NodePtr &node, uint32_t max_task_count) const {
...
void StreamAllocator::AddNodeNum(const NodePtr &node, int64_t &node_num) const {
  node_num++;
...
void StreamAllocator::GetSendEventIdList(const NodePtr &node, std::vector<uint32_t> &send_list) const {
  send_list.clear();
  auto find_it = node_to_send_events_.find(node);
  if (find_it != node_to_send_events_.end()) {
...
void StreamAllocator::GetRecvEventIdList(const NodePtr &node, std::vector<uint32_t> &recv_list) const {
  recv_list.clear();
  auto find_it = node_to_recv_events_.find(node);
  if (find_it != node_to_recv_events_.end()) {
...
NodePtr StreamAllocator::GetNodeFromSendEventId(uint32_t send_event_id) const {
  for (const auto &one_pair : node_to_send_events_) {
    const std::vector<uint32_t> &events = one_pair.second;
...
Status StreamAllocator::AddEventId(const NodePtr &pre_node, const NodePtr &not_cur, const NodePtr &cur_node,
                                   bool not_use_cur) {
  auto recv_node = not_use_cur ? not_cur : cur_node;
  GE_CHECK_NOTNULL(recv_node);
  if (recv_node->GetOwnerComputeGraph()->GetParentNode() == pre_node) {
    GELOGD("No need to insert event between node[name: %s, type: %s, graph: %s] and its parent node[name: %s, type: %s,"
           " graph: %s].", recv_node->GetName().c_str(), recv_node->GetType().c_str(),
           recv_node->GetOwnerComputeGraph()->GetName().c_str(), pre_node->GetName().c_str(),
           pre_node->GetType().c_str(), pre_node->GetOwnerComputeGraph()->GetName().c_str());
    return SUCCESS;
  }
...
Status StreamAllocator::AddActiveNodes(
    const NodePtr &switch_node, const std::vector<std::string> &ori_active_label_list,
