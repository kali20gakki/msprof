...
bool IsNodeAndPeerNodeTaskSupportZeroCopy(const ge::NodePtr &node, uint32_t output_index) {
  const static std::set<std::string> kTaskUnsupportZeroCopyOp{ge::IDENTITY};
  GELOGD("Check node %s and peer node of output %u task zero copy supported", node->GetName().c_str(), output_index);

  if (kTaskUnsupportZeroCopyOp.count(node->GetType()) > 0U) {
    return false;
  }
  ...
  bool support = std::all_of(peer_anchors.begin(), peer_anchors.end(), [](const ge::InDataAnchorPtr &anchor) {
    return ((anchor != nullptr) && (anchor->GetOwnerNode() != nullptr) &&
            (kTaskUnsupportZeroCopyOp.count(anchor->GetOwnerNode()->GetType()) == 0U));
  });

  GELOGD("Task of node %s and peer node of output %u %s zero copy", node->GetName().c_str(), output_index,
         (support ? "support" : "not support"));
  return support;
}

bool IsOutputFlowToOnlyOneOutput(const ge::NodePtr &node, uint32_t output_index) {
  auto out_anchor = node->GetOutDataAnchor(static_cast<int32_t>(output_index));
  if (out_anchor == nullptr) {
    return false;
  }

  bool anchor_combined_with_netoutput = false;
  for (const auto &peer_in_anchor : out_anchor->GetPeerInDataAnchors()) {
    if ((peer_in_anchor == nullptr) || (peer_in_anchor->GetOwnerNode() == nullptr)) {
      return false;
    }
    auto peer_node = peer_in_anchor->GetOwnerNode();
    if ((peer_node->GetType() == ge::NETOUTPUT) && (peer_node->GetOwnerComputeGraph() != nullptr) &&
        (peer_node->GetOwnerComputeGraph()->GetParentGraph() == nullptr ||
         ((ge::GraphUtils::FindRootGraph(peer_node->GetOwnerComputeGraph()) != nullptr) &&
          (ge::GraphUtils::FindRootGraph(peer_node->GetOwnerComputeGraph())->GetGraphUnknownFlag()) &&
          (!peer_node->GetOwnerComputeGraph()->GetGraphUnknownFlag())))) {  // Combined with NET-OUTPUT of root graph
      if (anchor_combined_with_netoutput) {  // already combined with outputs, this means output flow to multi outputs
        return false;
      }
      anchor_combined_with_netoutput = true;
    }
  }

  if (anchor_combined_with_netoutput) {
    GELOGI("Node %s output %u flow to root graph output", node->GetName().c_str(), output_index);
    return true;
  }
  return false;
}
...
std::string ToString(ge::NodeTypeIndex &x) {
  std::stringstream ss;
...
/// |--child block1-1-|
///
bool CanIntervalLifeReuse(const MemoryBlock &parent_block, const MemoryBlock &child_block) {
...
bool MemoryBlock::IsSameBatchLabel() const {
  ...
  for (size_t index = 1; index < node_type_index_list_.size(); ++index) {
    ...
    const auto &index_op_desc = node_type_index_list_[index].node->GetOpDesc();
...
bool CanNotLifeReuse(const MemoryBlock *const block) {
...
void MemoryBlock::AddContinuousLifeReuseBlock(MemoryBlock *block,
                                              DependStreamLife &total_node_depend_stream_life) {
...
void MemoryBlock::AddLifeReuseBlock(MemoryBlock *block, DependStreamLife &total_node_depend_stream_life) {
  if (CanNotLifeReuse(this) || CanNotLifeReuse(block) || (batch_label_ != block->batch_label_) ||
      (memory_type_ != block->memory_type_)) {
    return;
  }
...
size_t MemoryBlock::GetLifeBegin() const {
  size_t life_time = 0;
...
size_t MemoryBlock::GetDependLifeBegin(int64_t stream_id, DependStreamLife &total_node_depend_stream_life) {
  ...
  std::map<int64_t, size_t>::const_iterator it = depend_stream_life_.find(stream_id);
  if (it == depend_stream_life_.cend()) {
    return 0;
  }
  return it->second;
}
...
void AddDependLife(const ge::NodePtr &org_node, const ge::NodePtr &node, int64_t stream_id,
  ...
  if (node->GetType() == STREAMMERGE) {
    GELOGD("Node [%s] will interrupt memory reuse among streams", node->GetName().c_str());
    return;
  }
...
void SetLastUsedInputMemAttr(const NodePtr &node, int32_t input_index) {
  if (node == nullptr) {
    return;
  }
...
Status GetNoAlignSize(const ge::OpDesc &desc, uint32_t index, size_t &size) {
  ...
  (void)AttrUtils::GetBool(output_op_desc, ATTR_NAME_TENSOR_NO_TILING_MEM_TYPE, is_no_tiling);
  graphStatus graph_status;
  if (is_no_tiling) {
    graph_status = TensorUtils::CalcTensorMemSizeForNoTiling(*output_op_desc, format, data_type, tensor_size);
  } else {
...
BlockMemAssigner::BlockMemAssigner(ComputeGraphPtr compute_graph,
                                   const std::map<std::string, std::string> &anchor_to_symbol,
...
void BlockMemAssigner::MarkContinuousAllocedForOneInputFromVariable(const NodePtr &node) const {
  auto node_op_desc = node->GetOpDesc();
...
void BlockMemAssigner::GetOutAndWorkSpaceMem(std::vector<int64_t> &all_memory_size) {
...
///
size_t GetBlockSize(size_t size, const std::vector<int64_t> &ranges) {
...
bool IsDirectOutputNode(const NodePtr &node) {
...
bool BlockMemAssigner::IsOutNodeSetContinuousInput(const NodePtr &n, uint32_t out_index, std::string &peer_name,
  ...
  if (static_cast<size_t>(out_index) < n->GetAllOutDataAnchors().size()) {
    ...
    for (auto const &peer_in_anchor : out_anchor->GetPeerInDataAnchors()) {
      ...
      if (is_input_continuous) {
        if (n->GetOwnerComputeGraph() != nullptr) {
          std::string graph_name = n->GetOwnerComputeGraph()->GetName();
          ...
          if (node_continuous_input_blocks_[peer_in_node_desc->GetName()].size() == 0) {
            ...
            GE_IF_BOOL_EXEC(has_lx_fusion_attr,
                            ...
                            continue;);
            node_continuous_input_counts_[peer_in_node_desc->GetName()] = std::make_pair(
                peer_in_node_desc->GetType(), peer_node->GetAllInDataAnchorsSize());
          }
    ...
    if (is_out_node_continuous_input) return true;
  }
  return false;
}
...
bool BlockMemAssigner::IsContinuousMemoryReuse(const NodePtr &n, const NodePtr &peer_node, uint32_t out_index) {
  ...
  for (const auto &in_anchor : peer_node->GetAllInDataAnchors()) {
    ...
    if (static_cast<size_t>(peer_out_node_desc->GetId()) < continuous_life_begin_) {
      ...
      GELOGI("Node[%s] life[%ld] output[%u] is not continuous input node[%s] life[%ld]'s min life time,"
    ...
    if (!IsContinuousInputNodeMaxLife(in_anchor->GetPeerOutAnchor()->GetOwnerNode(),
      ...
      GELOGI("Node[%s] life[%ld] output[%u]'s continuous input node[%s] life[%ld]'s is not node[%s] output[%d]'s "
             "max life node",
             n->GetName().c_str(), node_desc->GetId(), out_index, peer_node_desc->GetName().c_str(),
...
void BlockMemAssigner::InitReuseFlag() {
  ...
  for (const auto &pair : symbol_to_anchors_) {
    ...
    if ((mem_type == RT_MEMORY_P2P_DDR) || (mem_type == RT_MEMORY_HOST)) {
      symbol_to_mem_type_[symbol] = mem_type;
    }

    for (const auto &node_index_io : pair.second) {
      GE_CHECK_NOTNULL_EXEC(node_index_io.node_, continue);
      GE_CHECK_NOTNULL_EXEC(node_index_io.node_->GetOpDesc(), continue);
      if ((node_index_io.node_->GetType() == DATA) &&
          node_index_io.node_->GetOpDesc()->HasAttr(ATTR_NAME_PARENT_NODE_INDEX)) {
        GELOGD("Node: %s is subgraph data, continue", node_index_io.node_->GetName().c_str());
        continue;
      }
      ...
      for (const auto &in_anchor : out_anchor->GetPeerInDataAnchors()) {
        if (IsDirectOutputNode(in_anchor->GetOwnerNode())) {
          out_flg = true;
          break;
        }
      }
...
/// @return bool
///
bool BlockMemAssigner::IsSymbolExist(const NodeIndexIO &node_index_io, std::string &symbol) const {
...
/// @return bool
///
bool BlockMemAssigner::IsSymbolDescBlockExist(const NodeIndexIO &node_index_io, std::string &symbol) const {
  auto iter = anchor_to_symbol_.find(node_index_io.ToString());
  if (iter == anchor_to_symbol_.end()) {
    return false;
  }
  ...
  symbol = iter->second;
  return symbol_desc_blocks_.find(iter->second) != symbol_desc_blocks_.end();
}
...
void BlockMemAssigner::GetSymbolMemType(std::list<NodeIndexIO> node_index_io_list, int64_t &memory_type) const {
...
void BlockMemAssigner::UpdateOpTensorMemType(std::list<NodeIndexIO> node_index_io_list, int64_t memory_type) const {
  for (auto &node_index_io : node_index_io_list) {
    auto op_desc = node_index_io.node_->GetOpDesc();
    if (op_desc == nullptr) {
      GELOGW("Node[%s] op desc is null.", node_index_io.node_->GetName().c_str());
...
bool BlockMemAssigner::IsContinuousOutput(const NodePtr &n) const {
  if (n == nullptr) {
...
bool BlockMemAssigner::IsZeroCopyBlock(const NodePtr &node, uint32_t output_index, bool continuous) const {
  if (NodeUtils::IsDynamicShape(node)) {
    return ((node->GetType() == DATA_TYPE) && !continuous) || (node->GetType() == NETOUTPUT) ||
           (IsOutputFlowToOnlyOneOutput(node, output_index) &&
            IsNodeAndPeerNodeTaskSupportZeroCopy(node, output_index));
  }

  if (continuous) {  // Never zero copy for data flow to require-continuous-input node
    GELOGD("Node %s output %u can not zero copy as require continuous output", node->GetName().c_str(), output_index);
    return false;
  }

  if (node->GetType() == NETOUTPUT) {
    const auto &owner = node->GetOwnerComputeGraph();
    return (owner != nullptr) && (owner->GetParentGraph() == nullptr);
  }

  if ((node->GetType() == DATA_TYPE)) {
    if (node->GetOpDesc()->HasAttr(ATTR_NAME_PARENT_NODE_INDEX)) {  // Never zero copy for subgrapgh data
      return false;
    }
    if (IsStrictReuseZeroMemoryMode()) {
  ...
  if (IsOutputFlowToOnlyOneOutput(node, output_index)) {
...
namespace {
void MarkZeroCopyBlockAttr(OpDescPtr op_desc, bool is_zero_copy, bool mem_type, uint32_t out_index) {
  if (is_zero_copy && (mem_type == kOutput)) {
    auto output_desc = op_desc->MutableOutputDesc(out_index);
    if (output_desc != nullptr) {
      output_desc->SetAttr(ATTR_IS_ZERO_COPY_BLOCK, GeAttrValue::CreateFrom<bool>(true));
    } else {
      GELOGE(PARAM_INVALID, "Node %s output %u is zero copy block but not marked as output desc is nullptr",
             op_desc->GetName().c_str(), out_index);
    }
  }
}
}  // namespace
...
MemoryBlock *BlockMemAssigner::ApplyMemory(size_t block_size, size_t real_size, size_t no_align_size,
                                           ...
                                           const std::vector<bool> &workspace_reuse_flag, const bool is_op_reuse_mem,
  ...
  if (ge_disable_reuse_mem_env_ != "1" && mem_type != kOutputDesc) {
    ...
    if (do_reuse) {
      if (mem_release_first_reuse_first_) {
        MemoryBlock *reusable_block = GetFirstReleaseBlock(block_size, real_size, no_align_size, mem_type, n, out_index,
                                                           continuous, memory_type, thread_scope_id, batch_label);
        if (reusable_block != nullptr) {
          return reusable_block;
        }
      } else {
        MemoryBlock *reusable_block = GetLastReleaseBlock(block_size, real_size, no_align_size, mem_type, n, out_index,
                                                          continuous, memory_type, thread_scope_id, batch_label);
        if (reusable_block != nullptr) {
          return reusable_block;
        }
  ...
  block->is_zero_copy_ = ((mem_type == kOutput) && IsZeroCopyBlock(n, out_index, continuous));
  MarkZeroCopyBlockAttr(node_op_desc, block->is_zero_copy_, mem_type, out_index);
...
MemoryBlock *BlockMemAssigner::GetFirstReleaseBlock(const size_t block_size, const size_t real_size,
                                                    const size_t no_align_size, const OpMemoryType mem_type,
                                                    const ge::NodePtr &node, const uint32_t out_index,
                                                    const bool continuous, const uint64_t memory_type,
                                                    const int32_t thread_scope_id, const std::string &batch_label) {
  GELOGI("The block usage strategy: release first, reuse first.");
  const auto stream_id = node->GetOpDesc()->GetStreamId();
  for (auto it = reusable_blocks_[memory_type][stream_id].begin();
    ...
    if (CanReuseBlock(thread_scope_id, continuous_life_begin_, *reusable_block, block_size)) {
      reusable_block->AddNodeTypeIndex({node, mem_type, out_index, false, continuous_life_begin_, thread_scope_id},
                                       real_size, no_align_size);
      if (mem_type == kOutput) {
        const auto iter = anchor_to_symbol_.find(NodeIndexIO(node, out_index, kOut).ToString());
        if (iter != anchor_to_symbol_.end()) {
...
MemoryBlock *BlockMemAssigner::GetLastReleaseBlock(const size_t block_size, const size_t real_size,
                                                   const size_t no_align_size, const OpMemoryType mem_type,
                                                   const ge::NodePtr &node, const uint32_t out_index,
                                                   const bool continuous, const uint64_t memory_type,
                                                   const int32_t thread_scope_id, const std::string &batch_label) {
  GELOGI("The block usage strategy: release last, reuse first.");
  const auto stream_id = node->GetOpDesc()->GetStreamId();
...
Status BlockMemAssigner::ApplyContinuousMemory(const NodePtr &n, const std::vector<int64_t> &ranges,
  ...
  if (block != nullptr) {
    ...
    block->need_same_offset_in_batch_ = SizeIndependentOfBatch(n->GetType());
    ++(block->ref_count_);
  } else {
...
int32_t BlockMemAssigner::GetAllRefCount(const NodeIndexIO &out_node_index_io) const {
  int32_t ref_count = 0;
  auto iter = symbol_to_anchors_.find(out_node_index_io.ToString());
  if (iter != symbol_to_anchors_.end()) {
    for (const auto &node_index_io : iter->second) {
      if ((node_index_io.io_type_ != kIn) || (node_index_io.node_ == nullptr)) {
        continue;
      }
      if ((node_index_io.node_->GetInDataAnchor(node_index_io.index_) == nullptr) ||
          (node_index_io.node_->GetInDataAnchor(node_index_io.index_)->GetPeerOutAnchor() == nullptr)) {
        GELOGI("Node: %s has no input.", node_index_io.node_->GetName().c_str());
        continue;
      }
      ref_count++;
    }
...
MemoryBlock *BlockMemAssigner::ApplyOutMemory(const NodePtr &n, uint32_t index, const std::vector<int64_t> &ranges,
  ...
  GE_IF_BOOL_EXEC(
      ...
      return nullptr);
  GE_IF_BOOL_EXEC(GetTensorSize(*output_op_desc, size) != SUCCESS, GELOGI("Get size failed"));
  ...
  if (IsSymbolExist(node_index_io, symbol)) {
    ...
    if (iter1 != anchor_to_symbol_.end()) {
      std::map<std::string, size_t>::const_iterator iter2 = symbol_size_.find(iter1->second);
      if (iter2 != symbol_size_.cend()) {
        max_size = iter2->second;
      }
      std::map<std::string, int64_t>::const_iterator iter3 = symbol_to_mem_type_.find(iter1->second);
      if (iter3 != symbol_to_mem_type_.cend()) {
    ...
    int32_t ref_count = GetAllRefCount(node_index_io);
    // in case symbol ref ref_count is total input ref
    if (ref_count > out_count) {
      GELOGI("Node name: %s ref count: %d, out count: %d", n->GetName().c_str(), ref_count, out_count);
      out_count = ref_count;
    }
...
MemoryBlock *BlockMemAssigner::ApplyOutDescMemory(const NodePtr &n, uint32_t index,
  ...
  GELOGI("Node[%s] tensor[%u] apply output desc memory.", n->GetName().c_str(), index);

  GE_CHECK_NOTNULL_EXEC(n, return nullptr);
  ...
  NodeIndexIO node_index_io(n, index, kOut);
  std::string symbol;
  if (IsSymbolDescBlockExist(node_index_io, symbol)) {
    block = symbol_desc_blocks_[symbol];
    GE_IF_BOOL_EXEC(block == nullptr,
      REPORT_INNER_ERROR("E19999", "get ref block failed, node_name:%s, symbol:%s",
                         n->GetName().c_str(), node_index_io.ToString().c_str());
      GELOGE(FAILED, "[Get][RefBlock]node_name:%s, symbol:%s",
             n->GetName().c_str(), node_index_io.ToString().c_str());
      return nullptr);

    block->AddNodeTypeIndex({n, kOutputDesc, index, true, 0}, size, size);
    GELOGD("Ref tensor desc, symbol[%s], anchor[%s].", symbol.c_str(), node_index_io.ToString().c_str());
  } else {
    block = ApplyMemory(block_size, size, size, kOutputDesc, n, index, workspace_reuse_flag,
                        false, false, RT_MEMORY_HBM);
    if (block == nullptr) {
      REPORT_CALL_ERROR("E19999", "apply out desc Memory failed, node_name:%s, block_size:%ld, out_index:%u",
                        n->GetName().c_str(), block_size, index);
      GELOGE(FAILED, "[Apply][Memory]node_name:%s, block_size:%ld, out_index:%u", n->GetName().c_str(),
             block_size, index);
      return nullptr;
    }
  }
    ...
    for (auto &index : atomic_output_index) {
      if (static_cast<uint32_t>(index) == output_index) {
        if (node->GetOwnerComputeGraph() != nullptr) {
          std::string graph_name = node->GetOwnerComputeGraph()->GetName();
...
void BlockMemAssigner::ReleaseMemory(MemoryBlock *to_release, std::vector<MemoryBlock *> &reusable_memory,
                                     bool same_stream) {
...
void BlockMemAssigner::ReleaseMemorys(const std::vector<MemoryBlock *> &to_releases,
                                      std::vector<MemoryBlock *> &reusable_memory) {
  for (auto mem_block : to_releases) {
    ReleaseMemory(mem_block, reusable_memory);
  }
}

void BlockMemAssigner::ReleaseInputNodeOutMemory(const unordered_map<std::string,
    std::vector<MemoryBlock *>> &node_out_blocks, std::vector<MemoryBlock *> &reusable_memory, const NodePtr &node) {
  for (const auto &in_anchor : node->GetAllInDataAnchors()) {
    ...
    for (auto block : it->second) {
      const std::vector<NodeTypeIndex> &node_type_indexs = block->NodeTypeIndexList();
...
void BlockMemAssigner::CheckAndReleaseSuspendedBlock(const NodePtr &node, uint32_t idx, MemoryBlock *block) {
  ...
  auto out_data_anchor = node->GetOutDataAnchor(static_cast<int32_t>(idx));
  ...
  if (is_suspended) {
    ...
    GELOGI("The output is suspended, and will be released in allocation of next node. Name:%s, index:%u, "
           "size:%zu, ref_count:%d, batch:%s, stream:%ld, reuse_mem:%d.", node->GetName().c_str(), idx, block->Size(),
           block->ref_count_, block->batch_label_.c_str(), block->stream_id_, block->reuse_mem_);
  }
}

Status BlockMemAssigner::GetTensorSize(GeTensorDesc &tensor_desc, int64_t &size) {
  GE_IF_BOOL_EXEC(ge::TensorUtils::GetSize(tensor_desc, size) != SUCCESS, GELOGI("Get size failed"));
...
Status BlockMemAssigner::AssignOutputMemoryWithReuse(const NodePtr &node, std::vector<int64_t> &ranges) {
  ...
  for (uint32_t i = 0; i < static_cast<uint32_t>(op_desc->GetOutputsSize()); i++) {
    ...
    GE_IF_BOOL_EXEC(GetTensorSize(*output_tensor_desc, size) != SUCCESS, GELOGI("Get size failed"));
    ...
    if (mem_block != nullptr) {
      ...
      node_out_blocks_[mem_block->memory_type_][node->GetName()].emplace_back(mem_block);
      GE_IF_BOOL_EXEC(out_node_set_continuous_input,
...
///
void BlockMemAssigner::AssignMemoryWithReuse(std::vector<int64_t> &ranges) {
  ...
  std::string op_no_reuse_mem_str;
  const char *op_no_reuse_mem = std::getenv(OP_NO_REUSE_MEM);
  GE_IF_BOOL_EXEC(op_no_reuse_mem != nullptr, op_no_reuse_mem_str = std::string(op_no_reuse_mem);
  ...
  (void)AttrUtils::GetBool(compute_graph_, ATTR_NAME_MEM_RELEASE_FIRST_REUSE_FIRST, mem_release_first_reuse_first_);
  ...
  for (NodePtr &n : compute_graph_->GetAllNodes()) {
    ...
    for (auto &reusable_block : reusable_blocks_) {
      auto mem_type_to_node_out_blocks = node_out_blocks_.find(reusable_block.first);
      GE_IF_BOOL_EXEC(
          (mem_type_to_node_out_blocks != node_out_blocks_.end()),
          ReleaseInputNodeOutMemory(mem_type_to_node_out_blocks->second, reusable_block.second[stream_id], n));
    }
  }
...
void BlockMemAssigner::GetNodeWorkSpaceSize(const NodePtr &node, std::vector<int64_t> &workspace_memory,
  ...
  std::vector<int64_t> workspace_byte_nums = node->GetOpDesc()->GetWorkspaceBytes();
...
void BlockMemAssigner::AssignContinuousBlocks() {
  for (auto &block_map : node_continuous_input_blocks_) {
    ...
    GELOGI("Node:%s continuous input block count:%zu input count:%u, size_independent:%d", block_map.first.c_str(),
...
void BlockMemAssigner::ResizeDynamicBatchBlocks() {
  ...
  for (auto block : memory_blocks_) {
    ...
    if (DynamicBatchBlockReuse(*block)) {
      ...
      // for example, hccl ops in the multi-batch training scenario.
      if (block->need_same_offset_in_batch_) {
        need_same_offset_blocks[block->batch_label_].emplace_back(block);
      } else {
        dynamic_batch_blocks[block->batch_label_].emplace_back(block);
      }
    }
  }
  DoResizeDynamicBatchBlocks(dynamic_batch_blocks);
  DoResizeDynamicBatchBlocks(need_same_offset_blocks);
}

void BlockMemAssigner::DoResizeDynamicBatchBlocks(
    const std::map<std::string, std::vector<MemoryBlock *>> &dynamic_batch_blocks) {
  if (dynamic_batch_blocks.empty()) {
    return;
  }
...
void SetOffsetSize(const NodeTypeIndex &node_type, const MemoryBlock *block,
  ...
  std::string graph_name = owner_graph->GetName();
  ...
  if (node_type.mem_type == kOutput) {
    ...
    static const std::set<std::string> kSetOffsetTypes = { DATA_TYPE, AIPP_DATA_TYPE, MULTISHAPE, NETOUTPUT };
  ...
  } else if (node_type.mem_type == kOutputDesc) {
    auto tensor = op_desc->MutableOutputDesc(node_type.index);
...
void SetBlockOpMemOffset(const MemoryBlock *block, int32_t child_block_level) {
  if (block == nullptr) {
    return;
  }
  size_t index = 0;
...
bool BlockMemAssigner::CheckIsZeroMemNodeType(const std::string &node_type) const {
  return (node_type == VARIABLE) || (node_type == CONSTANT) || (node_type == MULTISHAPE) ||
         (node_type == CONSTANTOP) || (node_type == HVDWAIT) || (node_type == FILECONSTANT);
}
