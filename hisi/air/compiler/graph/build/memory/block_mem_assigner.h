...
enum OpMemoryType { kOutput, kWorkspace, kOutputDesc };
 ...
 public:
  explicit MemoryBlock(size_t block_size, int64_t stream_id = 0, bool reuse_mem = true,
      ...
      : ref_count_(0),
        ...
        tail_offset_(0),
        child_offset_(0) {
    if (memory_type_ == RT_MEMORY_HOST) {
  ...
  void AddLifeReuseBlock(MemoryBlock *block, DependStreamLife &total_node_depend_stream_life);
  ...
  void AddDependLifeBegin(DependStreamLife &total_node_depend_stream_life);

  size_t GetDependLifeBegin(int64_t stream_id, DependStreamLife &total_node_depend_stream_life);
...
class BlockMemAssigner : public MemAssigner {
 ...
 protected:
  ...
  /// @return bool
  ///
  bool IsSymbolExist(const NodeIndexIO &node_index_io, std::string &symbol) const;
  ...
  /// @return bool
  ///
  bool IsSymbolDescBlockExist(const NodeIndexIO &node_index_io, std::string &symbol) const;
  ...
  /// @return void
  ///
  void GetSymbolMemType(std::list<NodeIndexIO> node_index_io_list, int64_t &memory_type) const;
  ...
  /// @return void
  ///
  void UpdateOpTensorMemType(std::list<NodeIndexIO> node_index_io_list, int64_t memory_type) const;
 ...
 private:
  ...
  Status GetTensorSize(GeTensorDesc &tensor_desc, int64_t &size);
  ///
  /// @ingroup GE
  ...
  /// @param [in] block_size applied memory block size
  /// @param [in] real_size actual memory size required
  /// @param [in] no_align_size no align memory size required
  /// @param [in] mem_type output memory or workspace memory
  /// @param [in] node node in compute_graph_
  /// @param [in] out_index output node index
  /// @param [in] continuous whether the memory of op is continuous
  /// @param [in] memory_type device memory type
  /// @param [in] thread_scope_id thread scope id
  /// @param [in] batch_label batch label
  /// @return MemoryBlock*
  /// @author
  ///
  MemoryBlock *GetFirstReleaseBlock(const size_t block_size, const size_t real_size,
  ...
  ///
  /// @ingroup GE
  /// @brief Get the block: release last, reuse first
  /// @param [in] block_size applied memory block size
  /// @param [in] real_size actual memory size required
  /// @param [in] no_align_size no align memory size required
  /// @param [in] mem_type output memory or workspace memory
  ...
  /// @author
  ///
  MemoryBlock *GetLastReleaseBlock(const size_t block_size, const size_t real_size,
                                   const size_t no_align_size, const OpMemoryType mem_type,
                                   const ge::NodePtr &node, const uint32_t out_index,
                                   const bool continuous, const uint64_t memory_type,
                                   const int32_t thread_scope_id, const std::string &batch_label);
  ...
  /// @author
  ///
  void ReleaseMemory(MemoryBlock *to_release, std::vector<MemoryBlock *> &reusable_memory, bool same_stream = true);
  ...
  /// @author
  ///
  void ReleaseMemorys(const std::vector<MemoryBlock *> &to_releases, std::vector<MemoryBlock *> &reusable_memory);
  ...
  /// @author
  ///
  void ReleaseInputNodeOutMemory(const std::unordered_map<std::string, std::vector<MemoryBlock *>> &node_out_blocks,
                                 std::vector<MemoryBlock *> &reusable_memory, const ge::NodePtr &node);
  ...
  void DoResizeDynamicBatchBlocks(const std::map<std::string, std::vector<MemoryBlock *>> &dynamic_batch_blocks);

  void AssignContinuousBlocks();

  bool IsZeroCopyBlock(const NodePtr &node, uint32_t output_index, bool continuous) const;
  ...
  bool IsContinuousOutput(const NodePtr &n) const;
  ...
  void MarkContinuousAllocedForOneInputFromVariable(const NodePtr &node) const;

  void CheckAndReleaseSuspendedBlock(const NodePtr &node, uint32_t idx, MemoryBlock *block);

  int32_t GetAllRefCount(const NodeIndexIO &out_node_index_io) const;
  ...
  std::unordered_map<int64_t, std::unordered_map<std::string, std::vector<MemoryBlock *>>> node_out_blocks_;

  std::unordered_map<std::string, MemoryBlock *> symbol_blocks_;

  std::unordered_map<std::string, MemoryBlock *> symbol_desc_blocks_;

  std::map<std::string, std::unordered_map<uint32_t, MemoryBlock *>> node_continuous_input_blocks_;

  std::map<std::string, std::pair<std::string, uint32_t>> node_continuous_input_counts_;

  // reuse memory
  std::vector<std::string> op_no_reuse_mem_vec_;
  ...
  bool mem_release_first_reuse_first_ = false;
};
}  // namespace ge
