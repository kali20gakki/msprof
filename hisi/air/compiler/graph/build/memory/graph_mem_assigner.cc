...
const uint32_t kMaxBlockRecursiveDepth = 10U;
...
Status GraphMemoryAssigner::SetMemReuseInfo() const {
  ...
  for (auto &memory_block : priority_assigner->GetMemoryBlocks()) {
    if (memory_block == nullptr || memory_block->is_zero_copy_ || memory_block->deleted_block_) {
      continue;
    }
    GELOGD("Begin to process memory block[%s] and record reuse info.", memory_block->String().c_str());
    for (const auto &node_type_index : memory_block->NodeTypeIndexList()) {
      const auto &node = node_type_index.node;
      GE_CHECK_NOTNULL(node);
      const auto &op_desc = node->GetOpDesc();
      GE_CHECK_NOTNULL(op_desc);
      GELOGD("Begin to set memory reuse info: node[%s], memory_type[%s], index[%u].", node->GetName().c_str(),
             node_type_index.GetMemType().c_str(), node_type_index.index);
      // Get reuse attr that already have reuse info on the current node
      std::map<std::string, std::vector<MemReuseInfo>> node_to_reuse_info =
          op_desc->TryGetExtAttr(ATTR_NAME_MEMORY_REUSE_INFO, std::map<std::string, std::vector<MemReuseInfo>>{});

      // Get the value, which is the reuse info of current node output or workspace
      std::vector<MemReuseInfo> mem_reuse_info;
      RecordSubsequentReuseNodeInfo(op_desc->GetId(), memory_block, mem_reuse_info);
      if (!mem_reuse_info.empty()) {
        // Get the key of current node and identify output index or workspace index: [memtype][index], eg:output0
        const std::string key = node_type_index.GetMemType() + std::to_string(node_type_index.index);
        GELOGD("Set memory reuse info attr for node[%s]: key is[%s], mem_reuse_info size is %zu.",
               node->GetName().c_str(), key.c_str(), mem_reuse_info.size());
        node_to_reuse_info[key] = mem_reuse_info;
        (void)node->GetOpDesc()->SetExtAttr(ATTR_NAME_MEMORY_REUSE_INFO, node_to_reuse_info);
      }
    }
  }
  return SUCCESS;
...
void GraphMemoryAssigner::RecordSubsequentReuseNodeInfo(const int64_t op_index, const MemoryBlock *const memory_block,
                                                        std::vector<MemReuseInfo> &mem_resue_info,
                                                        uint32_t recursive_depth) const {
  if (recursive_depth > kMaxBlockRecursiveDepth) {
    GELOGW("The system will stop recording subsequent reuse node info, because too many nesting levels(%u)",
           recursive_depth);
    return;
  }
  ...
  for (const auto &node_type_index : memory_block->NodeTypeIndexList()) {
    if (node_type_index.node != nullptr && node_type_index.node->GetOpDesc() != nullptr) {
      ...
      if (node_type_index.node->GetOpDesc()->GetId() > op_index) {
        MemReuseInfo reuse_info;
        reuse_info.node = node_type_index.node;
        reuse_info.index = node_type_index.index;
        reuse_info.mem_type = (node_type_index.GetMemType() == "output") ? MemType::OUTPUT_MEM : MemType::WORKSPACE_MEM;
        GELOGD("Record subsequent reuse node info : node[%s], op memory type[%s], index[%u]",
               reuse_info.node->GetName().c_str(), node_type_index.GetMemType().c_str(), node_type_index.index);
        mem_resue_info.emplace_back(reuse_info);
      }
    }
  }

  for (auto &child_block : memory_block->ChildBlockList()) {
    if (child_block != nullptr) {
      GELOGD("Begin to process child memory block[%s].", child_block->String().c_str());
      RecordSubsequentReuseNodeInfo(op_index, child_block, mem_resue_info, recursive_depth + 1U);
    }
  }
}
...
ge::Status CalculateTensorRealSizeAndOutSize(const ge::ConstGeTensorDescPtr &output_desc,
  ...
  if (dim_index >= static_cast<int64_t>(output_dims.size())) {
    REPORT_INNER_ERROR("E19999", "Inner param dim_index value:%ld invalid, bigger than dim size:%zu in shape:%s",
...
Status GraphMemoryAssigner::ReAssignMemory(std::map<uint64_t, size_t> &mem_type_to_offset) {
  ...
  GE_CHK_STATUS_RET(ReAssignContinuousMemory(),
...
Status GraphMemoryAssigner::SetMemOffset(const ge::NodePtr &node, const InDataAnchorPtr &in_data_anchor,
    bool reverse_refresh, int64_t &mem_offset, int64_t &continuous_mem_start) const {
  auto op_desc = node->GetOpDesc();
  GE_CHECK_NOTNULL(op_desc);
...
Status GraphMemoryAssigner::ReAssignAtomicMemory() {
  ...
  std::map<std::string, std::map<NodePtr, std::vector<NodePtr>>> normal_atomic_and_clean_nodes_map;
      ...
      for (const auto &peer_in_control_anchor : out_control_anchor->GetPeerInControlAnchors()) {
        if (peer_in_control_anchor != nullptr) {
          ...
          if (peer_in_node_desc != nullptr) {
            ...
            if (is_atomic_node) {
              if (!CheckAtomicNodeIsSupportRef(peer_in_node_desc)) {
...
bool GraphMemoryAssigner::CheckAtomicNodeIsSupportRef(const ge::OpDescPtr &op_desc) const {
  bool is_reference = false;
  // If GetBool fail, is_reference is false
  (void) ge::AttrUtils::GetBool(op_desc, ATTR_NAME_REFERENCE, is_reference);
  if (!is_reference) {
    return true;
  }
  ...
  if (atomic_output_index.size() > op_desc->GetOutputsSize()) {
    GELOGW("op[%s]: The size [%zu] of atomic output index is greater than output size[%zu].",
           op_desc->GetName().c_str(), atomic_output_index.size(), op_desc->GetOutputsSize());
    return false;
  }

  for (size_t i = 0; i < atomic_output_index.size(); ++i) {
    std::vector<std::string> all_input_name = op_desc->GetInputName();
    if (std::count(all_input_name.begin(), all_input_name.end(),
                   op_desc->GetOutputNameByIndex(atomic_output_index[i]))) {
      GELOGW("op[%s] output index[%zu] is both atomic and reference.", op_desc->GetName().c_str(), i);
      return false;
    }
  }
  return true;
...
Status GraphMemoryAssigner::AssignConnectNetOutputAtomicMemory(std::vector<NodePtr> &connect_netoutput_nodes) {
  MemoryOffsetMap::const_iterator iter = memory_offset_.find(RT_MEMORY_HBM);
  if (iter == memory_offset_.cend()) {
    REPORT_INNER_ERROR("E19999", "InnerData memory_offset_ does not have type[HBM], not expected, "
                       "graph_id:%u, graph_name:%s", compute_graph_->GetGraphID(), compute_graph_->GetName().c_str());
    GELOGE(FAILED, "[Check][InnerData]memory_offset_ does not have memory type[HBM]"
           "graph_id:%u, graph_name:%s", compute_graph_->GetGraphID(), compute_graph_->GetName().c_str());
...
Status GraphMemoryAssigner::AssignReferenceMemory() const {
  for (auto &node : compute_graph_->GetDirectNode()) {
    ...
    bool is_ref = false;
    // If GetBool fail, is_ref is false.
    (void) ge::AttrUtils::GetBool(out_op_desc, ATTR_NAME_REFERENCE, is_ref);
    if (!is_ref) {
      continue;
    }
    ...
    std::map<std::string, int32_t> input_name_index;
    for (const auto &input_name : out_op_desc->GetAllInputNames()) {
      int32_t index = out_op_desc->GetInputIndexByName(input_name);
      input_name_index.emplace(input_name, index);
    }

    for (auto &out_data_anchor : node->GetAllOutDataAnchors()) {
      std::string out_data_anchor_name = out_op_desc->GetOutputNameByIndex(out_data_anchor->GetIdx());
      std::map<std::string, int32_t>::const_iterator iter = input_name_index.find(out_data_anchor_name);
      if (iter != input_name_index.cend()) {
...
bool GraphMemoryAssigner::CheckInputIsSupportAtomic(const ge::NodePtr &node) const {
  for (auto &in_data_anchor : node->GetAllInDataAnchors()) {
    auto peer_out_data_anchor = in_data_anchor->GetPeerOutAnchor();
...
Status GraphMemoryAssigner::AssignAtomicOutputMemory(const ge::NodePtr &node, std::vector<int64_t> &mem_offset_end) {
  auto op_desc = node->GetOpDesc();
  ...
  if (node->GetType() == DATA) {
    ...
    if (is_no_tiling) {
      ...
      if (!AttrUtils::GetInt(input_tensor, ATTR_NAME_TENSOR_DESC_MEM_OFFSET, mem_offset)) {
        REPORT_INNER_ERROR("E19999", "Update op[%s] input no tiling mem offset by parent failed, parent node[%s].",
  ...
  for (const auto &anchor : node->GetAllInDataAnchors()) {
    ...
    if (is_no_tiling) {
      ...
      if (!AttrUtils::GetInt(out_tensor_desc, ATTR_NAME_TENSOR_DESC_MEM_OFFSET, mem_offset)) {
        REPORT_INNER_ERROR("E19999", "Update op[%s] input no tiling mem offset failed, peer node[%s].",
...
Status GraphMemoryAssigner::SetIndependentAtomicAttr(const ge::NodePtr &node,
                                                     int64_t atomic_mem_start,
  ...
  std::vector<int64_t> memory_offset_start;
  std::vector<int64_t> memory_offset_size;
...
ge::Status GraphMemoryAssigner::SetAtomicCleanAttr(const NodePtr &node, const std::vector<int64_t> &atomic_mem_start,
                                                   const std::vector<int64_t> &atomic_mem_size,
                                                   int64_t memory_type) const {
  auto node_op_desc = node->GetOpDesc();
  if (node_op_desc != nullptr) {
    GELOGD("Node %s, set atomic clean attr start.", node->GetName().c_str());
    std::vector<int64_t> workspace_vector = node_op_desc->GetWorkspace();
    std::vector<int64_t> workspace_byte_vector = node_op_desc->GetWorkspaceBytes();
    workspace_vector.insert(workspace_vector.cend(), atomic_mem_start.cbegin(), atomic_mem_start.cend());
    workspace_byte_vector.insert(workspace_byte_vector.cend(), atomic_mem_size.cbegin(), atomic_mem_size.cend());
    ...
    mem_start_vector.insert(mem_start_vector.cend(), atomic_mem_start.cbegin(), atomic_mem_start.cend());
    ...
    mem_size_vector.insert(mem_size_vector.cend(), atomic_mem_size.cbegin(), atomic_mem_size.cend());
    ...
    std::string atomic_mem_start_str = ss.str();
    ss.clear();
    ss.str("");
    for (auto iter : atomic_mem_size) {
      ss << iter << " ";
    }
    std::string atomic_mem_size_str = ss.str();
...
ge::Status GraphMemoryAssigner::GetNodeListMemoryType(const std::vector<NodePtr> &nodes, int32_t mem_reuse_model,
                                                      int64_t &memory_type) const {
...
void GraphMemoryAssigner::PrintMemoryOffset() const {
  for (auto pair : memory_offset_) {
