...
const uint64_t kProfilingFpStartLogid = 2;
const uint64_t kProfilingBpEndLogid = 3;
const uint64_t kProfilingArStartLogid = 10000;
const uint64_t kProfilingArEndLogid = 10001;
const uint64_t kProfilingIterEndLogid = 4;
...
const std::set<std::string> kHcclKernelInfoNames = {"ops_kernel_info_hccl", "ops_kernel_info_hccl_gradtune",
                                                    "hvd_ops_kernel_info"};
...
Status TaskGenerator::GetTaskInfo(Model &model, ComputeGraphPtr &graph, uint64_t session_id, RunContext &run_context) {
  ...
  std::map<uint32_t, std::string> op_name_map;
...
Status TaskGenerator::AddModelTaskToModel(const ModelTaskDef &model_task_def, uint64_t session_id, ge::Model &model,
  ...
  if (!model_task_def.SerializePartialToArray(serial_buff.GetData(), static_cast<int32_t>(task_size))) {
...
Status TaskGenerator::UpdateOpIsVarAttr(const OpDescPtr &op_desc, uint64_t session_id) const {
  ...
  if (!(output_offsets.empty())) {
    ...
    for (uint32_t i = 0; i < op_desc->GetAllOutputsDescSize(); i++) {
      ...
      output_var.push_back(VarManager::Instance(session_id)->IsVarAddr(output_offsets[valid_output_index] -
                                                                       inner_offset));
      valid_output_index++;
    }
...
Status TaskGenerator::GenTaskForNode(const NodePtr &node, RunContext &context,
                                     std::vector<domi::TaskDef> &tasks) const {
  // 1. call generate task by engine name
  GE_CHK_STATUS_RET(OpsKernelBuilderManager::Instance().GenerateTask(*node, context, tasks),
                    "[Generate][Task] fail for op:%s(%s)", node->GetName().c_str(), node->GetType().c_str());
  // 2. try to call generate task again by others engine
  GE_CHK_STATUS_RET(GenTaskForPartiallySupportedNode(node, context, tasks),
                    "[Generate][PartiallySupportedTask] fail for op:%s(%s)", node->GetName().c_str(),
                    node->GetType().c_str());
  GE_CHK_STATUS_RET(GenTaskForNodeByAliasEngine(node, context, tasks), "[Generate][MixL2Task] fail for op:%s(%s)",
...
Status TaskGenerator::GenTaskForPartiallySupportedNode(const NodePtr &node, RunContext &context,
  ...
  if ((instance_ptr == nullptr) || (!instance_ptr->InitFlag())) {
    ...
    REPORT_INNER_ERROR("E19999", "[Get][GELib] GELib is not init before, check invalid.");
  ...
  GE_CHECK_NOTNULL(ops_kernel_builder);
...
Status TaskGenerator::GenerateTask(RunContext &run_context, ComputeGraphPtr &graph,
  ...
  bool check_ge_lib_valid = (ge_lib == nullptr) || !ge_lib->InitFlag();
  if (check_ge_lib_valid) {
  ...
  std::function<void()> callback = [this, &is_unknown_shape, &run_context, &stream]() {
    if (is_unknown_shape) {
  ...
  GE_CHK_STATUS_RET(UpdateNodeAnchor(graph), "Update node anchor status failed.");
  ...
  for (auto &node : graph->GetNodes(graph->GetGraphUnknownFlag(), nullptr, ffts_filter)) {
    ...
    auto fusion_task_info =
        FusionTaskInfo{run_context,        graph,         node,        op_desc,         node_index,      ge_lib,
                       ops_kernel_manager, task_def_list, op_name_map, profiling_point, all_reduce_nodes, 0};
    ...
    ret = GenerateTaskForFftsPlus(node, run_context, task_def_list);
    ...
    for (size_t idx = task_list_size_before; idx < task_list_size_after; ++idx) {
      ...
      if (kHcclKernelInfoNames.count(op_kernel_lib_name) > 0) {
...
Status TaskGenerator::GenerateTaskForFftsPlus(const NodePtr &node, RunContext &run_context,
                                              std::vector<domi::TaskDef> &task_def_list) const {
  auto ret = SUCCESS;
  OpDescPtr op_desc = node->GetOpDesc();
  GE_CHECK_NOTNULL(op_desc);
  if (op_desc->HasAttr(ATTR_NAME_FFTS_PLUS_SUB_GRAPH)) {
    ...
    for (const auto &subgraph : subgraphs) {
      for (const auto &tmp_node : subgraph->GetAllNodes()) {
        auto tmp_op_dec = tmp_node->GetOpDesc();
        GE_CHECK_NOTNULL(tmp_op_dec);
        GE_CHK_STATUS_RET(UpdateOpIsVarAttr(tmp_op_dec, subgraph->GetSessionID()));
        bool attr_notask = false;
        bool get_attr_notask_flag = ge::AttrUtils::GetBool(tmp_op_dec, ATTR_NAME_NOTASK, attr_notask);
        GE_IF_BOOL_EXEC(get_attr_notask_flag && attr_notask,
                        GELOGI("Node[name:%s, type:%s] does not need to generate task.",
                               tmp_op_dec->GetName().c_str(), tmp_op_dec->GetType().c_str());
                            continue);
        ...
        if (ret != SUCCESS) {
          ...
          GELOGE(ret, "[Generate][Task] fail for op:%s(%s)", tmp_node->GetName().c_str(), tmp_node->GetType().c_str());
          return ret;
        }
      }
    }
    ...
    if (op_desc->HasAttr(ATTR_NAME_FFTS_SUB_GRAPH)) {
      std::vector<ComputeGraphPtr> subgraphs;
      GE_CHK_STATUS_RET(NodeUtils::GetDirectSubgraphs(node, subgraphs),
    ...
    ret = GenTaskForNode(node, run_context, task_def_list);
  }
  return ret;

}

Status TaskGenerator::GenerateTaskForFusionNode(const FusionTaskInfo &fusion_task_info,
                                                std::map<int64_t, std::vector<NodePtr>> &fusion_nodes,
                                                std::unordered_set<Node *> &fusion_nodes_seen) {
  Status ret = SUCCESS;
  int64_t group_key;
...
Status TaskGenerator::UpdateNodeAnchor(const ComputeGraphPtr &graph) const {
  for (const auto &node : graph->GetAllNodes()) {
    GE_CHECK_NOTNULL(node);
    GE_CHK_STATUS_RET(UpdateAnchorStatus(node),
                      "[Call][UpdateAnchorStatus] node:%s(%s) failed.",
                      node->GetName().c_str(),
                      node->GetType().c_str());
  }
  return SUCCESS;
...
Status TaskGenerator::MarkNodeAndSetIndex(const ComputeGraphPtr &graph) const {
  ...
  std::map<int64_t, std::vector<OpDescPtr>> all_stream_ops;
...
Status TaskGenerator::MarkFirstAndLastOps(const std::vector<OpDescPtr> &ops, bool is_single_stream) const {
  std::vector<std::vector<OpDescPtr>> continuous_op_lists(1);
  ...
  for (auto &op_desc : ops) {
    ...
    if ((!is_single_stream && !op_desc->GetSubgraphInstanceNames().empty()) || separator_types.count(op_type) != 0) {
      continuous_op_lists.emplace_back(std::vector<OpDescPtr>());
    } else {
  ...
  for (const auto &continuous_ops : continuous_op_lists) {
    std::map<std::string, std::pair<OpDescPtr, OpDescPtr>> first_and_last_ops;
...
bool TaskGenerator::IsGlobalStep(const NodePtr &node) const {
  ...
  if (op_desc == nullptr) {
    GELOGD("The pointer of operator description is nullptr.");
    return false;
  }
...
Status TaskGenerator::FindLastBpFromBpNode(const ComputeGraphPtr &graph, const NodePtr &target_node,
  ...
  for (auto &in_node : target_node->GetInAllNodes()) {
    ...
    if (IsGlobalStep(in_node)) {
      GELOGD("The Node name is %s", in_node->GetName().c_str());
      continue;
    }
...
Status TaskGenerator::FindBpOfEnv(const ComputeGraphPtr &graph, const std::string &bp_point_str,
                                  ProfilingPoint &profiling_point, std::vector<uint32_t> &all_reduce_nodes) const {
...
Status TaskGenerator::GetFpBpIndex(const ComputeGraphPtr &graph, ProfilingPoint &profiling_point,
                                   std::vector<uint32_t> &all_reduce_nodes, std::string &fp_point_str,
...
Status TaskGenerator::FindProfilingTaskIndex(const ComputeGraphPtr &graph, ProfilingPoint &profiling_point,
  ...
  bool is_profiling = ProfilingProperties::Instance().ProfilingOn() ||
...
Status TaskGenerator::InsertProfilingArTaskBefore(
    const OpDescPtr &op_desc, const std::vector<uint32_t> &all_reduce_nodes, uint32_t node_index,
    std::vector<domi::TaskDef> &task_def_list, bool is_insert_bp_profiling_task) const {
...
Status TaskGenerator::InsertProfilingTaskBefore(const OpDescPtr &op_desc, const ProfilingPoint &profiling_point,
                                                const std::vector<uint32_t> &all_reduce_nodes, uint32_t node_index,
                                                std::vector<domi::TaskDef> &task_def_list) const {
  bool is_profiling = ProfilingProperties::Instance().ProfilingOn() ||
                      ProfilingProperties::Instance().ProfilingTrainingTraceOn();
  ...
  bool no_insert_profiling_task = ((profiling_point.bp_index == 0) || (profiling_point.end_index.empty())) &&
  ...
  if ((profiling_point.fp_index == node_index) || is_insert_fp_profiling_task) {
    GELOGI("The first FP operator is %s, idx %u", op_desc->GetName().c_str(), node_index);
...
Status TaskGenerator::InsertProfilingArTaskAfter(
    const OpDescPtr &op_desc, const std::vector<uint32_t> &all_reduce_nodes, uint32_t node_index,
    std::vector<domi::TaskDef> &task_def_list, bool is_insert_bp_profiling_task) const {
  bool is_insert_all_reduce_task = false;
  int64_t ar_log_id = 0xFFFF;
...
Status TaskGenerator::InsertProfilingTaskAfter(const OpDescPtr &op_desc, const ProfilingPoint &profiling_point,
  ...
  bool no_insert_profiling_task = profiling_point.end_index.empty() &&
...
Status TaskGenerator::SetUnknownShapeStream(RunContext &run_context, rtStream_t &stream) const {
...
Status TaskGenerator::DestroyUnknownShapeStream(RunContext &run_context, rtStream_t &stream) const {
  GE_CHK_RT(rtModelUnbindStream(run_context.model, stream));
  GE_CHK_RT_RET(rtStreamDestroy(stream));
  return SUCCESS;
}

Status TaskGenerator::SetKnownShapeStream(RunContext &run_context, int64_t stream_id) const {
  if (stream_id < 0 || stream_id >= static_cast<int64_t>(run_context.graphStreamList.size())) {
