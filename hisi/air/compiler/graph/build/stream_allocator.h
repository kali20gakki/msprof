...
class StreamAllocator {
 ...
 private:
  ...
  Status InsertOneEventInTwoNodes(const NodePtr &cur_node, const NodePtr &next_node);
  ...
  int64_t GetMaxNodeNumPerStream(const NodePtr &node, uint32_t max_task_count) const;
  void AddNodeNum(const NodePtr &node, int64_t &node_num) const;
