...
bool LabelAllocator::CollectFunctionalNode(ComputeGraphPtr &graph, std::set<NodePtr> &functional_nodes) const {
  ...
  if (owner_graph->HasAttr(ATTR_NAME_PIPELINE_PARTITIONED)) {
    bool is_pipeline_partitioned = false;
    (void) AttrUtils::GetBool(owner_graph, ATTR_NAME_PIPELINE_PARTITIONED, is_pipeline_partitioned);
    if (is_pipeline_partitioned) {
      GELOGD("Graph[%s] is pipeline partitioned, skip label allocator.", owner_graph->GetName().c_str());
      return true;
    }
  }
