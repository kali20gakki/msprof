...
const std::string kMixCubeCoreType = "MIX_AIC";
const std::string kMixVectorCoreType = "MIX_AIV";
const std::string kMixCubeTBEKernelNamePrefix = "_mix_aic";
const std::string kMixVectorTBEKernelNamePrefix = "_mix_aiv";
...
Status ModelBuilder::AdjustConstWeightSize(const ge::NodePtr &node, size_t &mem_offset) const {
...
Status ModelBuilder::BuildModelDef(ge::Model &model) {
  ...
  GE_CHK_BOOL_EXEC(ge::AttrUtils::SetInt(&model, MODEL_ATTR_TASK_GEN_HOST_BASE_ADDR, kMemoryHostFeatureMapLogicBase),
      REPORT_INNER_ERROR("E19999", "Set Attr:%s in model failed", MODEL_ATTR_TASK_GEN_HOST_BASE_ADDR.c_str());
  ...
  GE_CHK_BOOL_EXEC(ge::AttrUtils::SetInt(&model, MODEL_ATTR_HOST_MEMORY_SIZE, host_memory_size),
                   REPORT_INNER_ERROR("E19999", "Set Attr:%s in model failed", MODEL_ATTR_HOST_MEMORY_SIZE.c_str());
  ...
  GE_CHK_BOOL_EXEC(ge::AttrUtils::SetListStr(&model, ATTR_MODEL_OUT_NODES_NAME, GetLocalOmgContext().net_out_nodes),
                   ...
                   return FAILED);
  GELOGI(
      "For model, max_mem_offset: %zu, host_max_mem_offset: %zu, p2p_mem_size: %zu, zero_copy_mem_size: %zu, "
      "session_scope_mem_size: %zu",
      max_mem_offset_, host_max_mem_offset_, p2p_mem_offset_, zero_copy_mem_size_, session_scope_mem_offset);
...
Status ModelBuilder::SaveAtomicTBEKernel(const OpDescPtr &op_desc) {
  ...
  (void)AttrUtils::SetStr(atomic_op_desc, ATTR_NAME_TBE_KERNEL_NAME_FOR_LOAD, tbe_kernel->GetName());
...
Status ModelBuilder::SaveNormalTBEKernel(const OpDescPtr &op_desc) {
  ...
  (void)AttrUtils::SetStr(op_desc, ATTR_NAME_TBE_KERNEL_NAME_FOR_LOAD, tbe_kernel->GetName());
  tbe_kernel_store_.AddTBEKernel(tbe_kernel);

  GE_CHK_STATUS_RET(SaveAtomicTBEKernel(op_desc),
                    "[Save][TBEKernel] Node[%s] type[%s] save atomic tbekernel failed!",
                    op_desc->GetName().c_str(), op_desc->GetType().c_str());
  GE_CHK_STATUS_RET(SavaAtomicWorkspace(op_desc),
...
Status ModelBuilder::SaveMixAicAivTBEKernel(const OpDescPtr &op_desc) {
  ...
  if ((core_type == kMixCubeCoreType) || (core_type == kMixVectorCoreType)) {
    GE_CHK_STATUS_RET_NOLOG(SaveMixTBE(kMixCubeTBEKernelNamePrefix, core_type));
    GE_CHK_STATUS_RET_NOLOG(SaveMixTBE(kMixVectorTBEKernelNamePrefix, core_type));
  }
...
TBEKernelPtr ModelBuilder::CreateOpTBEKernel(const OpDescPtr &op_desc, const std::string &prefix_kernel_name) const {
  ...
  if (!kernel_name.empty() && (kernel_buffer.GetSize() > 0U)) {
    GE_CHK_BOOL_EXEC_WARN((kernel_buffer.GetData() != nullptr), return nullptr,
                          "kernel data of op:%s(%s) is nullptr",
                          op_desc->GetName().c_str(), op_desc->GetType().c_str());
    std::vector<char> data(kernel_buffer.GetData(), kernel_buffer.GetData() + kernel_buffer.GetSize());
    tbe_kernel = MakeShared<OpKernelBin>(kernel_name, std::move(data));
    GE_CHK_BOOL_EXEC(tbe_kernel != nullptr, return nullptr, "[Create][TBEKernel] failed");
    std::string ext_attr_name = prefix_kernel_name + ge::OP_EXTATTR_NAME_TBE_KERNEL;
    GE_CHK_BOOL_EXEC_WARN(op_desc->SetExtAttr(ext_attr_name, tbe_kernel), return nullptr,
                          "set ext attr:%s for op:%s(%s) failed", op_desc->GetName().c_str(),
                          op_desc->GetType().c_str(), ext_attr_name.c_str());
  }
  return tbe_kernel;
}
...
Status ModelBuilder::SaveDataToModel(ge::Model &model, ge::GeModel &ge_model) {
  ...
  for (const auto &node : compute_graph_->GetNodes(compute_graph_->GetGraphUnknownFlag())) {
    ...
    GE_CHK_STATUS_RET_NOLOG(SaveMixAicAivTBEKernel(node_op_desc));
    ...
    if (node_op_desc->HasAttr(ATTR_NAME_FFTS_SUB_GRAPH) || node_op_desc->HasAttr(ATTR_NAME_FFTS_PLUS_SUB_GRAPH)) {
      ...
      for (const auto &sgt_node : sgt_graph->GetAllNodes()) {
        ...
        GE_CHK_STATUS_RET_NOLOG(SaveMixAicAivTBEKernel(sgt_op_desc));
      }
    }
  }
...
void ModelBuilder::DelNodeRepeatSaveAttr() {
  for (const ge::NodePtr &n : compute_graph_->GetNodes(compute_graph_->GetGraphUnknownFlag())) {
    ...
    (void)node_op_desc->DelAttr(kMixCubeTBEKernelNamePrefix + ATTR_NAME_TBE_KERNEL_BUFFER);
    (void)node_op_desc->DelAttr(kMixVectorTBEKernelNamePrefix + ATTR_NAME_TBE_KERNEL_BUFFER);
  }
}
...
Status ModelBuilder::BuildModelForGetTask(ge::Model &model) {
  ...
  GE_CHK_STATUS_RET(mem_assigner.AssignMemory(mem_type_to_mem_offset_, zero_copy_mem_size_),
  ...
  GE_TIMESTAMP_START(OptimizeStreamedWholeGraph);
  ...
  GE_TIMESTAMP_EVENT_END(OptimizeStreamedWholeGraph, "GraphBuilder::OptimizeStreamedWholeGraph");
