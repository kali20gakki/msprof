...
Status StreamGraphOptimizer::OptimizeStreamedWholeGraph(const ComputeGraphPtr &comp_graph) const {
  ...
  if ((instanece_ptr == nullptr) || !instanece_ptr->InitFlag()) {
    REPORT_INNER_ERROR("E19999", "Gelib is not inited before, check valid, graph:%s", comp_graph->GetName().c_str());
  ...
  for (const auto &iter : graph_optimizers) {
    ...
    if (ret != SUCCESS) {
      REPORT_INNER_ERROR("E19999", "Call OptimizeStreamedWholeGraph failed, ret:%d, engine_name:%s, graph_name:%s",
                         ret, iter.first.c_str(), comp_graph->GetName().c_str());
      GELOGE(ret, "[Call][OptimizeStreamedWholeGraph] failed, ret:%d, engine_name:%s, graph_name:%s", ret,
