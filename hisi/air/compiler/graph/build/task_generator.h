...
class TaskGenerator {
 ...
 private:
  ...
  Status UpdateNodeAnchor(const ComputeGraphPtr &graph) const;
  ...
  Status GenerateTaskForFftsPlus(const NodePtr &node, RunContext &run_context,
                                 std::vector<domi::TaskDef> &task_def_list) const;

  Status GenTaskForNode(const NodePtr &node, RunContext &context, std::vector<domi::TaskDef> &tasks) const;
  ...
  Status MarkFirstAndLastOps(const std::vector<OpDescPtr> &ops, bool is_single_stream) const;
  ...
  Status AutoFindBpOpIndex(const ComputeGraphPtr &graph, ProfilingPoint &profiling_point,
                           std::vector<uint32_t> &all_reduce_nodes) const;
  Status FindLastBpFromBpNode(const ComputeGraphPtr &graph, const NodePtr &target_node,
  ...
  Status GetFpBpIndex(const ComputeGraphPtr &graph, ProfilingPoint &profiling_point, std::vector<uint32_t> &all_reduce_nodes,
  ...
  Status InsertProfilingTaskBefore(const OpDescPtr &op_desc, const ProfilingPoint &profiling_point,
                                   ...
                                   std::vector<domi::TaskDef> &task_def_list) const;
  Status InsertProfilingArTaskBefore(const OpDescPtr &op_desc, const std::vector<uint32_t> &all_reduce_nodes,
                                     uint32_t node_index, std::vector<domi::TaskDef> &task_def_list,
                                     bool is_insert_bp_profiling_task) const;
  Status InsertProfilingTaskAfter(const OpDescPtr &op_desc, const ProfilingPoint &profiling_point,
                                  const std::vector<uint32_t> &all_reduce_nodes, uint32_t node_index,
                                  std::vector<domi::TaskDef> &task_def_list) const;
  Status InsertProfilingArTaskAfter(const OpDescPtr &op_desc, const std::vector<uint32_t> &all_reduce_nodes,
                                    uint32_t node_index, std::vector<domi::TaskDef> &task_def_list,
                                    bool is_insert_bp_profiling_task) const;
  ...
  ///         Other: failed
  ///
  Status GenerateTaskForFusionNode(const FusionTaskInfo &fusion_task_info,
                                   std::map<int64_t, std::vector<NodePtr>> &fusion_nodes,
                                   std::unordered_set<Node *> &fusion_nodes_seen);

  Status SaveFusionNodes(std::map<int64_t, std::vector<NodePtr>> &fusion_nodes, const ComputeGraphPtr &graph) const;

  Status SetUnknownShapeStream(RunContext &run_context, rtStream_t &stream) const;

  Status DestroyUnknownShapeStream(RunContext &run_context, rtStream_t &stream) const;

  Status SetKnownShapeStream(RunContext &run_context, int64_t stream_id) const;

  bool IsSubGraphOfDynamicGraph(const ComputeGraphPtr &graph) const;

  bool IsGlobalStep(const NodePtr &node) const;
