...
Status AssignByDependencyPass::Run(ComputeGraphPtr graph, const std::vector<SubgraphPtr> &subgraphs, Context &context) {
  (void)graph;
  bool changed = false;
  std::map<NodePtr, SubgraphPtr> end_subgraph_map;
  std::map<NodePtr, SubgraphPtr> pld_subgraph_map;
...
void AssignByDependencyPass::InitEndSubgraphMap(const std::vector<SubgraphPtr> &subgraphs,
                                                std::map<NodePtr, SubgraphPtr> &end_subgraph_map) const {
...
void AssignByDependencyPass::InitPldSubgraphMap(const std::vector<SubgraphPtr> &subgraphs,
                                                std::map<NodePtr, SubgraphPtr> &pld_subgraph_map) const {
  for (const auto &subgraph : subgraphs) {
    const SubGraphInfo &subgraph_info = subgraph->subgraph_info;
...
bool AssignByDependencyPass::CouldReuse(const SubgraphPtr &subgraph, const SubgraphPtr &pred_subgraph,
                                        const std::map<NodePtr, SubgraphPtr> &pld_subgraph_map) const {
...
LogicalStreamPass::SubgraphPtr AssignByDependencyPass::GetReusableSubgraph(
    const SubgraphPtr &subgraph, const std::map<NodePtr, SubgraphPtr> &end_subgraph_map,
    const std::map<NodePtr, SubgraphPtr> &pld_subgraph_map) const {
  const SubGraphInfo &subgraph_info = subgraph->subgraph_info;
...
LogicalStreamAllocator::LogicalStreamAllocator(const std::map<std::string, SchedulerConf> &scheduler_confs,
...
Status LogicalStreamAllocator::DoAssign(const ComputeGraphPtr &graph, const Graph2SubGraphInfoList &subgraph_map,
  ...
  const std::vector<SubGraphInfoPtr> &subgraph_info_list = iter->second;
...
Status LogicalStreamAllocator::ConvertSubgraphs(const std::vector<SubGraphInfoPtr> &subgraph_infos,
                                                const std::map<std::string, EngineConfPtr> &engine_confs,
                                                std::vector<SubgraphPtr> &subgraphs) {
