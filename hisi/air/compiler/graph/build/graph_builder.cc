...
Status GraphBuilder::Build(ComputeGraphPtr &comp_graph, GeRootModelPtr &ge_root_model_ptr, uint64_t session_id) {
  ...
  if (IsCompoundGraph(comp_graph)) {
    ge_root_model_ptr->SetModelName(comp_graph->GetName());
    return BuildForCompoundGraph(session_id, comp_graph, ge_root_model_ptr);
  }
...
bool GraphBuilder::IsCompoundGraph(const ComputeGraphPtr &graph) {
  bool is_pipeline_partitioned = false;
  (void) AttrUtils::GetBool(graph, ATTR_NAME_PIPELINE_PARTITIONED, is_pipeline_partitioned);
  return is_pipeline_partitioned;
}

Status GraphBuilder::BuildForCompoundGraph(uint64_t session_id,
  ...
  for (const auto &node : root_graph->GetDirectNode()) {
    ...
    auto subgraph = NodeUtils::GetSubgraph(*node, kSubgraphIndexOfPartitionedCall);
    GE_CHECK_NOTNULL(subgraph);
    GE_CHK_STATUS_RET_NOLOG(DivideSubgraphFromRootGraph(root_graph, subgraph));
    GELOGD("Start to build subgraph: %s", subgraph->GetName().c_str());
    GeRootModelPtr submodel;
    GE_CHK_STATUS_RET(Build(subgraph, submodel, session_id),
...
Status GraphBuilder::DivideSubgraphFromRootGraph(const ComputeGraphPtr &root_graph, const ComputeGraphPtr &sub_root_graph) {
  for (const auto &node : sub_root_graph->GetDirectNode()) {
    GE_CHECK_NOTNULL(node);
    if (node->GetType() == DATA) {
      auto op_desc = node->GetOpDesc();
      int64_t input_index = -1;
      (void) AttrUtils::GetInt(op_desc, ATTR_NAME_PARENT_NODE_INDEX, input_index);
      (void) op_desc->DelAttr(ATTR_NAME_PARENT_NODE_INDEX);
      ...
      for (const auto &input_desc : node->GetOpDesc()->GetAllInputsDescPtr()) {
        int64_t input_index = -1;
        (void) AttrUtils::GetInt(input_desc, ATTR_NAME_PARENT_NODE_INDEX, input_index);
        (void) input_desc->DelAttr(ATTR_NAME_PARENT_NODE_INDEX);
        (void) AttrUtils::SetInt(input_desc, ATTR_NAME_INDEX, input_index);
      }
    } else {
      std::vector<ComputeGraphPtr> subgraphs;
      GE_CHK_GRAPH_STATUS_RET(NodeUtils::GetDirectSubgraphs(node, subgraphs),
                              "Failed to get subgraphs from node: %s",
                              node->GetName().c_str());
      for (const auto &subgraph : subgraphs) {
        GE_CHK_GRAPH_STATUS_RET(root_graph->RemoveSubGraph(subgraph),
                                "Failed to remove subgraph from root graph, subgraph name = %s",
                                subgraph->GetName().c_str());
        GE_CHK_GRAPH_STATUS_RET(sub_root_graph->AddSubgraph(subgraph->GetName(), subgraph),
                                "Failed to add subgraph to sub root graph, subgraph name = %s",
                                subgraph->GetName().c_str());
        GELOGI("Node [%s] contains subgraph [%s], and moved from root graph to sub root graph [%s] successflly",
               node->GetName().c_str(),
               subgraph->GetName().c_str(),
               sub_root_graph->GetName().c_str());
      }
    }
  }
  sub_root_graph->SetParentGraph(nullptr);
  sub_root_graph->SetParentNode(nullptr);
  return SUCCESS;
}
}  // namespace ge
