...
std::vector<std::string> LabelMaker::GetActiveLabelList(const ComputeGraphPtr &graph) {
  ...
  for (const auto &node : graph->GetDirectNode()) {
    ...
    if (op_desc != nullptr) {
      ...
      if (AttrUtils::GetListStr(op_desc, ATTR_NAME_ACTIVE_LABEL_LIST, tmp_activated_label_list)) {
        activated_label_list.insert(tmp_activated_label_list.begin(), tmp_activated_label_list.end());
      }
    }
  }
  ...
  if (activated_label_list.empty()) {
    need_active_label_list.insert(need_active_label_list.begin(), stream_label_list.begin(), stream_label_list.end());
  } else {
