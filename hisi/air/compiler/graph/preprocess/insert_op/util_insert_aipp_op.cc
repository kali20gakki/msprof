...
Status InsertNewOpUtil::CheckInputNamePositionNotRepeat() const {
  for (int32_t i = 0; i < insert_op_conf_->aipp_op_size(); i++) {
...
Status InsertNewOpUtil::CheckInputRankPositionNoRepeat() const {
  for (int32_t i = 0; i < insert_op_conf_->aipp_op_size(); i++) {
    const domi::AippOpParams *item = insert_op_conf_->mutable_aipp_op(i);
    GE_CHECK_NOTNULL(item);
...
Status InsertNewOpUtil::GetAippParams(const std::unique_ptr<domi::AippOpParams> &aippParams,
...
Status InsertNewOpUtil::UpdatePrevNodeByAipp(const NodePtr &node) const {
...
void InsertNewOpUtil::UpdateMultiBatchInputDims(const OpDescPtr &data_opdesc, Format &old_format) const {
...
Status InsertNewOpUtil::GetDataRelatedNode(const NodePtr &node,
  ...
  for (auto out_data_anchor : node->GetAllOutDataAnchors()) {
    ...
    for (auto peer_in_data_anchor : peer_in_anchors) {
      ...
      if ((dst_op->GetType() == AIPP) || (dst_op->GetType() == CASE)) {
        const auto data_iter = data_next_node_map.find(node);
...
Status InsertNewOpUtil::GetAllAipps(const NodePtr &data_node, const NodePtr &node, std::vector<NodePtr> &aipps) const {
...
Status InsertNewOpUtil::SetModelInputDims(NodePtr &data_node, NodePtr &aipp_node) const {
  GE_CHECK_NOTNULL(data_node);
  GE_CHECK_NOTNULL(aipp_node);
