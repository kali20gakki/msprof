...
Status AippOp::GetAndCheckTarget(const ComputeGraphPtr &graph, int32_t rank, NodePtr &target,
  ...
  if (!edge_indexes.empty() && (*edge_indexes.rbegin() >= data_node->GetOutDataNodes().size())) {
    std::string error_msg = "The aipp parameter input_edge_idx[" + std::to_string(*edge_indexes.crbegin()) +
...
Status AippOp::ConvertRelatedInputNameToRank() {
  ...
  if (!convert_flag) {
    std::string error_msg = "Top name " + related_input_name + "convert rank failed, Please"
...
void AippOp::ConvertParamToAttr(NamedAttrs &aipp_attrs) {
  GE_CHECK_NOTNULL_JUST_RETURN(aipp_params_);
  SAVE_AIPP_ATTR(aipp_mode, int64_t);
  SAVE_AIPP_ATTR(related_input_rank, int64_t);

  if (aipp_params_->aipp_mode() == domi::AippOpParams::static_) {
    SAVE_AIPP_ATTR(input_format, int64_t);
    SAVE_AIPP_ATTR(csc_switch, bool);
    SAVE_AIPP_ATTR(crop, bool);
    SAVE_AIPP_ATTR(resize, bool);
    SAVE_AIPP_ATTR(load_start_pos_w, int64_t);
    SAVE_AIPP_ATTR(load_start_pos_h, int64_t);
    SAVE_AIPP_ATTR(crop_size_w, int64_t);
    SAVE_AIPP_ATTR(crop_size_h, int64_t);
    SAVE_AIPP_ATTR(resize, bool);
    SAVE_AIPP_ATTR(resize_output_w, int64_t);
    SAVE_AIPP_ATTR(resize_output_h, int64_t);
    SAVE_AIPP_ATTR(padding, bool);
    SAVE_AIPP_ATTR(left_padding_size, int64_t);
    SAVE_AIPP_ATTR(right_padding_size, int64_t);
    SAVE_AIPP_ATTR(top_padding_size, int64_t);
    SAVE_AIPP_ATTR(bottom_padding_size, int64_t);
    SAVE_AIPP_ATTR(src_image_size_w, int64_t);
    SAVE_AIPP_ATTR(src_image_size_h, int64_t);
    SAVE_AIPP_ATTR(cpadding_value, float);
    SAVE_AIPP_ATTR(rbuv_swap_switch, bool);
    SAVE_AIPP_ATTR(ax_swap_switch, bool);
    SAVE_AIPP_ATTR(single_line_mode, bool);
    SAVE_AIPP_ATTR(mean_chn_0, int64_t);
    SAVE_AIPP_ATTR(mean_chn_1, int64_t);
    SAVE_AIPP_ATTR(mean_chn_2, int64_t);
    SAVE_AIPP_ATTR(mean_chn_3, int64_t);
    SAVE_AIPP_ATTR(min_chn_0, float);
    SAVE_AIPP_ATTR(min_chn_1, float);
    SAVE_AIPP_ATTR(min_chn_2, float);
    SAVE_AIPP_ATTR(min_chn_3, float);
    SAVE_AIPP_ATTR_LIST(var_reci_chn_0, float);
    SAVE_AIPP_ATTR_LIST(var_reci_chn_1, float);
    SAVE_AIPP_ATTR_LIST(var_reci_chn_2, float);
    SAVE_AIPP_ATTR_LIST(var_reci_chn_3, float);
    SAVE_AIPP_ATTR_LIST(matrix_r0c0, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r0c1, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r0c2, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r1c0, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r1c1, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r1c2, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r2c0, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r2c1, int64_t);
    SAVE_AIPP_ATTR_LIST(matrix_r2c2, int64_t);
    SAVE_AIPP_ATTR_LIST(output_bias_0, int64_t);
    SAVE_AIPP_ATTR_LIST(output_bias_1, int64_t);
    SAVE_AIPP_ATTR_LIST(output_bias_2, int64_t);
    SAVE_AIPP_ATTR_LIST(input_bias_0, int64_t);
    SAVE_AIPP_ATTR_LIST(input_bias_1, int64_t);
    SAVE_AIPP_ATTR_LIST(input_bias_2, int64_t);
  } else {
    SAVE_AIPP_ATTR(max_src_image_size, int64_t);
    SAVE_AIPP_ATTR(support_rotation, bool);
  }
}
Status AippOp::CreateAippData(const NodePtr &aipp_node) {
  ...
  if (batch_count <= 0) {
    std::string error_msg = "Batch count[" + std::to_string(batch_count) + "] is invalid, it must positive.";
    GE_ERRORLOG_AND_ERRORMSG(PARAM_INVALID, error_msg.c_str());
    return PARAM_INVALID;
  }
  ...
  if (max_dynamic_aipp_size < 0) {
    std::string error_msg = "The dynamic aipp size is not positive";
    GE_ERRORLOG_AND_ERRORMSG(PARAM_INVALID, error_msg.c_str());
    return PARAM_INVALID;
  }
