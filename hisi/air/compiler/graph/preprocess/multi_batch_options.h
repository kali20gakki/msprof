...
Status UpdateNameOfInputShape(const ComputeGraphPtr &graph, const std::vector<NodePtr> &data_nodes,
                              const std::vector<NodePtr> &getnext_nosink_nodes, const std::vector<NodePtr> &getnext_sink_nodes);
...
/// @param [out] std::map<std::string, std::vector<vector<int64_t>>> &data_to_dynamic_info: key:data_name. value:dynamic dims.
/// @return SUCCESS / PARAM_INVALID
///
