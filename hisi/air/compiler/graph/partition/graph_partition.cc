...
namespace ge {
Status ge::GraphPartitioner::CheckIfEnd2PldEmpty(ge::ComputeGraphPtr &output_merged_compute_graph) const {
...
void ge::GraphPartitioner::SetMergedGraphId(ge::ComputeGraphPtr &output_merged_compute_graph) const {
...
Status ge::GraphPartitioner::MergeAfterSubGraphOptimization(ge::ComputeGraphPtr &output_merged_compute_graph,
  ...
  for (const auto &sub_graph : original_compute_graph->GetAllSubgraphs()) {
    ...
    GE_IF_BOOL_EXEC(graph_info.corresponding_node_in_partitions_.count(parent_node->GetName()) == 0U,
                    ...
                    return FAILED;)
    auto corresponding_node = graph_info.corresponding_node_in_partitions_[parent_node->GetName()];
...
Status ge::GraphPartitioner::MergeSubGraph(ge::ComputeGraphPtr &output_merged_compute_graph,
  ...
  for (const auto &it : sub_graph_list) {
    ...
    // Merge overflow detection attr
    int64_t global_workspace_value = kOverflowDefaultValue;
    (void)AttrUtils::GetInt(it->GetSubGraph(), "globleworkspace_status", global_workspace_value);
    int64_t global_workspace_value_bytes = kOverflowDefaultValue;
    (void)AttrUtils::GetInt(it->GetSubGraph(), "globleworkspace_status_bytes", global_workspace_value_bytes);

    if ((global_workspace_value == kOverflowDefaultValue) || (global_workspace_value_bytes == kOverflowDefaultValue)) {
      continue;
    }

    if (!AttrUtils::HasAttr(output_merged_compute_graph, "globleworkspace_status")) {
      (void)AttrUtils::SetInt(output_merged_compute_graph, "globleworkspace_status", global_workspace_value);
      (void)AttrUtils::SetInt(output_merged_compute_graph,
                              "globleworkspace_status_bytes",
                              global_workspace_value_bytes);
    }
  }
  ...
  (void)InheritOriginalAttr(original_compute_graph, output_merged_compute_graph);
  GE_TIMESTAMP_END(MergeSubGraphEnginePlacerRun, "GraphPartitioner::MergeGraphEnginePlacerRun");
  return UpdateCorrespondNodeInPartitions(original_compute_graph, output_merged_compute_graph);
}

void GraphPartitioner::InheritOriginalAttr(const ComputeGraphPtr &original_compute_graph,
...
Status ge::GraphPartitioner::UpdatePldOpDesc(const NodePtr &dst_node, int32_t input_index,
...
Status ge::GraphPartitioner::UpdateEndOpDesc(const NodePtr &src_node, int32_t output_index,
...
graphStatus ge::GraphPartitioner::AddPlaceHolderEndInSrcDstGraph(const AnchorPtr &out_anchor,
  ...
  GE_CHECK_NOTNULL(end_dst_anchor);
...
void ge::GraphPartitioner::AddNewGraphToPartition(const ge::ComputeGraphPtr &input_graph,
...
bool ge::GraphPartitioner::IsDataLike(ge::NodePtr node) const {
  return (node->GetType() == CONSTANT) || (node->GetType() == DATA) || (node->GetType() == AIPPDATA) ||
         (node->GetType() == CONSTANTOP) || (node->GetType() == VARIABLE);
}

bool ge::GraphPartitioner::HasNoInput(ge::NodePtr node) const {
  if (node == nullptr) {
  ...
  for (const auto &node : compute_graph->GetDirectNode()) {
    ...
    if (HasNoInput(node) && IsDataLike(node)) {
      ...
      if (node_engine_map.count(node) == 0) {
        ...
        if (graph_info_.engine_placer_.SelectEngine(node, is_check_support_success) != SUCCESS) {
...
Status ge::GraphPartitioner::AddPartitionsToGraphNode(std::vector<ge::SubGraphInfoPtr> &output_subgraphs,
  ...
  for (const auto &sub_graph : graph_info_.rank_2_partitions_) {
    ...
    if (compute_graph->GetParentNode() == nullptr) {
      GE_IF_BOOL_EXEC(!AttrUtils::SetBool(sub_graph, ATTR_NAME_IS_ROOT_GRAPH, true),
                      GELOGW("Set attr ATTR_NAME_IS_ROOT_GRAPH failed");)
    } else {
    ...
    if (SetMemberForSubGraphInfo(sgi, sub_graph, engine_name) != SUCCESS) {
      REPORT_INNER_ERROR("E19999", "set members for subgraph:%s info failed", sub_graph->GetName().c_str());
...
Status ge::GraphPartitioner::SetMemberForSubGraphInfo(ge::SubGraphInfoPtr &sgi, const ComputeGraphPtr &sub_graph,
                                                      const std::string &engine_name) {
  ...
  GE_RETURN_WITH_LOG_IF_TRUE(sub_graph->GetDirectNode().empty(),
                             "[Check][Param]graph:%s has no node", sub_graph->GetName().c_str());
...
Status ge::GraphPartitioner::SplitSubGraphs(ge::ComputeGraphPtr compute_graph) {
  ...
  for (auto &node : compute_graph->GetDirectNode()) {
    ...
    if (!graph_info_.corresponding_node_in_partitions_.insert({node->GetName(), corresponding_node}).second) {
      REPORT_CALL_ERROR("E19999", "Node name %s already existed in graph %s", node->GetName().c_str(),
                        compute_graph->GetName().c_str());
    ...
    for (const auto &in_anchor : node->GetAllInAnchors()) {
      ...
      for (auto &peer_out_anchor : in_anchor->GetPeerAnchors()) {
        ...
        auto iter = graph_info_.corresponding_node_in_partitions_.find(peer_out_anchor->GetOwnerNode()->GetName());
...
Status GraphPartitioner::UpdateCorrespondNodeInPartitions(const ComputeGraphPtr &original_graph,
                                                          const ComputeGraphPtr &compute_graph) {
  GE_CHECK_NOTNULL(compute_graph);
  auto iter_graph = graph_2_graph_partition_info_.find(original_graph);
  if (iter_graph == graph_2_graph_partition_info_.end()) {
    GELOGE(FAILED, "Can not find graph: %s", original_graph->GetName().c_str());
    return FAILED;
  }

  GELOGI("Graph partition info: %s", iter_graph->second.output_name_.c_str());
  for (const auto &node_after_optimize : compute_graph->GetDirectNode()) {
    auto iter = iter_graph->second.corresponding_node_in_partitions_.find(node_after_optimize->GetName());
    if (iter != iter_graph->second.corresponding_node_in_partitions_.end()) {
      GELOGI("Update correspond node in partitions[%s]", node_after_optimize->GetName().c_str());
