...
class GraphPartitioner {
 ...
 private:
  ...
  void InheritOriginalAttr(const ComputeGraphPtr &original_compute_graph,
  ...
  Status CheckIfEnd2PldEmpty(ComputeGraphPtr &output_merged_compute_graph) const;
  ...
  bool HasNoInput(NodePtr node) const;
  ...
  Status UpdateEndOpDesc(const NodePtr &src_node, int32_t output_index, const OpDescPtr &end_op_desc) const;
  Status UpdatePldOpDesc(const NodePtr &dst_node, int32_t input_index, const OpDescPtr &pld_op_desc) const;

  // Clear partition data
  void ClearAllPartitionData();
  void SetMergedGraphId(ComputeGraphPtr &output_merged_compute_graph) const;

  const NodeEngineMap &GetNodeEngineMap() const;
  Status UpdateCorrespondNodeInPartitions(const ComputeGraphPtr &original_graph, const ComputeGraphPtr &compute_graph);
