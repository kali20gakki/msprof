...
class GraphManager {
 public:
  ...
  Status InitDynamicParams(const ComputeGraphPtr &compute_graph) const;
  ...
  /// @return Status result of function
  ///
  Status BuildGraph(const GraphId &graph_id, const std::vector<GeTensor> &inputs, FlowModelPtr &models,
  ...
  /// @return Status result of function
  ///
  Status RunGraphAsync(const GraphId &graph_id, const std::vector<Tensor> &inputs,
  ...
  Status RegisterCallBackFunc(const std::string &key,
                              const std::function<Status(uint32_t, const std::map<std::string, Tensor> &)> &callback);

  Status RegisterCallBackFunc(const std::string &key,
                              const std::function<Status(uint32_t, const std::map<AscendString, Tensor> &)> &callback);
  ...
  Status BuildGraph(ComputeGraphPtr &compute_graph, PneModelPtr &model);
 ...
 private:
  ...
  Status RunCustomPass(const GraphNodePtr &graph_node) const;
  Status PreRun(const GraphNodePtr &graph_node, const std::vector<GeTensor> &inputs, FlowModelPtr &ge_root_model,
                uint64_t session_id = INVALID_SESSION_ID);
  ...
  Status StartForRunGraph(const GraphNodePtr &graph_node, const std::vector<GeTensor> &inputs,
                          FlowModelPtr &ge_root_model, uint64_t session_id = INVALID_SESSION_ID);

  Status InnerRunGraph(const GraphNodePtr &graph_node, const GraphId &graph_id, const std::vector<GeTensor> &inputs,
  ...
  Status PushSummaryData2ME(const GraphId &graph_id, const std::map<std::string, Tensor> &summary_data);

  // call the callback function of ME to push save result data to ME
  Status PushSaveData2ME(const GraphId &graph_id, const std::map<std::string, Tensor> &save_data);
  ...
  Status ConvertGraphToFile(ComputeGraphPtr &compute_graph, GraphPartitioner &partitioner, std::string path,
  ...
  bool CheckModelLoad(const FlowModelPtr &flow_model, bool load_flag) const;

  Status LoadGraph(const FlowModelPtr &flow_root_model, const GraphNodePtr &graph_node);

  bool IsGraphNeedBuild(const GraphNodePtr &graph_node) const;

  Status SetRunContext(const GraphNodePtr &graph_node) const;
  void PushRunArgs(const RunArgs &args) const;

  void PreRunThread();
  void StopQueue();
  void ReturnError(RunAsyncCallback callback, Status ret, const std::string &log);
  ...
  Status SetRtContext(rtContext_t rt_context, rtCtxMode_t mode, uint64_t session_id, uint32_t graph_id) const;
  ...
  Status CheckIncreBuildAndPreRun(const PreRunArgs &args, GraphNodePtr &graph_node, FlowModelPtr &flow_model);
  ...
  Status UnloadModel(FlowModelPtr flow_model, uint32_t graph_id);
  ...
  Status ProcessNodeEngineBuild(const GraphNodePtr &graph_node,
                                ComputeGraphPtr &compute_graph,
  ...
  std::map<std::string, std::function<Status(uint32_t, const std::map<std::string, Tensor> &)>> me_callback_map_;

  std::map<std::string, std::function<Status(uint32_t, const std::map<AscendString, Tensor> &)>> callback_map_;
  ...
  GraphRebuildStateCtrl graph_rebuild_state_ctrl_;
