...
class AtomicAddrCleanPass : public GraphPass {
 ...
 private:
   ...
   * @return
   */
  Status HandleNormalGraph(ComputeGraphPtr &graph, const std::vector<NodePtr> &atomic_node_vec);
  /**
   * Insert atomic clean node to graph
   * @param graph
   * @return
   */
  NodePtr InsertAtomicAddrCleanNode(ComputeGraphPtr &graph) const;
   ...
   * @return
   */
  Status CompileUnknownGraphOp(const std::vector<NodePtr> &atomic_node_vec) const;
