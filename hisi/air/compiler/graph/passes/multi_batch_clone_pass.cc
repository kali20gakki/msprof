...
Status MultiBatchClonePass::GetInputAndOutputNum(uint32_t &input_num, uint32_t &output_num) {
  if (all_data_nodes_.size() + all_const_nodes_.size() > UINT32_MAX) {
    REPORT_CALL_ERROR("E19999", "Sum of data node size[%zu] and const node size[%zu] exceed UINT32_MAX.",
  ...
  if (data_count_from_getnext_ != 0) {
    if (input_num + data_count_from_getnext_ - kNumOfGetnextNode > UINT32_MAX) {
      REPORT_CALL_ERROR("E19999", "Sum[%zu] exceed UINT32_MAX.",
...
Status MultiBatchClonePass::CreateRootGraph(const ComputeGraphPtr &graph) {
  ...
  if (GetInputAndOutputNum(input_num, output_num) != SUCCESS) {
    REPORT_CALL_ERROR("E19999", "Get input and output num of graph: %s failed.", graph->GetName().c_str());
...
Status MultiBatchClonePass::UpdateSubgraphData(const NodePtr &data, size_t batch_index) {
  ...
  int32_t parent_index = node_index + 1;
  ...
  if (pos == std::string::npos) {
    REPORT_INNER_ERROR("E19999", "Cannot find key std::string [%s] of multi-batch in name of virtual input node:%s(%s)",
...
Status MultiBatchClonePass::CreateSubgraphs(const ComputeGraphPtr &graph, const ComputeGraphPtr &branch) {
  ...
  for (size_t i = 0; i < batch_shapes_.size(); ++i) {
    ...
    for (const auto &node : subgraph->GetDirectNode()) {
      if (node->GetType() == VARIABLE) {
        ...
        const auto pos = var_name.find(postfix);
        const std::string var_key_name = (pos == std::string::npos) ? var_name : (var_name.substr(0, pos));
        VarManager::Instance(session_id_)->SetBatchVariablesKeyName(var_name, var_key_name);
      }
    }
  }
  ...
  for (const auto &n : branch->GetDirectNode()) {
    ...
    const std::string new_name = n->GetName() + kMultiBatchNodePostfix + "0";
    if (n->GetType() == VARIABLE) {
      VarManager::Instance(session_id_)->SetBatchVariablesKeyName(new_name, n->GetName());
    }
