...
Status BufferPoolMemoryPass::TryToFixNodeOrder(NodePtr &pre_node, NodePtr &curr_node, bool &not_change) {
  ...
  if ((pre_node_graph == curr_node_graph) &&
    ...
    if (ret != GRAPH_SUCCESS) {
      ...
      REPORT_CALL_ERROR("E19999", "Failed to add ctrl edge from %s to %s.", pre_node->GetName().c_str(),
...
Status BufferPoolMemoryPass::InsertMemCpyNodeAfter(ComputeGraphPtr &graph, NodePtr &node) const {
