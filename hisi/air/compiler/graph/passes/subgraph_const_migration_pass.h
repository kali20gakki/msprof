...
class SubgraphConstMigrationPass : public GraphPass {
 ...
 private:
  ...
  Status ClassifyGraphNodes(const ComputeGraphPtr &graph, const OpDescPtr &func_desc,
                            std::map<ComputeGraphPtr, std::map<std::string, NodePtr>> &all_const_nodes,
                            std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &all_data_nodes) const;
  ...
  ///
  bool GetAssociatedNodes(const std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &all_data_nodes,
  ...
  ///
  bool IsParallelNodeSame(const std::map<ComputeGraphPtr, std::map<std::string, NodePtr>> &all_const_nodes,
  ...
  Status GraphNodeMigration(const ComputeGraphPtr &graph, const NodePtr &func_node,
                            const std::map<ComputeGraphPtr, std::map<std::string, NodePtr>> &all_const_nodes,
                            std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &all_data_nodes,
                            const NodePtr &const_node, const std::string &node_key) const;
  ...
  Status MoveNodeToParent(const ComputeGraphPtr &graph, const NodePtr &func_node,
                          const std::map<ComputeGraphPtr, std::map<std::string, NodePtr>> &all_const_nodes,
                          const std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &all_data_nodes,
