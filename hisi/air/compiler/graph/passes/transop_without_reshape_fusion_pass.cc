...
bool TransOpWithoutReshapeFusionPass::FormatContinuousCheck(const OutDataAnchorPtr &out_anchor,
  ...
  if ((in_op->GetType() == CAST)) {
    if (!TransOpUtil::CheckPrecisionLoss(in_node)) return false;
  }

  if ((out_op->GetType() == CAST)) {
    if (!TransOpUtil::CheckPrecisionLoss(out_owner_node)) return false;
  }
...
OpDescPtr TransOpWithoutReshapeFusionPass::GetFormatTransferOp(const GeTensorDesc &format_trans_input_desc,
  ...
  std::string src_format = TypeUtils::FormatToSerialString(format_trans_input_desc.GetFormat());
  std::string dst_format = TypeUtils::FormatToSerialString(format_trans_output_desc.GetFormat());
...
bool TransOpWithoutReshapeFusionPass::OpAccuracyAbilityCheck(const OpDescPtr &op_desc) const {
