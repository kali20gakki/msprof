...
graphStatus InferBasePass::UpdateTensorDescToPeerInputs(const NodePtr &node, std::set<NodePtr> &changed_nodes) {
...
void InferBasePass::PrintInOutTensors(const NodePtr &node, const std::string &phase) const {
  ...
  if (op_desc == nullptr) {
    REPORT_INNER_ERROR("E19999", "Node has no opdesc, check invalid");
    GELOGE(GRAPH_FAILED, "[Get][OpDesc] op_desc is null.");
    return;
  }
  std::stringstream ss;
  ss << "{";
