...
Status AtomicAddrCleanPass::HandleNormalGraph(ComputeGraphPtr &graph, const std::vector<NodePtr> &atomic_node_vec) {
...
NodePtr AtomicAddrCleanPass::InsertAtomicAddrCleanNode(ComputeGraphPtr &graph) const {
  ...
  std::string name = node_name + session_graph_id;
...
Status AtomicAddrCleanPass::CompileUnknownGraphOp(const std::vector<NodePtr> &atomic_node_vec) const {
