...
bool TransOpSymmetryEliminationPass::DescAreSymmetry(const NodePtr &src_node, const NodePtr &dst_node) {
  ...
  if (src_node->GetType() == CAST && dst_node->GetType() == CAST) {
    ...
    if (src_input_origin_format == dst_output_origin_format) {
      // if origin_format is symmetry, requiered origin_shape is symmetry
  ...
  if (!is_symmetry) {
    GELOGD("Not satisfied symmetry. ignore pass.\n"
           "Src node %s input type: %s format: %s shape: %s, origin_shape: %s."
           "dst node %s output type: %s format: %s shape: %s, origin_shape: %s.",
           ...
           formats::ShapeToString(src_input_origin_shape).c_str(),
