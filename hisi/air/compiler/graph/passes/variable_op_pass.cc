...
Status VariableOpPass::Run(ge::ComputeGraphPtr graph) {
  ...
  for (auto &var_to_refs : var_and_var_ref_map_) {
    ...
    if (ret != SUCCESS) {
      REPORT_CALL_ERROR("E19999", "Set Trans road for node:%s(Variable) failed, session_id:%lu",
    ...
    if (ret != SUCCESS) {
      REPORT_CALL_ERROR("E19999", "Update graph_id:%u for node:%s(Variable) failed, session_id:%lu",
