...
class SubgraphPass : public GraphPass {
 ...
 private:
   ...
   * @return: 0 for SUCCESS / others for FAILED
   */
  Status WhileInputNodes(const ComputeGraphPtr &graph, const NodePtr &node) const;
   ...
   */
  bool IsOutputContinuesRequired(const NodePtr &node) const;
   ...
   */
  bool IsInputContinuesRequired(const NodePtr &node) const;
