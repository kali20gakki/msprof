...
Status UnlinkControlEdgeBetweenNodes(const NodePtr &a, const NodePtr &b) {
  const auto &in_ctrl_anchor_a = a->GetInControlAnchor();
  const auto &out_ctrl_anchor_a = a->GetOutControlAnchor();
  const auto &in_ctrl_anchor_b = b->GetInControlAnchor();
  const auto &out_ctrl_anchor_b = b->GetOutControlAnchor();
  if (in_ctrl_anchor_a->IsLinkedWith(out_ctrl_anchor_b)) {
    (void)in_ctrl_anchor_a->Unlink(out_ctrl_anchor_b);
    GELOGD("Unlink ctrl edge from %s to %s.", b->GetName().c_str(), a->GetName().c_str());
  }
  if (in_ctrl_anchor_b->IsLinkedWith(out_ctrl_anchor_a)) {
    (void)in_ctrl_anchor_b->Unlink(out_ctrl_anchor_a);
    GELOGD("Unlink ctrl edge from %s to %s.", a->GetName().c_str(), b->GetName().c_str());
  }
  return SUCCESS;
}
}
Status TransOpBreadthFusionPass::Run(ge::ComputeGraphPtr graph) {
  ...
  // init cycle detector
  cycle_detector_ = GraphUtils::CreateCycleDetector(graph);
  GE_CHECK_NOTNULL(cycle_detector_);
...
graphStatus TransOpBreadthFusionPass::Fusion(const std::vector<NodePtr> &trans_nodes, ComputeGraphPtr &graph) const {
  ...
  for (size_t i = 0; i < trans_nodes.size(); ++i) {
    ...
    // check could cause cycle, currently not support self cycle detect
    if (cycle_detector_->HasDetectedCycle({{trans_nodes[i], node_remain}})) {
      GELOGI("May cause cycle when fusion on node %s and %s. Give up fusion.", trans_nodes[i]->GetName().c_str(),
             node_remain->GetName().c_str());
      continue;
    }
    // unlink control edge between transnode.
    // cause it could cause cycle, and this kind of ctrl is useless.
    GE_CHK_STATUS_RET_NOLOG(UnlinkControlEdgeBetweenNodes(trans_nodes[i], node_remain));
    ...
    // udpate cycle detector
    (void)cycle_detector_->Update(graph, {trans_nodes[i], node_remain});
  }
