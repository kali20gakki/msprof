...
const char *const kPassName = "AicpuConstantFoldingPass";
}  // namespace
namespace ge {
bool AicpuConstantFoldingPass::NeedIgnorePass(const NodePtr &node) {
  if (IsSkipFold(node)) {
    return true;
  }
  return false;
...
bool AicpuConstantFoldingPass::NeedFold() const {
  return need_fold_;
}

Status AicpuConstantFoldingPass::ComputePotentialWeight(NodePtr &node, std::vector<GeTensorPtr> &outputs) {
  need_fold_ = true;
  ...
  if (input_nodes_2_out_anchors.empty() || input_nodes_2_out_anchors.size() != node_desc->GetInputsSize()) {
    GELOGD("Const input nodes size is %zu, and nodeDesc inputsSize is %zu, skip fold.",
  ...
  vector<ConstGeTensorPtr> input_weight_vec = OpDescUtils::GetWeightsFromNodes(input_nodes_2_out_anchors);
  if (input_weight_vec.size() != input_nodes_2_out_anchors.size()) {
    GELOGW("Get weights from const_inputs size %zu, not match with inputs size %zu. Ignore pass.",
           input_weight_vec.size(), input_nodes_2_out_anchors.size());
    return NOT_CHANGED;
  }
  ...
  return ComputeWithAICPUKernel(node, input_weight_vec, outputs);
}

Status AicpuConstantFoldingPass::GetInputAddrs(const std::vector<ConstGeTensorPtr> &weight_vec,
                                               std::vector<AddrAndType> &input_addrs) const {
  if (weight_vec.empty()) {
...
Status AicpuConstantFoldingPass::GetOutputAddrs(const OpDescPtr &node_desc, std::vector<uint64_t> &output_addrs) const {
...
Status AicpuConstantFoldingPass::GenerateDataPtrInfo(const std::vector<uint64_t> &output_addrs,
                                                     std::vector<DataPtrInfo> &data_vec,
...
Status AicpuConstantFoldingPass::UpdateWorkSpaceAddr(std::string &task_info, STR_FWK_OP_KERNEL &task) const {
...
Status AicpuConstantFoldingPass::UpdateInputAndOutputAddr(const std::vector<uint64_t> &io_addrs,
...
Status AicpuConstantFoldingPass::UpdateSingleOpAddr(std::string &task_info,
                                                    const std::vector<AddrAndType> &input_addrs,
                                                    const std::vector<uint64_t> &outputs_addr_vec,
                                                    STR_FWK_OP_KERNEL &task) const {
  // Build the SingleOpAddr
  std::vector<uint64_t> inputs_addr_vec;
...
Status AicpuConstantFoldingPass::UpdateMemCopyAddr(std::string &task_info, const std::vector<uint64_t> &data_infos,
                                                   std::vector<uint64_t> &internal_addrs,
                                                   STR_FWK_OP_KERNEL &task) const {
  std::vector<uint64_t> release_flags;
  ...
  std::vector<std::vector<uint64_t>> inputs = {release_flags, data_sizes, src_addrs, dst_addrs};
...
Status AicpuConstantFoldingPass::LaunchSingleOpRunTask(const NodePtr &node, const std::vector<AddrAndType> &input_addrs,
  ...
  std::function<void()> callback = [&aicpu_task]() {
...
Status AicpuConstantFoldingPass::LaunchMemCopyTask(const std::vector<uint64_t> &data_infos) {
  void *task_buf = nullptr;
  auto instance_ptr = ge::GELib::GetInstance();
  if (instance_ptr == nullptr || !instance_ptr->InitFlag()) {
    REPORT_INNER_ERROR("E19999", "GeLib is not init before, check invalid");
  ...
  std::vector<uint64_t> internal_addrs;
  std::function<void()> callback = [&internal_addrs, &aicpu_task]() {
...
Status AicpuConstantFoldingPass::GenerateTaskForLaunch(STR_FWK_OP_KERNEL &aicpu_task, void *&task_buf) const {
  ...
  rtError_t rt_ret = rtMemcpy(task_buf, sizeof(STR_FWK_OP_KERNEL), &aicpu_task,
...
Status AicpuConstantFoldingPass::KernelLaunch(void *task_buf) const {
  ...
  std::function<void()> callback = [&task_buf, &model, &stream, &stream_run]() {
    if (task_buf != nullptr) {
      GE_CHK_RT(rtFree(task_buf));
    }
    if (model != nullptr) {
...
Status AicpuConstantFoldingPass::GenerateGeTensor(const OpDescPtr &node_desc, const std::vector<DataPtrInfo> &data_vec,
...
void AicpuConstantFoldingPass::ReleaseMemory(const std::vector<AddrAndType> &input_addrs,
                                             const std::vector<uint64_t> &output_addrs,
...
bool AicpuConstantFoldingPass::IsSkipFold(const ge::NodePtr &node) const {
...
Status AicpuConstantFoldingPass::ComputeWithAICPUKernel(const NodePtr &node,
                                                        const vector<ConstGeTensorPtr> &input_weight_vec,
  ...
  Status ret = GetInputAddrs(input_weight_vec, input_addrs);
  if (ret != SUCCESS) {
...
Status AicpuConstantFoldingPass::Init(uint64_t session_id, const ComputeGraphPtr &compute_graph) {
  cur_session_id_ = session_id;
  GE_CHECK_NOTNULL(compute_graph);
  cur_compute_graph_ = compute_graph;
  int32_t dev_count = 0;
  const char *aicpu_constant_folding_on = std::getenv("AICPU_CONSTANT_FOLDING_ON");
  if (aicpu_constant_folding_on != nullptr) {
    rt_get_device_count_err_ = rtGetDeviceCount(&dev_count);
    if (rt_get_device_count_err_ == RT_ERROR_NONE) {
      Status result = RtContextUtil::GetInstance().SetRtContext(cur_session_id_, cur_compute_graph_->GetGraphID(),
          ge::GetContext().DeviceId(), RT_CTX_NORMAL_MODE, rtContext_t());
      if (result != SUCCESS) {
...
Status AicpuConstantFoldingPass::Finalize() const {
  const char *aicpu_constant_folding_on = std::getenv("AICPU_CONSTANT_FOLDING_ON");
  if (aicpu_constant_folding_on != nullptr) {
    if (rt_get_device_count_err_ == RT_ERROR_NONE) {
      Status result = RtContextUtil::GetInstance().SetRtContext(cur_session_id_, cur_compute_graph_->GetGraphID(),
          ge::GetContext().DeviceId(), RT_CTX_GEN_MODE, rtContext_t());
      if (result != SUCCESS) {
...
string AicpuConstantFoldingPass::GetPassName() const {
  return kPassName;
}
}  // namespace ge
