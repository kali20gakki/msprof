...
Status InputOutputConnectionIdentifyPass::Run(ComputeGraphPtr graph) {
  ...
  std::map<NodePtr, std::vector<uint32_t>> connect_input_node_idx_map;
  std::map<NodePtr, std::vector<uint32_t>> connect_output_node_idx_map;
...
Status InputOutputConnectionIdentifyPass::ProcessInputNode(
    const NodePtr &node, std::map<NodePtr, std::vector<uint32_t>> &connect_input_node_idx,
...
Status InputOutputConnectionIdentifyPass::UpdateNodeIdxMap(
    const std::string &symbol_string, std::map<NodePtr, std::vector<uint32_t>> &connect_input_node_idx,
    std::map<NodePtr, std::vector<uint32_t>> &connect_output_node_idx) {
...
Status InputOutputConnectionIdentifyPass::ProcessOutputNode(
    const NodePtr &node, std::map<NodePtr, std::vector<uint32_t>> &connect_input_node_idx,
    std::map<NodePtr, std::vector<uint32_t>> &connect_output_node_idx) {
  GE_CHECK_NOTNULL(node);
...
Status InputOutputConnectionIdentifyPass::SetNodeAttrOfConnectingInputOutput(
    const std::map<NodePtr, std::vector<uint32_t>> &connect_input_node_idx,
    const std::map<NodePtr, std::vector<uint32_t>> &connect_output_node_idx) const {
