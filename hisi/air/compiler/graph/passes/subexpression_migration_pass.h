...
class SubexpressionMigrationPass : public GraphPass {
 ...
 private:
  ...
  ///
  bool GetAssociatedNodes(const NodePtr &node, std::map<uint32_t, uint32_t> &inputs, std::map<uint32_t, uint32_t> &outputs) const;
  ...
  ///
  bool IsParallelNodeSame(const std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &graph_nodes,
  ...
  Status MoveNodeToParent(const ComputeGraphPtr &graph, const NodePtr &func_node,
                          const std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &graph_nodes,
                          uint32_t anchor_idx, const std::map<uint32_t, uint32_t> &inputs,
  ...
  /// @return 0: SUCCESS / others: FAILED
  ///
  Status AppendParallelNode(std::map<ComputeGraphPtr, std::map<uint32_t, NodePtr>> &graph_nodes,
