...
class BaseNodePass {
 ...
 public:
  ...
  const std::unordered_set<NodePtr> &GetGlobalNodesNeedRePassImmediately() {
 ...
 private:
  ...
  std::unordered_set<NodePtr> global_nodes_need_repass_immediately_;
...
class GEPass {
 public:
  explicit GEPass(ComputeGraphPtr &graph) : depth_(1), graph_(graph), root_graph_(graph) {
