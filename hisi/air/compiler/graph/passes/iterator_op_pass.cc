  ...
  for (ge::NodePtr &node : graph->GetDirectNode()) {
    ...
    if ((type == ITERATORV2) || (type == ITERATOR) || (op_type == GETNEXT)) {
      ...
      GE_IF_BOOL_EXEC(status != SUCCESS, GELOGW("Fail to Get var_desc of NODE_NAME_FLOWCTRL_LOOP_PER_ITER failed.");
                      continue);
      Status ret = SetRtContext(graph->GetSessionID(), graph->GetGraphID(), rtContext_t(), RT_CTX_NORMAL_MODE);
...
Status IteratorOpPass::GetVariableValue(uint64_t session_id, const ge::GeTensorDesc &tensor_desc,
                                        const std::string &var_name, void *dest) const {
  // base_addr
  uint8_t *var_mem_base = VarManager::Instance(session_id)->GetVarMemoryBase(RT_MEMORY_HBM, GetContext().DeviceId());
  GE_CHECK_NOTNULL(var_mem_base);
  // offset + logic_base
  uint8_t *dev_ptr = nullptr;
  const auto status = VarManager::Instance(session_id)->GetVarAddr(var_name, tensor_desc, dev_ptr);
  if (status != ge::SUCCESS) {
...
ge::NodePtr IteratorOpPass::InsertEndOfSequenceNode(const ge::NodePtr &pre_node, const ge::NodePtr &memcpy_node,
  ...
  ge::graphStatus status = GraphUtils::AddEdge(out_anchor, end_of_seq_node->GetInDataAnchor(0));
...
/// @return ge::OpDescPtr
///
ge::OpDescPtr IteratorOpPass::CreateEndOfSequenceOp(const ge::NodePtr &pre_node) const {
  ...
  std::string node_name = pre_node->GetName() + "_EndOfSequence";
