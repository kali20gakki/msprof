 ...
 * @return: 0 for SUCCESS / others for FAILED
 */
Status SubgraphPass::WhileInputNodes(const ComputeGraphPtr &graph, const NodePtr &node) const {
  for (InDataAnchorPtr &in_data_anchor : node->GetAllInDataAnchors()) {
    const OutDataAnchorPtr &peer_out_anchor = in_data_anchor->GetPeerOutAnchor();
    GE_IF_BOOL_EXEC(peer_out_anchor == nullptr, continue);
    NodePtr in_node = peer_out_anchor->GetOwnerNode();
 ...
 */
bool SubgraphPass::IsOutputContinuesRequired(const NodePtr &node) const {
 ...
 */
bool SubgraphPass::IsInputContinuesRequired(const NodePtr &node) const {
  OpDescPtr op_desc = node->GetOpDesc();
  if (op_desc != nullptr) {
