...
Status GraphOptimize::OptimizeOriginalGraph(const ComputeGraphPtr &compute_graph) const {
  ...
  std::string exclude_core_Type = (core_type_ == kVectorCore) ? kAicoreEngine : kVectorEngine;
  ...
  if (graph_optimizer.size() != 0) {
    for (auto iter = graph_optimizer.begin(); iter != graph_optimizer.end(); ++iter) {
      ...
      if ((GetContext().GetHostExecFlag() && (iter->first != kHostCpuEngine)) || (iter->second == nullptr)) {
...
Status GraphOptimize::OptimizeOriginalGraphJudgeInsert(const ComputeGraphPtr &compute_graph) const {
