...
struct NodeInputOutputRWType {
  std::map<uint32_t, InputRWType> input_rw_type_map;
  std::map<uint32_t, OutputRWType> output_rw_type_map;
};
// input and output rw_type of node in current graph
thread_local std::map<std::string, NodeInputOutputRWType> node_rwtype_map_;
...
InputRWType GetInputRwTypeInConflict(const std::set<int32_t> &rw_type_set) {
  ...
  int32_t total_rw_type = 0;
  for (const auto rw : rw_type_set) {
    if (CheckInt32AddOverflow(total_rw_type, rw) != SUCCESS) {
...
NodePtr AddIdentityToGraph(const Node &src_node, int32_t out_anchor_idx) {
...
OutputRWType GetOutputRWTypeByIndex(const Node &node, uint32_t index) {
  ...
  if (subgraph_names.empty()) {
    ...
    if ((output_node_vec.size() == 1) && (output_node_vec.at(0) != nullptr)) {
      // find rw type from map.
      std::map<std::string, NodeInputOutputRWType>::const_iterator iter =
      ...
      std::map<uint32_t, OutputRWType>::const_iterator index_2_output_rw_type =
...
InputRWType GetInputRWTypeByIndex(const Node &node, uint32_t index) {
  ...
  if (subgraph_names.empty()) {
    ...
    for (const auto &data_node : data_node_vec) {
      ...
      std::map<uint32_t, InputRWType>::const_iterator input_rw_type =
...
bool JudgeOptimizableByParentNode(const NodePtr &parent_node, int64_t output_idx) {
  auto output_anchor = parent_node->GetOutDataAnchor(output_idx);
  if ((parent_node->GetType() == PARTITIONEDCALL) || (output_anchor == nullptr)) {
    return true;
  }

  for (const auto &in_anchor : output_anchor->GetPeerInDataAnchors()) {
    if ((in_anchor != nullptr) && (!in_anchor->GetOwnerNode()->GetOpDesc()->GetSubgraphInstanceNames().empty())) {
      return false;
    }
  ...
  for (const auto &node : sub_graph->GetDirectNode()) {
    ...
    if (node->GetType() == DATA) {
      ...
      for (const auto anchor_2_node_pair : anchor_2_node_vec) {
        ...
        auto input_rw_type = GetInputRWTypeByIndex(*anchor_2_node_pair.second,
                                                   anchor_2_node_pair.first->GetIdx());
        GELOGD("Input rw type of Node %s %dth input anchor is %s",
               anchor_2_node_pair.second->GetName().c_str(),
               anchor_2_node_pair.first->GetIdx(),
      ...
      GELOGD("Input rw type of Node %s is %s",
             node->GetName().c_str(),
             InputRWTypeToSerialString(anchor_rw_type).c_str());
      std::map<uint32_t, InputRWType> input_rw_type_map{std::make_pair(0, anchor_rw_type)};
    ...
    if (node->GetType() == NETOUTPUT) {
      ...
      for (const auto &in_data_anchor : node->GetAllInDataAnchors()) {
        ...
        auto pre_output_rw_type = GetOutputRWTypeByIndex(*pre_node,
                                                         pre_out_anchor->GetIdx());
        GELOGD("Output rw type of Node %s %dth output anchor is %s",
        ...
        if ((pre_output_rw_type == OutputRWType::kWriteable) &&
            (!JudgeOptimizableByParentNode(parent_node, pre_out_anchor->GetIdx()))) {
          // insert identity
...
vector<std::vector<ConflictResult>> output_2_input_rwtype =
    {{ConflictResult::DO_NOTHING, ConflictResult::WRONG_GRAPH, ConflictResult::INSERT_IDENTITY},
     {ConflictResult::DO_NOTHING, ConflictResult::WRONG_GRAPH, ConflictResult::DO_NOTHING},
