...
#include "generate_cmo_prefetch_task.h"
...
Status GenerateCMOPrefetchTask::GenerateTask(std::vector<domi::TaskDef> &task_defs, const int32_t &stream_id,
  ...
  for (auto &cmo_attr : cmo_attrs) {
    ...
    if (InitCmoNodeAddrs(cmo_attr.node, task_args) != SUCCESS) {
      CM_LOGW("[GenTask][GenPrefetchTask] Init node[%s] addrs failed.", cmo_attr.node->GetName().c_str());
      return FAILED;
    }
    ...
    CM_LOGD("Generate prefetch cmo task id[%d] for node[%s] success.", cmo_id, node_.GetName().c_str());
