...
class FormatDtypeOpCustomizeSelector : public FormatDtypeSelectorBase {
 public:
  explicit FormatDtypeOpCustomizeSelector(OpStoreAdapterManagerPtr op_store_adapter_manager_ptr);
 ...
 private:
  Status ParseJsonStr(const OpKernelInfoPtr &op_kernel_info_ptr, const ge::OpDesc &op_desc,
                      const string &json_str, const bool &is_dynamic_check,
                      std::map<std::string, vector<ge::Format>> &format_map,
                      std::map<std::string, vector<ge::DataType>> &data_type_map);
  Status ConvertFormatDtype(const string &format_vec_str, const string &data_type_vec_str,
                            uint32_t &format_size_of_first_input_or_output, vector<ge::Format> &format_vec,
                            vector<ge::DataType> &data_type_vec);
  Status FilterC04Format(std::map<std::string, vector<ge::Format>> &format_map,
