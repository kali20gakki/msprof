...
Status OpDtypeSelectionStrategyForceFp16::Run(FormatDtypeSelectionBasicInfo& basic_info,
  ...
  FE_LOGD("Op %s: match dtype for tensor %u in force fp16 mode.", cur_op_desc_name.c_str(), basic_info.index);
  ...
  if (format_dtype_querier_ptr_->GetSupportDataTypes(basic_info.op_kernel_info_ptr, basic_info.tensor_kernel_info_ptr,
    ...
    REPORT_FE_ERROR("[GraphOpt][DtypeJdg][ForceFp16] Fail to get the support data_types.");
    return FAILED;
  }

  FE_LOGD("Op %s: match the dtype in force_fp16 mode, the expected dtype is %u.", cur_op_desc_name.c_str(),
          origin_dtype);
