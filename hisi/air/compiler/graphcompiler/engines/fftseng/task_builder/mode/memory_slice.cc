...
Status Block2DataCtxParam(const Block &block, int64_t dtype_size, int64_t burst_len,
  ...
  if (burst_len <= 0) {
    REPORT_FFTS_ERROR("Burst len %ld is less than zero.", burst_len);
    return FAILED;
  }
  FFTS_INT64_MULCHECK(block.dim[0], dtype_size);
  ...
  for (int64_t i = 0; i < block.count; i++) {
    ...
    for (int64_t j = 0; j < inner_block_count; j++) {
      ...
      FFTS_INT64_MULCHECK(block.dim_stride[1], dtype_size);
      ctx.stride_inner = block.dim_stride[1] * dtype_size;
      ctx.num_outter = block.dim[kDimIndex];
      FFTS_INT64_MULCHECK(block.dim_stride[kDimIndex], dtype_size);
      ctx.stride_outter = block.dim_stride[kDimIndex] * dtype_size;
...
Status DimensionMerge(const std::vector<int64_t> &shape,
  ...
  if (dim_count != slice.size()) {
    REPORT_FFTS_ERROR("[GenTsk][DataTsk][DimMerge] the dim count %zu is not same as slice size %zu.",
  ...
  for (size_t i = 0; i < dim_count; i++) {
    ...
    if (slice_low < 0 || slice_high <= slice_low) {
      REPORT_FFTS_ERROR("[GenTsk][DataTsk][DimMerge] slice_low %ld is < 0.", slice_low);
      return FAILED;
    }

    FFTS_LOGD("before block is: dim {%ld, %ld, %ld}, dim_stride {%ld, %ld, %ld}."
              "slice low = %ld"
              "ori dim = %ld"
              "blk.offset = %ld."
              "stride = %ld",
    ...
    if (slice_high - slice_low != dim) {
      ...
      if (curr_dim >= kBlockDim) {
        REPORT_FFTS_ERROR("[GenTsk][DataTsk][DimMerge] curr_dim %d is larger than max block dim(3).",
                          curr_dim);
        return FAILED;
      }
    ...
    FFTS_LOGD("after block is: dim {%ld, %ld, %ld}, dim_stride {%ld, %ld, %ld}."
              "slice low = %ld"
              "ori dim = %ld"
              "blk.offset = %ld."
              "stride = %ld",
  ...
  block = blk;
  FFTS_LOGD("block is: dim {%ld, %ld, %ld}, dim_stride {%ld, %ld, %ld}.",
...
Status MemorySlice::GenerateDataCtxParam(const std::vector<int64_t> &shape,
  ...
  if (DimensionMerge(shape, slice, block) != SUCCESS) {
    ...
    Block2DataCtxParam(block, dtype_size, burst_len, data_ctx);
  }
  return SUCCESS;
}
...
Status GetSliceList(const ge::NodePtr &node, int index, bool is_input, ge::GeTensorDescPtr &tensor,
  ...
  if (is_input) {
    ...
    if (slice_info_ptr->input_tensor_slice.size() <= thread_id) {
      REPORT_FFTS_ERROR("[GenTsk][DataTsk][MemSlice]input_tensor_slice of node %s is empty.",
                        node->GetName().c_str());
      return FAILED;
    }

    if (slice_info_ptr->input_tensor_slice[thread_id].size() <= static_cast<size_t>(index)) {
      REPORT_FFTS_ERROR("[GenTsk][DataTsk][MemSlice]size of input thread %u of node %s is %zu, which is <= %d.",
                        thread_id, node->GetName().c_str(), slice_info_ptr->input_tensor_slice[thread_id].size(),
                        index);
      return FAILED;
    }
    ...
    if (slice_info_ptr->thread_mode) {
      ...
      if (slice_info_ptr->input_tensor_slice[last_index].size() <= static_cast<size_t>(index)) {
        REPORT_FFTS_ERROR("[GenTsk][DataTsk][MemSlice] Size of input last_index[%zu] of node[%s] is %zu, which is <= \
                          %d.", last_index, node->GetName().c_str(),
                          slice_info_ptr->input_tensor_slice[last_index].size(), index);
        return FAILED;
      }
    ...
    if (slice_info_ptr->output_tensor_slice[thread_id].size() <= static_cast<size_t>(index)) {
      REPORT_FFTS_ERROR("[GenTsk][DataTsk][MemSlice]size of output thread %u of node %s is %zu, which is <= %d.",
                        thread_id, node->GetName().c_str(), slice_info_ptr->output_tensor_slice[thread_id].size(),
                        index);
      return FAILED;
    }
    ...
    if (slice_info_ptr->thread_mode) {
      ...
      if (slice_info_ptr->output_tensor_slice[last_index].size() <= static_cast<size_t>(index)) {
        REPORT_FFTS_ERROR("[GenTsk][DataTsk][MemSlice] Size of output last_index[%zu] of node[%s] is %zu, which is <= \
                          %d", last_index, node->GetName().c_str(),
                          slice_info_ptr->output_tensor_slice[last_index].size(), index);
        return FAILED;
      }
...
Status MemorySlice::GenerateManualDataCtxParam(const ge::NodePtr &node, int index, bool is_input, int64_t burst_len,
  ...
  if (GetSliceList(node, index, is_input, tensor, slice_list) != SUCCESS) {
    REPORT_FFTS_ERROR("[GenTsk][DataTsk][GenManualDataCtxParam] Index[%d] of node[%s] Get slice list failed.",
                      index, node->GetName().c_str());
    return FAILED;
  }
  if (slice_list.size() != 1) {
    REPORT_FFTS_ERROR("[GenTsk][DataTsk][GenManualDataCtxParam] Index[%d] of node[%s] slice list size is %zu.",
  ...
  FFTS_LOGD("[GenManualDataCtxParam] slice size is: %zu, Shape of index %d of node %s is %s.", slice_list.size(), index,
            node->GetName().c_str(), fe::StringUtils::IntegerVecToString(shape.GetDims()).c_str());
  FFTS_LOGD("Range is: {");
  ...
  FFTS_LOGD("}");
  return GenerateDataCtxParam(shape.GetDims(), slice_list[0], tensor->GetDataType(), burst_len, data_ctx);
}
...
Status MemorySlice::GenerateAutoDataCtxParam(const ge::NodePtr &node, int index, bool is_input, int64_t burst_len,
  ...
  vector<std::vector<DimRange>> slice_list;
  if (GetSliceList(node, index, is_input, tensor, slice_list) != SUCCESS) {
    REPORT_FFTS_ERROR("[GenTsk][DataTsk][GenAutoDataCtxParam] Index[%d] of node[%s] Get slice list failed.",
                      index, node->GetName().c_str());
    return FAILED;
  }
  if (slice_list.size() <= 1) {
  ...
  FFTS_CHECK_NOTNULL(tensor);
  auto shape = tensor->MutableShape();
  FFTS_LOGD("[GenAutoDataCtxParam] slice size is: %zu, Shape of index %d of node %s is %s.", slice_list.size(), index,
            node->GetName().c_str(), fe::StringUtils::IntegerVecToString(shape.GetDims()).c_str());
  FFTS_LOGD("Range is: {");
  for (auto eles : slice_list) {
    for (auto ele : eles) {
  ...
  for (const auto &slice : slice_list) {
     std::vector<DataContextParam> data_ctx;
     GenerateDataCtxParam(shape.GetDims(), slice, tensor->GetDataType(), burst_len, data_ctx);
     if (data_ctx.size() != 1) {
       FFTS_LOGW("[GenTsk][DataTsk][GenAutoDataCtxParam] Index[%d] of node[%s] data_ctx size:[%zu] != 1.", index,
