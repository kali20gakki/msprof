...
class StridedSliceKernel : public Kernel {
 ...
 private:
  ...
  static Status StrideCal(const int64_t x_dims_i, int64_t &begin_i, int64_t &end_i, int64_t &stride_i,
                   int64_t &dim_final) ;
  void ExpandDimsWithNewAxis(const ConstGeTensorPtr &begin_tensor, const size_t x_dims_num, std::vector<int64_t> &x_dims);
  void ExpandStrideWithEllipsisMask(const size_t x_dims_num,
                                    const std::vector<int64_t> &x_dims, std::vector<int64_t> &orig_begin_vec,
