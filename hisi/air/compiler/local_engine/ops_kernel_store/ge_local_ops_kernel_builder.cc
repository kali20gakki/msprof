...
graphStatus GeLocalOpsKernelBuilder::CalcMemSizeByNodeType(OpDescPtr &op_desc, GeTensorDesc &output_tensor,
  ...
  if ((node_type == kConstantOpType) && (data_type == DT_STRING)) {
    return CalcConstantStrMemSize(op_desc, output_mem_size);
  }
  if (node_type == kDataOpType) {
  ...
  return TensorUtils::CalcTensorMemSize(output_shape, format, data_type, output_mem_size);
}
...
graphStatus GeLocalOpsKernelBuilder::SetSizeForStringInput(Node &node, GeTensorDesc &output_tensor,
  ...
  DataType data_type = output_tensor.GetDataType();
  if ((node_type != kDataOpType) || (data_type != DT_STRING)) {
  ...
  if (ge::AttrUtils::GetListInt(node_desc, "_op_max_size", op_max_size)) {
    if (op_max_size.size() <= anchor_idx) {
      REPORT_CALL_ERROR("E19999", "anchor index invalid, anchor_idx:%zu, total:%zu", anchor_idx, op_max_size.size());
...
Status GeLocalOpsKernelBuilder::CalcOpRunningParam(Node &node) {
  ...
  if (op_desc == nullptr) {
    REPORT_CALL_ERROR("E19999", "param node has no opdesc, check invalid.");
...
Status GeLocalOpsKernelBuilder::CalcConstantStrMemSize(const OpDescPtr &op_desc, int64_t &mem_size) const {
  if (op_desc == nullptr) {
