...
namespace ge {
namespace {
const char_t *kEnvKeyOppPath = "ASCEND_OPP_PATH";
const char_t *kHostCpuLibRelativePath = "/op_impl/built-in/host_cpu";
...
ge::Status HostCpuEngine::Initialize(const std::string &path_base) {
  ...
  std::string lib_dir;
  GE_CHK_STATUS_RET_NOLOG(GetLibPath(path_base, lib_dir));
...
Status HostCpuEngine::GetLibPath(const std::string &path_base, std::string &lib_path) {
  GELOGI("Start to get host cpu lib path");
  char_t path_env[MMPA_MAX_PATH] = {};
  const auto env_res = mmGetEnv(kEnvKeyOppPath, &path_env[0], static_cast<uint32_t>(MMPA_MAX_PATH));
  if (env_res == EN_OK) {
    lib_path = std::string(&path_env[0]);
    if (!lib_path.empty()) {
  ...
  lib_path = lib_path.substr(0U, lib_path.rfind('/'));
  lib_path = lib_path.substr(0U, lib_path.rfind('/'));
  lib_path += "/opp";
  lib_path += kHostCpuLibRelativePath;

  GELOGI("Get host cpu so path from: %s", lib_path.c_str());
  return SUCCESS;
}
