...
DEFINE_string(status_check, "", "Optional; switch for status check such as overflow");
...
class GFlagUtils {
 public:
   ...
   * @return void
   */
  static void InitGFlag(int32_t argc, char *argv[]) {
    // -help
  ...
  static Status CheckFlags() {
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
        (FLAGS_mode == static_cast<int32_t>(ge::RunMode::GEN_OM_MODEL)) && FLAGS_output == "",
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
        (FLAGS_display_model_info != "0") && (FLAGS_display_model_info != "1"),
    ...
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
        (FLAGS_shape_generalized_build_mode != "shape_generalized") &&
            (FLAGS_shape_generalized_build_mode != "shape_precise"),
        REPORT_INNER_ERROR(
            "E19999",
            "Value[%s] of shape_generalized_build_mode is invalid, must be shape_generalized or shape_precise.",
            FLAGS_shape_generalized_build_mode.c_str());
        ret = ge::FAILED,
        "[Check][Parameter]Input parameter[--shape_generalized_build_mode]'s value must be shape_generalized or "
        "shape_precise.");
   ...
   * false: Not set up
   * */
  static bool CheckFlagSet(std::string flag) {
    gflags::CommandLineFlagInfo info;
    return !(gflags::GetCommandLineFlagInfo(flag.c_str(), &info) && info.is_default);
  }

 private:
  static bool CheckEncryptModeValid(const int32_t encrypt_mode) {
  ...
  static Status CheckFrameWorkValid(int32_t framework, const std::string &weight_file) {
    if (framework != static_cast<int32_t>(domi::CAFFE) && framework != static_cast<int32_t>(domi::TENSORFLOW) &&
        framework != static_cast<int32_t>(domi::MINDSPORE)  && framework != static_cast<int32_t>(domi::ONNX)) {
    ...
    if ((framework == static_cast<int32_t>(domi::CAFFE)) && (weight_file == "")) {
    ...
    if ((framework == static_cast<int32_t>(domi::TENSORFLOW)) && (weight_file != "")) {
      GELOGW("Parameter weight is ignored for TensorFlow.");
    }

    if ((framework == static_cast<int32_t>(domi::ONNX)) && (weight_file != "")) {
...
void GetPluginSoFileList(const std::string &path, std::vector<std::string> &fileList, std::string &caffe_parser_path) {
  ...
  std::vector<std::string> v_path = ge::StringUtils::Split(path, ':');
...
Status CreateInputsForInference(const ge::Graph &graph, std::vector<ge::GeTensor> &inputs) {
  ...
  for (ge::NodePtr &input_node : compute_graph->GetAllNodes()) {
    ...
    if (op->GetType() == ge::DATA) {
      ...
      std::string data_type_str = ge::TypeUtils::DataTypeToSerialString(data_type);
...
domi::Status GenerateModel(std::map<std::string, std::string> &options, std::string output) {
  ge::GeGenerator ge_generator;
  ge::Status geRet;
  ...
  if (FLAGS_framework == domi::MINDSPORE) {
    ...
    atc_params.insert(std::pair<std::string, std::string>(ge::INPUT_SHAPE_RANGE, FLAGS_input_shape_range));
    ...
    atc_params.insert(std::pair<std::string, std::string>(string(ge::OUTPUT_DATATYPE), FLAGS_output_type));
    ...
    if (FLAGS_mode == static_cast<int32_t>(ge::RunMode::ONLY_PRE_CHECK)) {
      (void)ge_generator.Finalize();
      (void)ge::GELib::GetInstance()->Finalize();
      if (ret != ge::SUCCESS) {
...
static void SetEnvForSingleOp(std::map<std::string, std::string> &options) {
  ...
  options.emplace(ge::OPTION_EXEC_HCCL_FLAG, flag_off);
}
...
domi::Status GenerateSingleOp(const std::string& json_file_path) {
  ...
  ErrorManager::GetInstance().SetStage(error_message::kModelCompile, error_message::kParser);
  std::vector<ge::SingleOpBuildParam> build_params;
...
domi::Status GenerateOmModel() {
  ...
  const int32_t f_stream_num = 1;
  std::map<std::string, std::string> options;
  options.insert(std::pair<std::string, std::string>(std::string(ge::FRAMEWORK_TYPE),
                                                     std::to_string(FLAGS_framework)));
  options.insert(std::pair<std::string, std::string>(std::string(ge::STREAM_NUM), std::to_string(f_stream_num)));
  options.insert(std::pair<std::string, std::string>(std::string(ge::CALIBRATION_CONF_FILE), FLAGS_cal_conf));
  options.insert(std::pair<std::string, std::string>(std::string(ge::OUTPUT_NODE_NAME), FLAGS_out_nodes));
  options.insert(std::pair<std::string, std::string>(std::string(ge::INSERT_OP_FILE), FLAGS_insert_op_conf));
  options.insert(std::pair<std::string, std::string>(std::string(ge::OP_PRECISION_MODE), FLAGS_op_precision_mode));
  options.insert(std::pair<std::string, std::string>(std::string(ge::PRECISION_MODE), FLAGS_precision_mode));
  options.insert(std::pair<std::string, std::string>(std::string(ge::TUNE_DEVICE_IDS), FLAGS_device_id));

  options.insert(std::pair<std::string, std::string>(std::string(ge::RUN_FLAG), std::to_string(0)));
  options.insert(std::pair<std::string, std::string>(std::string(ge::TRAIN_FLAG), std::to_string(0)));
  options.insert(std::pair<std::string, std::string>(std::string(ge::OPTION_EXEC_HCCL_FLAG), std::to_string(0)));

  if (!FLAGS_output_type.empty()) {
    options.insert(std::pair<std::string, std::string>(std::string(ge::OUTPUT_DATATYPE), FLAGS_output_type));
  }

  options.insert(std::pair<std::string, std::string>(std::string(ge::OP_SELECT_IMPL_MODE), FLAGS_op_select_implmode));
  options.insert(std::pair<std::string, std::string>(std::string(ge::OPTYPELIST_FOR_IMPLMODE),
                                                     FLAGS_optypelist_for_implmode));

  if (!FLAGS_input_fp16_nodes.empty()) {
    GELOGI("FLAGS_input_fp16_nodes : %s .", FLAGS_input_fp16_nodes.c_str());
    options.insert(std::pair<std::string, std::string>(ge::INPUT_FP16_NODES, FLAGS_input_fp16_nodes));
  }

  options.insert(std::pair<std::string, std::string>(std::string(ge::AUTO_TUNE_MODE), FLAGS_auto_tune_mode));

  options.insert(std::pair<std::string, std::string>(std::string(ge::OPTION_EXEC_DISABLE_REUSED_MEMORY),
                                                     std::to_string(FLAGS_disable_reuse_memory)));

  options.insert(std::pair<std::string, std::string>(std::string(ge::SOC_VERSION), FLAGS_soc_version));

  options.insert(std::pair<std::string, std::string>(std::string(ge::CORE_TYPE), FLAGS_core_type));

  options.insert(std::pair<std::string, std::string>(std::string(ge::AICORE_NUM), FLAGS_aicore_num));

  options.insert(std::pair<std::string, std::string>(std::string(ge::BUFFER_OPTIMIZE), FLAGS_buffer_optimize));

  options.insert(std::pair<std::string, std::string>(std::string(ge::ENABLE_SMALL_CHANNEL),
                                                     FLAGS_enable_small_channel));

  options.insert(std::pair<std::string, std::string>(std::string(ge::FUSION_SWITCH_FILE), FLAGS_fusion_switch_file));

  options.insert(std::pair<std::string, std::string>(
      std::string(ge::ENABLE_COMPRESS_WEIGHT),
      (FLAGS_enable_compress_weight == "true") ? ge::kEnableCompressWeightTrue : ge::kEnableCompressWeightFalse));

  options.insert(std::pair<std::string, std::string>(std::string(ge::ENABLE_SPARSE_MATRIX_WEIGHT),
                                                     std::to_string(FLAGS_sparsity)));

  options.insert(std::pair<std::string, std::string>(std::string(ge::ENABLE_SINGLE_STREAM),
                                                     FLAGS_enable_single_stream));

  options.insert(std::pair<std::string, std::string>(std::string(ge::DEBUG_DIR), FLAGS_debug_dir));

  if (!FLAGS_status_check.empty()) {
    options.insert(std::pair<std::string, std::string>(std::string(ge::STATUS_CHECK), FLAGS_status_check));
  }

  options.insert(std::pair<std::string,
                 std::string>(std::string(ge::OP_COMPILER_CACHE_DIR), FLAGS_op_compiler_cache_dir));

  options.insert(std::pair<std::string, std::string>(std::string(ge::OP_COMPILER_CACHE_MODE),
                                                     FLAGS_op_compiler_cache_mode));

  SetDynamicInputSizeOptions();

  if (!FLAGS_save_original_model.empty()) {
    options.insert(std::pair<std::string, std::string>(std::string(ge::SAVE_ORIGINAL_MODEL),
                                                       FLAGS_save_original_model));
    options.insert(std::pair<std::string, std::string>(std::string(ge::ORIGINAL_MODEL_FILE),
                                                       FLAGS_output + "_original.om"));
  }

  options.insert(std::pair<std::string, std::string>(std::string(ge::OP_DEBUG_LEVEL),
                                                     std::to_string(FLAGS_op_debug_level)));

  options.insert(std::pair<std::string, std::string>(std::string(ge::MDL_BANK_PATH_FLAG), FLAGS_mdl_bank_path));

  options.insert(std::pair<std::string, std::string>(std::string(ge::OP_BANK_PATH_FLAG), FLAGS_op_bank_path));

  options.insert(std::pair<std::string, std::string>(std::string(ge::DISPLAY_MODEL_INFO), FLAGS_display_model_info));

  options.insert(std::pair<std::string, std::string>(std::string(ge::MODIFY_MIXLIST), FLAGS_modify_mixlist));

  options.insert(std::pair<std::string, std::string>(std::string(ge::SHAPE_GENERALIZED_BUILD_MODE),
...
bool CheckRet(domi::Status ret) {
  if (ret != domi::SUCCESS) {
    ...
    } else if (FLAGS_mode == static_cast<int32_t>(ge::RunMode::GEN_OM_MODEL)) {
      GELOGW("ATC generate offline model failed.");
    } else if (FLAGS_mode == static_cast<int32_t>(ge::RunMode::MODEL_TO_JSON)) {
      GELOGW("ATC convert model to json file failed.");
    } else if (FLAGS_mode == static_cast<int32_t>(ge::RunMode::PBTXT_TO_JSON)) {
...
int32_t init(int32_t argc, char* argv[]) {
  ...
  std::string cmdline = gflag_argv == nullptr ? "" : gflag_argv;
  domi::GetContext().atc_cmdline = cmdline;
  // set log level
  int32_t ret = -1;
  const std::set<std::string> log_level = {"null", "debug", "info", "warning", "error"};
...
int32_t main_impl(int32_t argc, char* argv[]) {
  ...
  do {
    ...
    if (FLAGS_mode == (static_cast<int32_t>(ge::RunMode::GEN_OM_MODEL)) ||
        FLAGS_mode == (static_cast<int32_t>(ge::RunMode::ONLY_PRE_CHECK))) {
      GE_IF_BOOL_EXEC(GenerateOmModel() != domi::SUCCESS,
                      ret = domi::FAILED;
                      break);
    } else if (FLAGS_mode == static_cast<int32_t>(ge::RunMode::MODEL_TO_JSON)) {  // Mode 1, transfer model to JSON
  ...
  if (!CheckRet(ret)) {
    std::cout << "ATC run failed, Please check the detail log, Try \'atc --help\' for more information" << std::endl;
    int32_t result = ErrorManager::GetInstance().OutputErrMessage(STDOUT_FILENO);
    if (result != 0) {
