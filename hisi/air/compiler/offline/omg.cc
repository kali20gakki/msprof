      ...
      if (node->GetType() == DATA) {
        ...
        for (auto dim : tensor_desc->GetShape().GetDims()) {
          if (dim < 0) {
            ...
            const std::string reason = "maybe you should set input_shape to specify its shape";
...
static Status CheckInputFp16Nodes(const ComputeGraphPtr &graph, const std::string &input_fp16_nodes,
...
static Status ParseOutputFp16NodesFormat(const std::string &is_output_fp16) {
...
Status ParseOutputType(const std::string &output_type, std::map<std::string,
...
///
Status InitDomiOmgContext(const std::string &input_shape, const std::string &input_format,
...
Status ParseOutNodes(const std::string &out_nodes) {
  try {
    // parse output node
    if (!out_nodes.empty()) {
...
FMK_FUNC_HOST_VISIBILITY Status ParseGraph(ge::Graph &graph, const std::map<std::string, std::string> &atc_params,
...
void GetGroupName(ge::proto::ModelDef &model_def) {
  ...
  GE_IF_BOOL_EXEC(
      fusionModelOpListIter != modelAttrMap->end(), int32_t fusionOpIndex = 0;
      for (int32_t i = 0; i < model_def.graph_size(); i++) {
        ...
        for (int32_t j = 0; j < graph->op_size(); j++) {
          ...
          uint16_t l1_id = ((static_cast<uint64_t>(scope_id) & 0xFFFF0000)) >> 16;
...
FMK_FUNC_HOST_VISIBILITY Status ConvertPbtxtToJson(const char *model_file, const char *json_file) {
  ...
  try {
    ge::proto::ModelDef model_def;
    const std::string file_input(static_cast<char *>(model.model_data), model.model_len);
    const bool flag = google::protobuf::TextFormat::ParseFromString(file_input, &model_def);
    if (!flag) {
      free_model_data(&model.model_data);
