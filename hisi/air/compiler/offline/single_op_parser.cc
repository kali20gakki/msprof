...
constexpr int32_t kShapeRangePairSize = 2;
constexpr int32_t kShapeRangeLow = 0;
constexpr int32_t kShapeRangeHigh = 1;
constexpr int32_t kMaxFileNameLen = 128;
...
std::string GenerateFileName(const SingleOpDesc &single_op_desc, int32_t index) {
...
bool AttrValueIsString(const Json &j, const std::string &key) {
  try {
    std::string tmp_str = j.at(key).get<string>();
    return true;
...
std::unique_ptr<OpDesc> SingleOpParser::CreateOpDesc(const std::string &op_type) {
