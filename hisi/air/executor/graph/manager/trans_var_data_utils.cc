...
class RtContextSwitchGuard {
 public:
  RtContextSwitchGuard(const rtCtxMode_t mode, const uint32_t device_id)
    ...
    if (MdsUtils::CtxCreate(&current_, mode, static_cast<int32_t>(device_id)) != SUCCESS) {
      REPORT_CALL_ERROR("E19999", "Call CtxSetCurrent failed, device_id:%u", device_id);
...
int64_t CalcVarSizeInBytes(const GeTensorDesc &desc) {
  ...
  for (size_t dim_index = 0U; dim_index < dim_num; ++dim_index) {
    if (Int64MulCheckOverflow(var_size, shape.GetDim(dim_index)) != SUCCESS) {
      return -1;
    }
...
Status CopyVarToDevice(const NodePtr &var, const formats::TransResult &trans_result, void *const var_addr) {
  ...
  const auto ret = rtMemcpy(var_addr, trans_result.length, reinterpret_cast<void *>(trans_result.data.get()),
...
Status CopyVarFromDevice(const uint64_t session_id, const NodePtr &var, std::unique_ptr<uint8_t[]> &var_data,
  ...
  const auto ret = VarManager::Instance(session_id)->GetVarAddr(var->GetName(), input_desc, var_logic);
  if (ret != SUCCESS) {
  ...
  const auto rt_rslt = rtMemcpy(reinterpret_cast<void *>(var_host.get()), static_cast<uint64_t>(var_size_bytes),
                                reinterpret_cast<void *>(var_addr), static_cast<uint64_t>(var_size_bytes),
...
Status TransVarOnHost(uint8_t *const var_data, const VarTransRoad &trans_road, formats::TransResult &result) {
  ...
  for (const auto &trans_info : trans_road) {
    if ((trans_info.node_type == RESHAPE) || (trans_info.node_type == REFORMAT) ||
        (trans_info.node_type == SQUEEZEV2) || (trans_info.node_type == UNSQUEEZEV2)) {
    ...
    if ((trans_info.node_type == TRANSDATA) || (trans_info.node_type == TRANSPOSED)) {
      ...
      const auto ret = formats::TransDataFormat({src_data, src_format, dst_format, src_shape, dst_shape, data_type},
                                                tmp_result);
      if (ret != SUCCESS) {
      ...
      const auto src_data_size = (input_shape.GetShapeSize() == 0) ? 1 : input_shape.GetShapeSize();
      ...
      GELOGD("Trans data type from %s to %s, input shape %s, data size %ld",
             ...
             src_data_size);
      const auto ret = formats::TransTensorDataType(
...
Status TransTensor(const uint8_t *const var_data, const NodePtr &var_src, const NodePtr &var_dst,
  ...
  const auto src_data_shape_size = var_src->GetOpDesc()->GetOutputDesc(0U).GetShape().GetShapeSize();
...
Status CopyTensorFromSrcVarNode(const NodePtr &var_src,
  ...
  const auto src_data_type = output_desc.GetDataType();
  const auto src_shape = output_desc.GetShape();
  const auto src_format = output_desc.GetFormat();
  ...
  const auto data_type = dst_tensor_desc.GetDataType();
  const auto data_shape = dst_tensor_desc.GetShape();
  const auto data_format = dst_tensor_desc.GetFormat();
...
Status TransVarDataUtils::TransAllVarData(const std::vector<NodePtr> &variable_nodes,
  ...
  ThreadPool executor(kDefaultVarTransThreadNum);
  ...
  for (auto &node : variable_nodes) {
    ...
    const auto trans_func = [](const NodePtr &inner_node, const uint64_t inner_session_id, const rtContext_t ctx,
                               const uint32_t inner_graph_id, const uint32_t inner_device_id,
                               const error_message::Context &error_context) -> Status {
      ...
      if (rt_ret != RT_ERROR_NONE) {
        ...
        GELOGE(RT_FAILED, "[Call][RtCtxSetCurrent] failed, session_id:%lu, graph_id:%u, ret:0x%X.", inner_session_id,
               inner_graph_id, rt_ret);
