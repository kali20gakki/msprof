...
namespace {
const size_t kAlignedSize = 512U;
const ge::float32_t kSplitBlockThreshold = 0.5F;
...
Status RdmaPoolAllocator::InitMemory(const size_t mem_size) {
  ...
  const auto dev_id = static_cast<int32_t>(device_id);
...
uint8_t *RdmaPoolAllocator::Malloc(const size_t size, const uint32_t device_id) {
  ...
  const std::lock_guard<std::recursive_mutex> lock(mutex_);
  const std::set<Block *, Comparison>::const_iterator it = block_bin_.lower_bound(&key);
  if (it != block_bin_.cend()) {
    Block *block = *it;
