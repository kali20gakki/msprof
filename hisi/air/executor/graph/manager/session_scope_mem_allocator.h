...
class SessionScopeMemoryInfo {
 public:
  SessionScopeMemoryInfo(const size_t size, const std::shared_ptr<uint8_t> &ptr) : size(size), ptr(ptr) {}
...
class SessionScopeMemAllocator {
 public:
  explicit SessionScopeMemAllocator(const rtMemType_t memory_type);
