...
class SuperKernelFactory {
 ...
 public:
  ...
  Status Init();
  Status FuseKernels(const std::vector<const void *> &stub_func_list, const std::vector<void *> &args_addr_list,
                     const uint32_t block_dim, std::unique_ptr<skt::SuperKernel> &h) const;
};
}  // namespace skt
}  // namespace ge
#endif
