...
class SuperKernel {
 ...
 public:
  SuperKernel(const void *const stub, void *const ptr, const uint64_t sz, const uint32_t dim)
      : func_stub_(stub), dev_nav_table_(ptr), nav_table_size_(sz), block_dim_(dim) {}
  ~SuperKernel() = default;
  Status Launch(rtStream_t stream, const uint32_t dump_flag);
  Status Launch(rtStream_t stream, const uint32_t dump_flag, const char_t *const op_name);
