...
namespace ge {
namespace {
const uint32_t kTrueBranchStreamNum_1 = 1U;
}  // namespace
...
Status StreamSwitchTaskInfo::Init(const domi::TaskDef &task_def, DavinciModel *const davinci_model) {
  ...
  op_index_ = op_index;
  const auto input_data_addr = ModelUtils::GetInputAddrs(davinci_model_->GetRuntimeParam(), op_desc);
  SetInputAndValuePtr(input_data_addr);
  uint32_t cond = 0U;
  ...
  if (active_stream_list.size() != kTrueBranchStreamNum_1) {
    REPORT_INNER_ERROR("E19999", "active_stream_list.size():%zu in op:%s(%s) != kTrueBranchStreamNum_1:%u, "
                       "check invalid", active_stream_list.size(),
                       op_desc->GetName().c_str(), op_desc->GetType().c_str(), kTrueBranchStreamNum_1);
    GELOGE(FAILED, "[Check][Param] active_stream_list.size():%zu in op:%s(%s) must be equal %u",
           active_stream_list.size(), op_desc->GetName().c_str(), op_desc->GetType().c_str(), kTrueBranchStreamNum_1);
    return FAILED;
  }

  const size_t true_stream_index = active_stream_list.front();
  if (true_stream_index >= davinci_model_->GetStreamList().size()) {
  ...
  true_stream_ = davinci_model_->GetStreamList()[true_stream_index];
  true_stream_id_ = stream_switch_def.true_stream_id();
  davinci_model_->DisableZeroCopy(input_ptr_);
  davinci_model_->DisableZeroCopy(value_ptr_);
...
Status StreamSwitchTaskInfo::CalculateArgs(const domi::TaskDef &task_def, DavinciModel *const davinci_model) {
  GE_CHECK_NOTNULL(davinci_model);
  const auto &stream_switch_def = task_def.stream_switch();
  const uint32_t op_index = stream_switch_def.op_index();
  ...
  for (uint32_t i = 0U; i < STREAM_SWITCH_INPUT_NUM; ++i) {
    const std::string input_tensor_name = op_desc->GetName() + std::to_string(i);
    ...
    const auto tensor_desc = op_desc->GetInputDesc(i);
...
void StreamSwitchTaskInfo::SetInputAndValuePtr(const std::vector<void *> &input_data_addrs) {
  if (davinci_model_->IsKnownNode() && (fixed_addr_offset_.size() == STREAM_SWITCH_INPUT_NUM)) {
    input_ptr_ = davinci_model_->GetCurrentFixedAddr(fixed_addr_offset_[0U]);
    value_ptr_ = davinci_model_->GetCurrentFixedAddr(fixed_addr_offset_[1U]);
  } else {
    if ((!input_data_addrs.empty()) && (input_data_addrs.size() >= STREAM_SWITCH_INPUT_NUM)) {
      input_ptr_ = input_data_addrs[0U];
      value_ptr_ = input_data_addrs[1U];
    }
  }
}
