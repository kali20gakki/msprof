...
Status StreamActiveTaskInfo::Init(const domi::TaskDef &task_def, DavinciModel *const davinci_model) {
  ...
  if (internal_index >= active_stream_index_list.size()) {
    REPORT_INNER_ERROR("E19999", "flowctrl index:%zu >= active_stream_list size:%zu in op:%s, check invalid",
  ...
  if (active_stream_index_list[internal_index] >= davinci_model_->GetStreamList().size()) {
    REPORT_INNER_ERROR("E19999", "active_stream_index:%u in op:%s(%s) >= stream size:%zu in model, check invalid",
  ...
  GELOGI("InitStreamActiveTaskInfo Init Success, index:%zu, active stream:%p, stream id:%u.",
...
Status StreamActiveTaskInfo::Distribute() {
  ...
  GELOGI("StreamActiveTaskInfo Distribute Success. active stream:%p.", active_stream_);
  return SUCCESS;
}
