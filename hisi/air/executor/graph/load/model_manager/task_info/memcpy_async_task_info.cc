...
Status MemcpyAsyncTaskInfo::Init(const domi::TaskDef &task_def, DavinciModel *const davinci_model) {
  ...
  if (davinci_model_->IsKnownNode()) {
    src_ = PtrToPtr<void, uint8_t>(davinci_model_->GetCurrentArgsAddr(args_offset_, args_mem_type_));
    dst_ = reinterpret_cast<uint8_t *>(PtrToValue(src_) + sizeof(void *));
    // for zero copy
  ...
  if ((!memory_type_list.empty()) && (memory_type_list[0U] == static_cast<int64_t>(RT_MEMORY_TS))) {
    ...
    dst_ = static_cast<uint8_t *>(rts_param.ts_mem_mall->Acquire(static_cast<int64_t>(mem_offset),
...
Status MemcpyAsyncTaskInfo::Distribute() {
  ...
  const rtError_t rt_ret = rtMemcpyAsync(dst_, dst_max_, src_, count_, static_cast<rtMemcpyKind_t>(kind_), stream_);
...
Status MemcpyAsyncTaskInfo::CalculateArgs(const domi::TaskDef &task_def, DavinciModel *const davinci_model) {
  GE_CHECK_NOTNULL(davinci_model);
  const OpDescPtr op_desc = davinci_model->GetOpByIndex(task_def.memcpy_async().op_index());
  // the num of src and dst size is 2
  const uint32_t args_size = sizeof(void *) * 2U;
  args_mem_type_ = rtGetTsMemType(MEM_REQUEST_FEATURE_DEFAULT, args_size);
  GELOGI("memory_type: %u", args_mem_type_);
  args_offset_ = davinci_model->GetTotalArgsSize(args_mem_type_);
  davinci_model->SetTotalArgsSize(args_size, args_mem_type_);
  davinci_model_ = davinci_model;
  ...
  if (AttrUtils::GetStr(op_desc, ATTR_DYNAMIC_SHAPE_FIXED_ADDR, peer_input_name) && (!peer_input_name.empty())) {
    ...
    const auto tensor_desc = op_desc->GetOutputDesc(output_index);
...
Status MemcpyAsyncTaskInfo::SetIoAddrs(const OpDescPtr &op_desc, const domi::MemcpyAsyncDef &memcpy_async) {
  ...
  io_addrs_.emplace_back(PtrToPtr<uint8_t, void>(src));
  ...
  if (op_desc->HasAttr(ATTR_DYNAMIC_SHAPE_FIXED_ADDR)) {
    ...
    io_addrs_.emplace_back(PtrToPtr<uint8_t, void>(dst));
  }
...
Status MemcpyAsyncTaskInfo::UpdateArgs() {
  ...
  davinci_model_->SetTotalIOAddrs(io_addrs_, args_mem_type_);
