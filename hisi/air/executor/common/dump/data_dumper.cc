...
const uint32_t kAicpuUnloadFlag = 0U;
const uint64_t kOpDebugSize = 2048U;
const uint64_t kOpDebugShape = 2048U;
const uint64_t kDumpL1FusionOpMByteSize = 2097152U;  // 2 * 1024 * 1024
const int8_t kDecimalFormat = 10;
const uint32_t kAddrLength = sizeof(void *);
const ge::char_t *const kDumpOutput = "output";
const ge::char_t *const kDumpInput = "input";
const ge::char_t *const kDumpAll = "all";
...
void DataDumper::SaveOpDebugId(const uint32_t task_id, const uint32_t stream_id, const void *const op_debug_addr,
...
void DataDumper::SaveDumpTask(const uint32_t task_id, const uint32_t stream_id, const std::shared_ptr<OpDesc> &op_desc,
  ...
  op_list_.push_back({task_id, stream_id, op_desc, args, true, 0, 0, {}, 0});
...
Status DataDumper::GenerateOutput(toolkit::aicpu::dump::Output &output,
  ...
  for (const auto dim : tensor_descs.at(index).GetShape().GetDims()) {
    output.mutable_shape()->add_dim(static_cast<uint64_t>(dim));
  }
  for (const auto dim : tensor_descs.at(index).GetOriginShape().GetDims()) {
...
Status DataDumper::DumpRefOutput(const DataDumper::InnerDumpInfo &inner_dump_info,
  ...
  if (input_or_output == kDumpInput) {
    ...
    addr += (index + replace_input_size) * kAddrLength;
  ...
  for (size_t i = 0U; i < output_descs.size(); ++i) {
    ...
    if (AttrUtils::GetStr(&output_desc, ATTR_DATA_DUMP_REF, node_name_index)) {
      ...
      if (IsTensorDescWithSkipDumpAddrType(has_mem_type_attr, v_memory_type, i)) {
        ...
        const auto addr = inner_dump_info.args + ((i + input_size) * kAddrLength);
...
Status DataDumper::DumpOutput(const InnerDumpInfo &inner_dump_info, toolkit::aicpu::dump::Task &task) {
  ...
  for (const auto dim : inner_dump_info.dims) {
    output.mutable_shape()->add_dim(static_cast<uint64_t>(dim));
  }
  ...
  const auto data_addr = inner_dump_info.args +
                         (kAddrLength * static_cast<uint32_t>(inner_dump_info.input_anchor_index));
  ...
  for (size_t i = 0U; i < input_descs.size(); ++i) {
    ...
    if (AttrUtils::GetStr(&input_descs.at(i), ATTR_DATA_DUMP_REF, node_name_index)) {
      ...
      if (IsTensorDescWithSkipDumpAddrType(has_mem_type_attr, v_memory_type, i)) {
        ...
        const auto addr = inner_dump_info.args + (kAddrLength * i);
...
void DataDumper::GenerateOpBuffer(const uint64_t size, toolkit::aicpu::dump::Task &task) {
  if (need_generate_op_buffer_) {
    ...
    op_buffer.set_address(l1_fusion_addr_);
...
Status DataDumper::BuildTaskInfo(toolkit::aicpu::dump::OpMappingInfo &op_mapping_info) {
  for (const auto &op_iter : op_list_) {
    ...
    if (dump_properties_.GetDumpMode() == kDumpOutput) {
      const Status ret = DumpOutput(op_iter, task);
      if (ret != SUCCESS) {
    ...
    if (dump_properties_.GetDumpMode() == kDumpInput) {
      if (op_iter.is_task) {
        const Status ret = DumpInput(op_iter, task);
        if (ret != SUCCESS) {
