...
const uint64_t kLogicVarBase = 0UL;
const uint64_t kVarSize = 0UL;
}
...
std::string BuildTaskUtils::GetTaskInfo(const OpDescPtr &op_desc,
  ...
  if (op_desc->GetAllInputsSize() == inputs.size()) {
    (void)std::for_each(inputs.begin(), inputs.end(),
                        [&input_addrs](const DataBuffer &db) { input_addrs.push_back(db.data); });
  }
  if (op_desc->GetOutputsSize() == outputs.size()) {
    (void)std::for_each(outputs.begin(), outputs.end(),
                        [&output_addrs](const DataBuffer &db) { output_addrs.push_back(db.data); });
  }
  return InnerGetTaskInfo(op_desc, input_addrs, output_addrs);
}
