...
class AicpuExtInfoHandler {
 public:
  ...
  Status UpdateSessionInfo(const uint64_t session_id, const uint64_t kernel_id, const bool sess_flag);

  Status UpdateSessionInfoId(const uint64_t session_id);

  Status UpdateExecuteMode(const bool flag);

  Status UpdateEventId(const uint32_t event_id);
