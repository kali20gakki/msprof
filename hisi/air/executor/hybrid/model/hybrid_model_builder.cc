...
const uint32_t kHybridBytes = 8U;
const uint32_t kHybridStringHeadElems = 2U;
  ...
  for (const auto &node : graph.GetDirectNode()) {
    ...
    if (op_type == NETOUTPUT) {
      for (InDataAnchorPtr &in_data_anchor : node->GetAllInDataAnchors()) {
        ...
        const NodePtr &in_node = peer_out_anchor->GetOwnerNode();
        GE_CHECK_NOTNULL(in_node);
        const auto &input_desc = op_desc->GetInputDesc(static_cast<uint32_t>(in_data_anchor->GetIdx()));
        std::string tensor_name;
        const bool has_tensor_name = AttrUtils::GetStr(input_desc, ATTR_NAME_ORIGIN_OUTPUT_TENSOR_NAME, tensor_name);
...
Status CollectDependenciesForFusedGraph(const NodeItem &node_item,
  ...
  for (const auto &node : node_item.fused_subgraph->nodes) {
    ...
    for (const auto &input_name : depends) {
      ...
      if (src_node->GetType() != DATA_TYPE) {
        ...
        REPORT_INNER_ERROR("E19999", "[%s(%s)::%s(%s)] Node in fused subgraph can only depend on Data nodes,"
                           "but depend on %s actually.", node_item.NodeName().c_str(), node_item.NodeType().c_str(),
                           node->GetName().c_str(), node->GetType().c_str(),
                           src_node->GetType().c_str());
        return UNSUPPORTED;
      ...
      data_deps.emplace_back(src_op_desc.get(), std::make_pair(op_desc.get(), input_index));
    }
  }
...
Status HybridModelBuilder::InitNodeBinMode() {
  ...
  int32_t node_bin_mode = kNodeBinModeEnd;
  if (GetContext().GetOption("RUNTIME_NODE_BIN_MODE", node_bin_mode_str) == GRAPH_SUCCESS) {
    ...
    hybrid_model_.SetNodeBinMode(static_cast<NodeBinMode>(node_bin_mode));
  } else {
    GELOGD("The RUNTIME_NODE_BIN_MODE not set, run in default mode: one-node-single-bin");
    hybrid_model_.SetNodeBinMode(kOneNodeSingleBinMode);
  }
  return SUCCESS;
}
...
Status HybridModelBuilder::InitOverflowAddr() const {
  ...
  (void)AttrUtils::GetInt(root_graph, "globleworkspace_status_bytes", global_workpace_size);
...
Status HybridModelBuilder::BuildForSingleOp() {
  ...
  GE_CHK_STATUS_RET(InitNodeBinMode(), "[Init]][InitNodeBinMode] failed, model_name[%s]", ModelName().c_str());
  GE_CHK_STATUS_RET(IndexTaskDefs(hybrid_model_.root_graph_, it->second),
                    "[Invoke][IndexTaskDefs] failed, model_name[%s]", ModelName().c_str());
  GE_CHK_STATUS_RET(LoadGraph(), "[Invoke][LoadGraph] failed, model_name[%s]", ModelName().c_str());
  GE_CHK_STATUS_RET(InitWeights(), "[Invoke][InitWeights] failed, model_name[%s]", ModelName().c_str());
  GE_CHK_STATUS_RET(LoadTasks(), "[Invoke][LoadTasks] failed, model_name[%s]", ModelName().c_str());
...
void HybridModelBuilder::PrintDynamicType() const {
  const auto &root_graph = ge_root_model_->GetRootGraph();
  ...
  if (is_dynamic_input) {
    GEEVENT("current execute mode is dynamic input, model id = %u, model_name = %s.",
            hybrid_model_.model_id_, hybrid_model_.model_name_.c_str());
  } else {
    if (is_dynamic_in_process) {
      GEEVENT("current execute mode is dynamic in process, model id = %u, model_name = %s.",
              hybrid_model_.model_id_, hybrid_model_.model_name_.c_str());
    }
  }
}
...
Status HybridModelBuilder::BuildNodeItem(const NodePtr &node, NodeItem &node_item) {
  ...
  const std::vector<std::string> dependencies = node->GetOpDesc()->GetOpInferDepends();
  ...
  for (int32_t i = 0; i < num_outputs; ++i) {
    ...
    if (out_data_anchor == nullptr) {
      ...
      REPORT_CALL_ERROR("E19999", "out anchor[%d] of node %s(%s) is nullptr.",
                        i, node->GetName().c_str(), node->GetType().c_str());
...
Status HybridModelBuilder::GetOrCreateNodeItem(const NodePtr &node, NodeItem *&node_item) const {
  ...
  const bool ret = AttrUtils::GetStr(node->GetOpDesc(), ATTR_DATA_RELATED_AIPP_MODE, data_mode);
...
Status HybridModelBuilder::ParseDependencies(NodeItem &node_item, const std::vector<std::string> &dependencies,
                                             std::set<NodePtr> &dependent_for_shape_inference) {
  for (const auto &input_name : dependencies) {
    const int32_t input_index = node_item.op_desc->GetInputIndexByName(input_name);
    if (input_index < 0) {
      GELOGE(INTERNAL_ERROR, "[Get][InputIndex]failed, node:[%s(%s)] inputname: %s.",
             node_item.NodeName().c_str(), node_item.NodeType().c_str(), input_name.c_str());
      REPORT_CALL_ERROR("E19999", "GetInputIndexByName failed, node:[%s(%s)] inputname: %s.",
                        node_item.NodeName().c_str(), node_item.NodeType().c_str(), input_name.c_str());
      return INTERNAL_ERROR;
    }
...
Status HybridModelBuilder::ParseDependentInputNodes(NodeItem &node_item, const std::vector<std::string> &dependencies) {
...
Status HybridModelBuilder::ParseDependentForFusedSubgraph(const NodeItem &node_item,
  ...
  for (const auto &dep : data_deps) {
    ...
    if (!AttrUtils::GetInt(*data_op_desc, ATTR_NAME_PARENT_NODE_INDEX, parent_index)) {
      GELOGE(INTERNAL_ERROR, "[Get][Attr] failed, node:[%s(%s)]  attr:[%s]",
             data_op_desc->GetName().c_str(), data_op_desc->GetType().c_str(), ATTR_NAME_PARENT_NODE_INDEX.c_str());
      REPORT_CALL_ERROR("E19999", "invoke GetInt failed, node:[%s(%s)]  attr:[%s]",
...
Status HybridModelBuilder::UpdateAnchorStatus(const NodePtr &node) {
  if (NodeUtils::SetAllAnchorStatus(node) != GRAPH_SUCCESS) {
    GELOGE(INTERNAL_ERROR, "[Invoke][SetAllAnchorStatus] failed for node:[%s(%s)].",
           node->GetName().c_str(), node->GetType().c_str());
    REPORT_CALL_ERROR("E19999", "SetAllAnchorStatus failed for node:[%s(%s)].",
                      node->GetName().c_str(), node->GetType().c_str());
    return INTERNAL_ERROR;
  }
    ...
    if (peer_anchor == nullptr) {
      if (AnchorUtils::SetStatus(anchor, ANCHOR_SUSPEND) != GRAPH_SUCCESS) {
        GELOGE(INTERNAL_ERROR, "[Invoke][SetStatus] failed to set ANCHOR_SUSPEND, node:[%s(%s)].",
               node->GetName().c_str(), node->GetType().c_str());
        REPORT_CALL_ERROR("E19999", "SetStatus failed to set ANCHOR_SUSPEND, node:[%s(%s)].",
                          node->GetName().c_str(), node->GetType().c_str());
        return INTERNAL_ERROR;
      }
      ...
      if (AnchorUtils::SetStatus(anchor, ANCHOR_CONST) != GRAPH_SUCCESS) {
        GELOGE(INTERNAL_ERROR, "[Invoke][SetStatus] failed to set ANCHOR_CONST, node:[%s(%s)].",
               node->GetName().c_str(), node->GetType().c_str());
        REPORT_CALL_ERROR("E19999", "SetStatus failed to set ANCHOR_CONST, node:[%s(%s)].",
                          node->GetName().c_str(), node->GetType().c_str());
        return INTERNAL_ERROR;
      }
    } else {
      if (AnchorUtils::SetStatus(anchor, ANCHOR_DATA) != GRAPH_SUCCESS) {
        GELOGE(INTERNAL_ERROR, "[Invoke][SetStatus] failed to set ANCHOR_DATA, node:[%s(%s)].",
               node->GetName().c_str(), node->GetType().c_str());
        REPORT_CALL_ERROR("E19999", "SetStatus failed to set ANCHOR_DATA, node:[%s(%s)].",
                          node->GetName().c_str(), node->GetType().c_str());
        return INTERNAL_ERROR;
      }
    }
...
Status HybridModelBuilder::MergeInputInData(const NodePtr &node, const NodePtr &wrapped_node,
  ...
  if (!AttrUtils::GetInt(data_op_desc, ATTR_NAME_PARENT_NODE_INDEX, parent_index)) {
    GELOGE(FAILED, "[Invoke][GetInt] failed, node:[%s(%s)] attr:[%s]",
           data_op_desc->GetName().c_str(), data_op_desc->GetType().c_str(), ATTR_NAME_PARENT_NODE_INDEX.c_str());
        REPORT_CALL_ERROR("E19999", "GetInt failed, node:[%s(%s)] attr:[%s]",
                          data_op_desc->GetName().c_str(), data_op_desc->GetType().c_str(),
                          ATTR_NAME_PARENT_NODE_INDEX.c_str());
    return FAILED;
  }
...
Status HybridModelBuilder::MergeNetOutputInData(const NodePtr &parent_node, const OpDescPtr &net_output_desc,
  ...
  if (input_desc == nullptr) {
    GELOGE(INTERNAL_ERROR, "[Invoke][MutableInputDesc][%s(%s)] Failed to get input desc[%d]",
        ...
        REPORT_CALL_ERROR("E19999", "[%s(%s)] Failed to get input desc[%d].",
                          net_output_desc->GetName().c_str(), net_output_desc->GetType().c_str(), anchor_index);
    return INTERNAL_ERROR;
  }
...
Status HybridModelBuilder::UnfoldSubgraph(const ComputeGraphPtr &root_graph, const ComputeGraphPtr &parent_graph,
  ...
  if (depth >= kHybridSubgraphRecursion) {
    ...
    REPORT_INNER_ERROR("E19999", "[Unfold]There are too much recursion:%u > max:%u", depth, kHybridSubgraphRecursion);
    return FAILED;
  }
...
Status HybridModelBuilder::BuildOutputMapping(GraphItem &graph_item,
  ...
  for (const auto &in_data_anchor : node_item.node->GetAllInDataAnchors()) {
...
Status HybridModelBuilder::LoadGraph() {
  ...
  if (!GetContext().GetHostExecFlag()) {
    ...
    GE_CHK_GRAPH_STATUS_RET(UnfoldSubgraphs(root_graph, merged_graph),
                            "[Invoke][UnfoldSubgraphs]Failed to unfold subgraphs, model_name[%s]", ModelName().c_str());
  ...
  for (const auto &node : root_graph->GetAllNodes()) {
    ...
    const auto &functional_node = parent_graph->GetParentNode();
    if ((functional_node != nullptr) && IsFftsGraphNode(*functional_node->GetOpDesc())) {
      continue;
    }
  ...
  for (const auto &it : hybrid_model_.known_shape_sub_models_) {
    ...
    GE_CHK_GRAPH_STATUS_RET(it.second->GetGraph().GetName(graph_name), "[Get][Name] of subgraph failed");
    GE_CHECK_NOTNULL(graph_name.GetString());
    const auto &subgraph = hybrid_model_.GetRootGraph()->GetSubgraph(graph_name.GetString());
    GE_CHECK_NOTNULL(subgraph);
...
Status HybridModelBuilder::VarNodeToTensor(const NodePtr &var_node, std::unique_ptr<TensorValue> &tensor) const {
  ...
  if (dev_mem == nullptr) {
    ...
    REPORT_CALL_ERROR("E19999", "GetVarMemoryAddr failed, Failed to copy var %s(%s) from device,"
                      "cant not get var addr from logic addr %p",
                      var_node->GetName().c_str(), var_node->GetType().c_str(), var_logic);
    return INTERNAL_ERROR;
  }
...
Status HybridModelBuilder::CopyConstantData(const NodePtr &node, const GeTensor &tensor,
  ...
  if (tensor_desc.GetDataType() == DT_STRING) {
    ...
    const GeShape &tensor_shape = tensor_desc.GetShape();
    int64_t shape_size = tensor_shape.GetShapeSize();
    if (shape_size < 0) {
      GELOGE(FAILED, "[%s] shape size:%ld is invalid", node->GetName().c_str(), shape_size);
      return FAILED;
    }

    shape_size = ((shape_size == 0) && tensor_shape.GetDims().empty()) ? 1 : shape_size;
    if (CheckInt64Uint32MulOverflow(shape_size, kHybridBytes * kHybridStringHeadElems) != SUCCESS) {
      GELOGE(FAILED, "[Invoke][CheckInt64Uint32MulOverflow] failed because Shape size:%ld is invalid.", shape_size);
      return FAILED;
    }

    const size_t elem_num = static_cast<size_t>(shape_size);
    const size_t offset = (elem_num * kHybridBytes) * kHybridStringHeadElems;

    const auto data_base = PtrToValue(tensor.GetData().data());
    std::vector<uint64_t> vct_data(static_cast<uint64_t *>(ValueToPtr(data_base)),
                                   static_cast<uint64_t *>(ValueToPtr(data_base + cst_data.size())));
    const auto logic_base = vct_data[0U];
    for (size_t i = 0U; (i < elem_num) && ((i * kHybridStringHeadElems) < vct_data.size()); ++i) {
      vct_data[i * kHybridStringHeadElems] = vct_data[i * kHybridStringHeadElems] - logic_base + offset;
    }

    GELOGI("[IMAS]InitConstant memcpy graph_%u type[V] name[%s] output[%d] memaddr[%p] mem_size[%zu] datasize[%zu]",
           runtime_param_.graph_id, node->GetName().c_str(), 0, output_addr, output_size, cst_data.size());
    GE_CHK_RT_RET(rtMemcpy(output_addr, output_size, vct_data.data(), cst_data.size(), RT_MEMCPY_HOST_TO_DEVICE));
  } else {
    GELOGI("[IMAS]InitConstant memcpy graph_%u type[V] name[%s] output[%d] memaddr[%p] mem_size[%zu] datasize[%zu]",
...
Status HybridModelBuilder::InitFileConstantInHost(const int64_t output_size, const OpDescPtr &op_desc,
  ...
  if (output_size > 0) {
    ...
    GE_CHK_STATUS_RET(CopyOneWeightFromFile(mem_addr, file_path, static_cast<size_t>(output_size), malloc_size),
                      "Failed to copy data.");
    var_tensor = MakeUnique<TensorValue>(mem_addr, static_cast<size_t>(output_size));
  } else {
    var_tensor = MakeUnique<TensorValue>(nullptr, 0U);
  }
  return SUCCESS;
...
Status HybridModelBuilder::InitFileConstantInDevice(const int64_t output_size, const OpDescPtr &op_desc,
  ...
  if (tensor_value.GetSize() > 0U) {
    ...
    return CopyOneWeightFromFile(tensor_value.GetData(), file_path, static_cast<size_t>(output_size), left_size);
  } else {
...
Status HybridModelBuilder::InitFileConstantOps() {
  ...
  for (const auto &it : constant_op_nodes_) {
    ...
    if (GetContext().GetHostExecFlag()) {
      ...
      if (!VarManager::Instance(runtime_param_.session_id)->IsVarReady(var_node->GetName(), *tensor_desc)) {
        ...
        VarManager::Instance(runtime_param_.session_id)->SetVarIsReady(var_node->GetName(), *tensor_desc);
      }
    }
    (void)hybrid_model_.variable_tensors_.emplace(var_name, std::move(var_tensor));
  }
...
Status HybridModelBuilder::InitVariableTensors() const {
  ...
  for (const auto &it : hybrid_model_.host_variable_nodes_) {
    ...
    if (TensorUtils::CalcTensorMemSize(output_tensor.GetShape(), output_tensor.GetFormat(),
      ...
      REPORT_CALL_ERROR("E19999", "CalcTensorMemSize failed, node:%s(%s)",
    ...
    if (HostMemManager::Instance().QueryVarMemInfo(it.first, mem_info)) {
      ...
      mem_addr = mem_instance.Malloc(static_cast<size_t>(tensor_size));
    }

    if (mem_addr == nullptr) {
      REPORT_INNER_ERROR("E19999", "[Malloc][HostMem] for variable [%s(%s)] failed.",
                         op_desc->GetName().c_str(), op_desc->GetType().c_str());
...
Status HybridModelBuilder::InitWeights() const {
  ...
  for (const auto &subgraph_model : ge_root_model_->GetSubgraphInstanceNameToModel()) {
    ...
    auto *const mem_allocator = NpuMemoryAllocator::GetAllocator(hybrid_model_.device_id_, nullptr);
    GE_CHECK_NOTNULL(mem_allocator);
    auto sub_weight_buffer = TensorBuffer::Create(mem_allocator, weight_buffer.size());
    ...
    for (const auto &node : subgraph->GetDirectNode()) {
      ...
      if (v_weights.empty()) {
        GELOGE(INTERNAL_ERROR, "[Invoke][GetWeights][%s(%s)] Constant has no value",
               node->GetName().c_str(), node->GetType().c_str());
        REPORT_CALL_ERROR("E19999", "[%s(%s)] Constant has no value.",
                          node->GetName().c_str(), node->GetType().c_str());
        return INTERNAL_ERROR;
      }
      ...
      int64_t tensor_size = 0;
      GE_CHK_GRAPH_STATUS_RET(TensorUtils::GetSize(*op_desc->MutableOutputDesc(0U), tensor_size),
...
Status HybridModelBuilder::LoadTask(NodeItem &node_item) const {
  ...
  if ((load_ret != UNSUPPORTED) && (load_ret != SUCCESS)) {
    ...
    REPORT_CALL_ERROR("E19999", "[%s(%s)] Failed to load task",
                      node_ptr->GetName().c_str(), node_ptr->GetType().c_str());
    return load_ret;
  }
...
Status HybridModelBuilder::IndexTaskDefs(const ComputeGraphPtr &sub_graph, const GeModelPtr &ge_model) {
  ...
  for (int32_t i = 0; i < tasks.size(); ++i) {
    ...
    } else if (task_type == RT_MODEL_TASK_FFTS_PLUS_TASK) {
...
Status HybridModelBuilder::GetPeerNodeAcrossSubGraphs(const NodePtr &data_node,
  ...
  const auto &wrapped_node = data_node->GetOwnerComputeGraph()->GetParentNode();
  if (wrapped_node == nullptr) {
    REPORT_INNER_ERROR("E19999", "parentnode of node:[%s(%s)] in root graph is nullptr.",
  ...
  int32_t parent_index = 0;
  if (!AttrUtils::GetInt(data_op_desc, ATTR_NAME_PARENT_NODE_INDEX, parent_index)) {
    REPORT_CALL_ERROR("E19999", "get attr [%s] on node:[%s(%s)] failed.",
                      ATTR_NAME_PARENT_NODE_INDEX.c_str(),
                      data_op_desc->GetName().c_str(), data_op_desc->GetType().c_str());
  ...
  const auto &src_out_anchor = wrapped_node_in_anchor->GetPeerOutAnchor();
  if ((src_out_anchor == nullptr) || (src_out_anchor->GetOwnerNode() == nullptr)) {
    REPORT_INNER_ERROR("E19999", "Parent node of node:%s(%s) do not have peer anchor.",
                       data_node->GetName().c_str(), data_node->GetType().c_str());
  ...
  GELOGE(FAILED, "[Get][PeerNode]Failed to find peer node for %s::%s(%s)", sub_graph->GetName().c_str(),
         data_node->GetName().c_str(), data_node->GetType().c_str());
  REPORT_INNER_ERROR("E19999", "Failed to find peer node for %s::%s(%s).",
...
Status HybridModelBuilder::InitRuntimeParams() {
  ...
  if (ge_root_model_->GetSubgraphInstanceNameToModel().empty()) {
    ...
    REPORT_INNER_ERROR("E19999", "Root model has no sub model, model_name[%s]", ModelName().c_str());
    return INTERNAL_ERROR;
  }
...
Status HybridModelBuilder::InitModelMem() {
  hybrid_model_.var_mem_base_ = var_manager_->GetVarMemoryBase(RT_MEMORY_HBM, hybrid_model_.device_id_);
  auto total_var_size = hybrid_model_.TotalVarMemSize();
  if ((total_var_size == 0U) && (!constant_op_nodes_.empty())) {
    total_var_size = (var_manager_->GetVarMemSize(RT_MEMORY_HBM) > 0) ? var_manager_->GetVarMemMaxSize() : 0U;
    GELOGD("Model var size = 0. but got uninitialized constant. set var size to %zu.", total_var_size);
  }

  if ((total_var_size > 0U) && (hybrid_model_.var_mem_base_ == nullptr)) {
    GE_CHK_STATUS_RET(var_manager_->MallocVarMemory(total_var_size, hybrid_model_.device_id_),
                      "[Malloc][VarMemory] failed, size:%zu.", total_var_size);
    hybrid_model_.var_mem_base_ = var_manager_->GetVarMemoryBase(RT_MEMORY_HBM, hybrid_model_.device_id_);
  }

  runtime_param_.var_base = PtrToValue(hybrid_model_.var_mem_base_);
  auto *const mem_allocator = NpuMemoryAllocator::GetAllocator(hybrid_model_.device_id_, nullptr);
  GE_CHECK_NOTNULL(mem_allocator);
...
Status HybridModelBuilder::LoadDynamicSubgraph(const ComputeGraph &graph, const bool is_root_graph) {
...
Status HybridModelBuilder::LoadDynamicNodeItem(GraphItem &graph_item, const NodePtr &node,
  ...
  if ((node_item->fused_subgraph != nullptr) && (hybrid_model_.node_bin_mode_ == kOneNodeMultipleBinsMode)) {
...
Status HybridModelBuilder::ParseVarOutputs(NodeItem &node_item) const {
  for (int32_t i = 0; i < node_item.num_outputs; ++i) {
    ...
    if (var_name.empty()) {
      (void)AttrUtils::GetStr(output_tensor_desc, REF_VAR_SRC_VAR_NAME, var_name);
    }
    if (!var_name.empty()) {
  ...
  for (const auto &node_item : input_nodes) {
    ...
    if (is_root_graph) {
      ...
      if (!AttrUtils::GetInt(node->GetOpDesc(), ATTR_NAME_PARENT_NODE_INDEX, data_index)) {
        GELOGE(FAILED, "[Invoke][GetInt] [%s(%s)] Failed to get attr [%s]",
               node->GetName().c_str(), node->GetType().c_str(), ATTR_NAME_PARENT_NODE_INDEX.c_str());
        REPORT_CALL_ERROR("E19999", "call GetInt failed, [%s(%s)] Failed to get attr [%s]",
                          node->GetName().c_str(), node->GetType().c_str(), ATTR_NAME_PARENT_NODE_INDEX.c_str());
        return FAILED;
      }
    }
  ...
  for (const auto &item : nodes) {
    ...
    if (item.second.second > 0U) {
      ...
      if (node_size == ((node_index + post_num) + 1U)) {
        ...
        post_items.insert(node_item->data_send_.begin(), node_item->data_send_.end());
        post_items.insert(node_item->ctrl_send_.begin(), node_item->ctrl_send_.end());
        for (uint32_t i = 1U; i <= post_num; ++i) {
          GE_CHK_BOOL_RET_STATUS((node_index + i) < node_size, FAILED,
                                 "[Check][Param] node index:%zu invalid, node size:%zu", node_index, node_size);
          const auto &curr_item = graph_item.node_items_[node_index + i];
...
void HybridModelBuilder::LoadTbeKernelBinToOpDesc(const rtModelTaskType_t task_type, const GeModelPtr &ge_model,
                                                  const NodePtr &node) {
  if ((task_type == RT_MODEL_TASK_KERNEL) || (task_type == RT_MODEL_TASK_ALL_KERNEL) ||
      (task_type == RT_MODEL_TASK_FFTS_PLUS_TASK)) {
