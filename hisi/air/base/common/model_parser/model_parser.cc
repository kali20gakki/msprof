...
Status ModelParserBase::LoadFromFile(const char_t *const model_path,
                                     const int32_t priority, ge::ModelData &model_data) {
  ...
  const uint32_t len = static_cast<uint32_t>(fs.tellg());
...
Status ModelParserBase::ParseModelContent(const ge::ModelData &model, uint8_t *&model_data, uint32_t &model_len) {
  ...
  // Get data address
  uint8_t *const data = PtrToPtr<void, uint8_t>(ValueToPtr(PtrToValue(model.model_data) + sizeof(ModelFileHeader)));
  model_data = data;
