...
static void Fp16NormalizeVal(int16_t &exp_val, uint16_t &man) {
  ...
  } else if ((exp_val == 0UL) && (man == kFp16ManHideBit)) {
    exp_val++;
    man = 0U;
  } else {
    // do nothing
...
static uint16_t Fp16AddCalVal(const uint16_t &s_ret, int16_t e_ret, uint16_t m_ret, uint32_t m_trunc,
  ...
  const bool b_trunc_high = (g_round_mode == kRoundToNearest) && ((m_trunc & kFp32SignMask) > 0U);
  const bool b_trunc_left = (g_round_mode == kRoundToNearest) && ((m_trunc & kFp32AbsMax) > 0U);
