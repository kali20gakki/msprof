...
Status FileSaver::WriteData(const void * const data, uint64_t size, const int32_t fd) {
  GE_CHK_BOOL_TRUE_EXEC_WITH_LOG((size == 0U) || (data == nullptr), return PARAM_INVALID);
...
Status FileSaver::SaveWithFileHeader(const std::string &file_path, const ModelFileHeader &file_header,
                                     const void * const data, const int32_t len) {
  ...
  do {
    ...
    // write data
    GE_CHK_BOOL_EXEC(WriteData(data, static_cast<uint64_t>(len), fd) == SUCCESS, ret = FAILED, "WriteData FAILED");
  } while (false);
  // Close file
...
Status FileSaver::SaveWithFileHeader(const std::string &file_path, const ModelFileHeader &file_header,
  ...
  do {
    ...
    const uint32_t table_size = SizeOfModelPartitionTable(model_partition_table);
    GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
        WriteData(static_cast<const void *>(&model_partition_table), static_cast<uint64_t>(table_size), fd) != SUCCESS,
                  ret = FAILED; break);
    // Write partition data
    for (const auto &partitionData : partition_datas) {
      GELOGI("GC:size[%u]", partitionData.size);
      GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
          WriteData(static_cast<const void *>(partitionData.data), static_cast<uint64_t>(partitionData.size), fd)
                    != SUCCESS, ret = FAILED; break);
    }
  } while (false);
  // Close file
  if (mmClose(fd) != EN_OK) {
...
Status FileSaver::SaveToFile(const std::string &file_path, const ge::ModelData &model,
  ...
  const Status ret = SaveWithFileHeader(file_path, file_header, model.model_data,
                                        static_cast<int32_t>(file_header.length));
  if (ret != SUCCESS) {
...
Status FileSaver::SaveToFile(const std::string &file_path, const ModelFileHeader &model_file_header,
  ...
  GE_CHK_BOOL_RET_STATUS(ret == SUCCESS, FAILED, "save file failed, file_path:%s, file header len:%u.",
                         file_path.c_str(), model_file_header.length);
  return SUCCESS;
}
  ...
  do {
    ...
    for (size_t index = 0U; index < model_partition_tables.size(); ++index) {
      ...
      for (const auto &partition_data : cur_partition_datas) {
        ...
        GE_CHK_BOOL_TRUE_EXEC_WITH_LOG(
            WriteData(static_cast<const void *>(partition_data.data), static_cast<uint64_t>(partition_data.size), fd)
                != SUCCESS, ret = FAILED; break);
      }
    }
  } while (false);
...
Status FileSaver::SaveToFile(const std::string &file_path, const void * const data, const uint64_t len) {
  if ((data == nullptr) || (len <= 0)) {
    ...
    REPORT_INNER_ERROR("E19999", "Save file failed, the model_data is null or "
                       "its length:%lu is less than 1.", len);
    return FAILED;
  }
