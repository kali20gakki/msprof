...
class NodeCompileCacheItem {
 public:
  ...
  static Status Build(const KernelLaunchBinType bin_type, const NodePtr &node, void *handle, NodeCompileCacheItem &item);
 ...
 private:
  uint64_t cache_item_id_ = -1;
  KernelLaunchBinType bin_type_ = kBinTypeEnd;
...
class NodeCompileCacheModule {
 ...
 private: 
  Status GetCompileCacheDescFromOp(const NodePtr &node, CompileCacheDesc &cache_desc, const bool need_range) const;
  Status GetOpAttrMem(OpDesc &op_desc, CompileCacheDesc &cache_desc) const;
  Status CopyAttrToMem(const std::map<std::string, AnyValue> &all_attributes,
    std::unique_ptr<uint8_t[]> &attr_mem, std::set<string> &ordered_origin_attr_name, const size_t attr_size) const;
  Status GetAttrTotalSize(const std::map<std::string, AnyValue> &all_attributes,
    std::set<string> &ordered_origin_attr_name, size_t &attr_size) const;
