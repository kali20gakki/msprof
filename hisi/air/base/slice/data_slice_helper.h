...
class DataSliceHelper {
 public:
  static Status InferAxisSlice(ge::OpDescPtr &op, const AxisTypeInfo &slice_info);
  static Status GetSliceInfo(ge::OpDescPtr &op, std::vector<AxisTypeInfo> &axis_type_vec);
 private:
  // cut tensor: axis index: shape range
  using DataSliceType = std::vector<std::vector<std::vector<int64_t>>>;
  static Status SetInputSlice(ge::OpDescPtr &op, const AxisTypeInfo &slice_info, DataSliceType &input_slice);
};
}
