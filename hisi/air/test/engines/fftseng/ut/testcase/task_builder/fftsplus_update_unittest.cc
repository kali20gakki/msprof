...
TEST_F(FFTS_plus_update, UpdateContext2)
{
  ...
  CreateAutoTaskInfo(fftsPlusTaskInfo_, 1);
  CreateAutoCubeNode();
  ge::AttrUtils::SetInt(node_->GetOpDesc(), kPrefetchEnableBm, 7);
...
TEST_F(FFTS_plus_update, CalculateDataParam)
{
  ...
  DataContextParam data_ctx;
  Status ret = GenerateDataCtxParam(tensor_desc, slice, &data_ctx, 0);
