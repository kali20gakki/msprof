...
using namespace fe;
namespace fe {
class fixpipe_common_st : public testing::Test {
 public:
 protected:
  void SetUp() {}
  void TearDown() {}
...
TEST_F(fixpipe_common_st, CalDSize01) {
  size_t data_size;
...
TEST_F(fixpipe_common_st, split01) {
...
TEST_F(fixpipe_common_st, ReadPlatFormConfig01) {
...
TEST_F(fixpipe_common_st, ReadPlatFormConfig02) {
  std::string path = "./air/test/engines/nneng/config/data/platform_config";
  std::string real_path = RealPath(path);
  PlatformInfoManager::Instance().platform_info_map_.clear();
  PlatformInfoManager::Instance().platform_infos_map_.clear();
