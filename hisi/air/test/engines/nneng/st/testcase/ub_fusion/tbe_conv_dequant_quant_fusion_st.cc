...
class UB_FUSION_ST_CONV_DEQUANT_QUANT : public testing::Test {
 public:
...
TEST_F(UB_FUSION_ST_CONV_DEQUANT_QUANT, tbe_conv_dequant_quant_pass01) {
  ...
  uint32_t id = 0;
  cerr << endl;
  cerr << "UB_FUSION_ST_CONV_DEQUANT_QUANT UB fusion before" << endl;
  ...
  int find = 0;
  cerr << endl;
  cerr << "UB_FUSION_ST_CONV_DEQUANT_QUANT UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr << "id:" << id << endl;
    uint32_t scope_id = 0;
    cerr << "name: " << node->GetName() <<
...
TEST_F(UB_FUSION_ST_CONV_DEQUANT_QUANT, tbe_conv_dequant_quant_pass02) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
...
TEST_F(UB_FUSION_ST_CONV_DEQUANT_QUANT, tbe_conv_dequant_quant_pass03) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
...
TEST_F(UB_FUSION_ST_CONV_DEQUANT_QUANT, tbe_conv_dequant_quant_pass05) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
...
TEST_F(UB_FUSION_ST_CONV_DEQUANT_QUANT, tbe_conv_dequant_quant_pass06) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
