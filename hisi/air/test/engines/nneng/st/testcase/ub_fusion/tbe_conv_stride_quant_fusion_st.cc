...
class UB_FUSION_ST_CONV_STRIDED_QUANT : public testing::Test {
 public:
...
TEST_F(UB_FUSION_ST_CONV_STRIDED_QUANT, strideconv1) {
...
// BufferFusion(graph, graph_out);
  ...
  cerr << endl;
  cerr << "UB_FUSION_ST_CONV_STRIDED_QUANT UB fusion before" << endl;
...
// create fused Graph, and merge matched sub-graphs into fusion ops
  ...
  int find = 0;
  cerr << endl;
  cerr << "UB_FUSION_ST_CONV_STRIDED_QUANT UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr << "id:" << id << endl;
    uint32_t scope_id = 0;
    cerr << "name: " << node->GetName()
...
TEST_F(UB_FUSION_ST_CONV_STRIDED_QUANT, strideconv2) {
...
TEST_F(UB_FUSION_ST_CONV_STRIDED_QUANT, strideconv3) {
