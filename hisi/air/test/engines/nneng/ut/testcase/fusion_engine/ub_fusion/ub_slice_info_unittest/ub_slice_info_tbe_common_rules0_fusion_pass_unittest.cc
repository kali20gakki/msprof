...
class COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST : public testing::Test {
public:
...
protected:
  ...
  // "outputList":[{"axis":[0],"idx":0}]}]}}
  void BuildGraphForTbeCommonRules0FusionPassOKCase1(ComputeGraphPtr &graph) {
    ...
    read->AddOutputDesc(conv_tensor_desc);
    ...
    end->AddInputDesc(conv_tensor_desc);
    end->AddOutputDesc(conv_tensor_desc);
    ...
    GraphUtils::AddEdge(write_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
  /************************
   *           x    filter  bias
   *            \    |    /
   ...
   *               quant --- end
   *************************/
   // fusion op slice info is : {"_fusion_op_slice_info":{"l1FusionEnable":2,"minTbeL1Space":0,"reduceMaps":[],"splitMaps":[
   // {"inputList":[{"axis":[3],"headOverLap":[0],"idx":0,"tailOverLap":[0]},{"axis":[3],"headOverLap":[],"idx":4,"tailOverLap":[]},
   // {"axis":[3],"headOverLap":[],"idx":5,"tailOverLap":[]}],"outputList":[{"axis":[3],"idx":0}]},
   // {"inputList":[{"axis":[2],"headOverLap":[0],"idx":0,"tailOverLap":[0]},{"axis":[2],"headOverLap":[],"idx":4,"tailOverLap":[]},
   // {"axis":[2],"headOverLap":[],"idx":5,"tailOverLap":[]}],"outputList":[{"axis":[2],"idx":0}]},
   // {"inputList":[{"axis":[0],"headOverLap":[-1],"idx":0,"tailOverLap":[-1]},{"axis":[0],"headOverLap":[],"idx":4,"tailOverLap":[]},
   // {"axis":[0],"headOverLap":[],"idx":5,"tailOverLap":[]}],"outputList":[{"axis":[0],"idx":0}]}]}}
  void BuildGraphForTbeCommonRules0FusionPassOKCase2(ComputeGraphPtr &graph) {
    OpDescPtr data = std::make_shared<OpDesc>("DATA0", fe::DATA);
    OpDescPtr data1 = std::make_shared<OpDesc>("DATA1", fe::DATA);
    OpDescPtr data2 = std::make_shared<OpDesc>("DATA2", fe::DATA);
    OpDescPtr data3 = std::make_shared<OpDesc>("DATA3", fe::DATA);
   ...
   *************************/
  // {"_fusion_op_slice_info":{"l1FusionEnable":2,"minTbeL1Space":0,"reduceMaps":[],"splitMaps":[
  // {"inputList":[{"axis":[3],"headOverLap":[0],"idx":0,"tailOverLap":[0]},{"axis":[3],"headOverLap":[],"idx":3,"tailOverLap":[]}],
  //  "outputList":[{"axis":[3],"idx":0}]},
  // {"inputList":[{"axis":[2],"headOverLap":[0],"idx":0,"tailOverLap":[0]},{"axis":[2],"headOverLap":[],"idx":3,"tailOverLap":[]}],
  // "outputList":[{"axis":[2],"idx":0}]},
  // {"inputList":[{"axis":[1],"headOverLap":[-1],"idx":1,"tailOverLap":[-1]},{"axis":[0],"headOverLap":[-1],"idx":2,"tailOverLap":[-1]},
  //               {"axis":[1],"headOverLap":[],"idx":3,"tailOverLap":[]}],"outputList":[{"axis":[1],"idx":0}]},
  // {"inputList":[{"axis":[0],"headOverLap":[-1],"idx":0,"tailOverLap":[-1]},{"axis":[0],"headOverLap":[],"idx":3,"tailOverLap":[]}],
  // "outputList":[{"axis":[0],"idx":0}]}]}}
  void BuildGraphForTbeCommonRules0FusionPassOKCase3  (ComputeGraphPtr &graph) {
    OpDescPtr data = std::make_shared<OpDesc>("DATA0", fe::DATA);
    OpDescPtr data1 = std::make_shared<OpDesc>("DATA1", fe::DATA);
    OpDescPtr data2 = std::make_shared<OpDesc>("DATA2", fe::DATA);
    OpDescPtr data3 = std::make_shared<OpDesc>("DATA3", fe::DATA);
    ...
    GraphUtils::AddEdge(eltwise_node->GetOutDataAnchor(0), relu_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(relu_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
};

TEST_F(COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST, pass_case1) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
  BuildGraphForTbeCommonRules0FusionPassOKCase1(graph_out);
  ...
  cerr << endl;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case1 UB fusion before" << endl;
  ...
  int find = 0;
  cerr << endl;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case1 UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr  << "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
...
TEST_F(COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST, pass_case2) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
  BuildGraphForTbeCommonRules0FusionPassOKCase2(graph_out);
  graph_out->TopologicalSorting();
  graph_out->Dump();
  // set op slice info for each op
  OpSetterPtr op_setter_ptr = std::make_shared<OpSetter>(AI_CORE_NAME, op_store_adapter_manager_ptr_);
  ...
  cerr << endl;
  string op_slice_info;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case2 UB fusion before" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr << "id:" << id << endl;
    uint32_t scope_id = 0;
    cerr << "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
  ...
  int find = 0;
  cerr << endl;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case2 UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
     cerr  <<  "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
    if (node->GetOpDesc()->GetType() == "Conv2D" && node->GetName()  == "convdequanteltwise1eltwise2eltwise3reluquant") {
      AttrUtils::GetStr(node->GetOpDesc(), OP_SLICE_INFO, op_slice_info);
      cerr << "op slice info is :   " << endl << op_slice_info << endl;
      AttrUtils::GetStr(node->GetOpDesc(), FUSION_OP_SLICE_INFO, fusion_op_slice_info);
      cerr << "fusion op slice info is :   " << endl << fusion_op_slice_info << endl;
...
TEST_F(COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST, pass_case3) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
  BuildGraphForTbeCommonRules0FusionPassOKCase3(graph_out);
  graph_out->TopologicalSorting();
  graph_out->Dump();
  // set op slice info for each op
  OpSetterPtr op_setter_ptr = std::make_shared<OpSetter>(AI_CORE_NAME, op_store_adapter_manager_ptr_);
  ...
  string op_slice_info;
  cerr << endl;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case3 UB fusion before" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr << "id:" << id << endl;
    uint32_t scope_id = 0;
    cerr << "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
  ...
  int find = 0;
  cerr << endl;
  cerr << "COMMON_RULES0_UB_FUSION_SLICE_INFO_UNITTEST::pass_case3 UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
     cerr  <<  "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
