...
tune::Status L2FusionOptimizer3(ge::ComputeGraph &, GraphCommPtr, ScopeAllocatorPtr, const string &, AOEOption) {
...
te::OpBuildResCode TeFusionStub(std::vector<ge::Node *> teGraphNode, ge::OpDescPtr opDesc,
  ...
  te::TbeOpInfo op_info("relu", "relu", "Relu", "1.0", "tbe");
...
TEST_F(UTEST_fusion_engine_op_compiler, case_run_compile_process_fail)
{
    ...
    Status ret = op_compiler_ptr->RunCompileProcess(*graph, graph_comm_ptr, buff_fus_compile_failed_nodes,
                                                    need_post_process);
    for(FEOpsStoreInfo &fe_op_store_info: Configuration::Instance(AI_CORE_NAME).ops_store_info_vector_) {
      if (op_impl_type == fe_op_store_info.op_impl_type) {
...
TEST_F(UTEST_fusion_engine_op_compiler, case_run_compile_process_failed1)
{
    ...
    ret = op_compiler_ptr->RunCompileProcess(*graph, graph_comm_ptr, buff_fus_compile_failed_nodes, need_post_process);
    ...
    fe_graph_optimizer_ptr->fusion_pass_mgr_ptr_->L2FusionOptimizer = L2FusionOptimizer3;
    ret = op_compiler_ptr->RunCompileProcess(*graph, graph_comm_ptr, buff_fus_compile_failed_nodes, need_post_process);
    EXPECT_EQ(fe::FAILED, ret);
}
...
TEST_F(UTEST_fusion_engine_op_compiler, parse_sgt_tvm_json_to_set_attr)
{
    auto op_compiler_ptr = std::make_shared<OpCompiler>("normal compiler", AI_CORE_NAME, op_store_adapter_manager_ptr_);
    string json_file_path = "air/test/engines/nneng/ut/testcase/fusion_engine/op_compiler/json/cce_reductionLayer_1_10_float16__1_SUMSQ_1_with_so.json";
    vector<int64_t> dim(4, 2);
...
TEST_F(UTEST_fusion_engine_op_compiler, parse_json_and_update_op_fail3)
{
    ...
    OpDescPtr op_desc_ptr = node->GetOpDesc();
    std::map<int64_t, std::string> scope_json_map;
    scope_json_map[1] = "air/test/engines/nneng/ut/testcase/fusion_engine/op_compiler/json/cce_reductionLayer_1_10_float16__1_SUMSQ_1_0_error1.json";
    ScopeJsonMap::iterator json_iter = scope_json_map.find(1);
    ffts::ThreadSliceMap thread_slice_map;
    thread_slice_map.thread_scope_id = 1;
    ffts::ThreadSliceMapPtr tsmp_ptr = make_shared<ffts::ThreadSliceMap>(thread_slice_map);;
...
TEST_F(UTEST_fusion_engine_op_compiler, parse_json_and_update_op_fail4)
{
    ...
    OpDescPtr op_desc_ptr = node->GetOpDesc();
    std::map<int64_t, std::string> scope_json_map;
    scope_json_map[1] = "air/test/engines/nneng/ut/testcase/fusion_engine/op_compiler/json/cce_reductionLayer_1_10_float16__1_SUMSQ_1_0_error2.json";
    ScopeJsonMap::iterator json_iter = scope_json_map.find(1);
    ffts::ThreadSliceMap thread_slice_map;
    thread_slice_map.thread_scope_id = 1;
    ffts::ThreadSliceMapPtr tsmp_ptr = make_shared<ffts::ThreadSliceMap>(thread_slice_map);;
...
TEST_F(UTEST_fusion_engine_op_compiler, ParseJsonAndCompressOp_success_3) {
  ...
  map<int64_t, std::string> scope_json_map;
  std::vector<ge::NodePtr> nodes_be_compiled;
  scope_json_map.insert(pair<int64_t, std::string>(4, "air/test/engines/nneng/ut/testcase/fusion_engine/op_compiler/json/cce_reductionLayer_1_10_float16__1_SUMSQ_1_0.json"));
  nodes_be_compiled.push_back(node_0);
  auto op_compiler_ptr = std::make_shared<OpCompiler>("normal compiler", AI_CORE_NAME, op_store_adapter_manager_ptr_);
  Status ret = op_compiler_ptr->ParseJsonAndCompressOp(*graph, scope_json_map, nodes_be_compiled);
  EXPECT_EQ(fe::SUCCESS, ret);
...
TEST_F(UTEST_fusion_engine_op_compiler, init_and_finalize_1)
{
...
TEST_F(UTEST_fusion_engine_op_compiler, run_compile_process_case1) {
  ...
  Status ret = op_compiler->RunCompileProcess(*graph, graph_comm_ptr, buff_fus_compile_failed_nodes, need_post_process);
  EXPECT_EQ(ret, fe::SUCCESS);
}
