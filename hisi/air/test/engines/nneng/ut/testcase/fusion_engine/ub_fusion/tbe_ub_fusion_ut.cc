...
void RunUbFusion(ComputeGraphPtr model_graph, string engine_name = fe::AI_CORE_NAME) {
  ...
  std::shared_ptr<FusionPriorityManager> fusion_priority_mgr_ptr =
      std::make_shared<FusionPriorityManager>(engine_name, fusion_pass_mgr_ptr, nullptr);
...
*                 nodec2
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_nodec1_nodec2_noder_nodee_nodeu_nodec3)
{
    srand(1);
...
/************************
*nodec1-->noder-->nodee-->nodeu
*                   ^
*                   |
*                 nodec2
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_nodec1_nodec2_noder_nodee_nodeu_for_coverity)
{
...
*   nodec1                       nodec3
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_complex)
{
...
*  nodec1
******************************************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_bifurcated)
{
...
*  nodec1
*****************************************************************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_bifurcated_with_multicircle)
{
 ...
 *  nodec1
*****************************************************************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_bifurcated_with_circle_nesting)
{
    ...
    vector<set<set<string>>> res{
        ...
        {{"nodec8", "nodec9", "nodec11", "nodec12"}, {"nodee", "multiOutput"}, {"nodec10", "nodec6"},
         {"nodec1", "noder"}, {"nodec7", "nodec2"}}, // 2
        {{"nodec8", "nodec9", "nodec11", "nodec12"}, {"nodec1", "noder"}, {"nodec7", "nodec2", "nodec6"},
         {"nodec5", "nodee"}}, // 3
...
*   |                   V
*  nodec1            nodec3
*  There is a control edge from nodee to temp to multiOutput
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_close_circle_1)
{
  vector<set<set<string>>> res{
      {{"nodec1", "noder", "nodee", "nodec5", "nodec4", "multiOutput", "nodec2", "nodec3"}}, // 0
  };
  ...
  /* Add control edge from nodee ->temp -> multiOutput */
  auto temp = std::make_shared<ge::OpDesc>("temp", "Temp");
  auto temp_node = model_graph->AddNode(temp);

  auto temp_out_ctrl = temp_node->GetOutControlAnchor();
  auto temp_in_ctrl = temp_node->GetInControlAnchor();

  ge::NodePtr e;
  ge::NodePtr mo;
  for (auto node : model_graph->GetDirectNode()) {
    if (node->GetName() == "nodee") {
      e = node;
    }
    if (node->GetName() == "multiOutput") {
      mo = node;
    }
  }

  auto e_out_ctrl = e->GetOutControlAnchor();
  auto mo_in_ctrl = mo->GetInControlAnchor();

  ge::GraphUtils::AddEdge(e_out_ctrl, temp_in_ctrl);
  ge::GraphUtils::AddEdge(temp_out_ctrl, mo_in_ctrl);
...
*
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_conv_fusion) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        input_desc_list.push_back(input_desc);
        output_desc_list.clear();
        OpDesc = CreateOpDefUbFusion("nodec4", "Eltwise", src_name_list, dst_name_list, input_desc_list, output_desc_list);
        ge::AttrUtils::SetListStr(OpDesc, OPDESC_DST_NAME, dst_name_list);
        ge::AttrUtils::SetListStr(OpDesc, OPDESC_SRC_NAME, src_name_list);
        op_list.push_back(OpDesc);
        ...
        vec8.push_back(dsc_info);
        it = dst_map.find("nodec4");
        vector<desc_info> &vec38 = it->second;
        dsc_info.targetname = "nodec5";
        dsc_info.index = 0;
        vec38.push_back(dsc_info);
...
*
*************************/
TEST_F(UBFUSION_UT, fusion_test_tefusion_conv_fusion2)
{
