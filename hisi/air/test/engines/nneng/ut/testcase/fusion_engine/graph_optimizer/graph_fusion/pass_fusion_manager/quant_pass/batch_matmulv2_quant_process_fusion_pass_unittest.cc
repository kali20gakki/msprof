...
class UTEST_BATCH_MATMULV2_QUANT_PROCESS_FUSION : public testing::Test {
 protected:
  void SetUp() {}
  void TearDown(){}
};
 ...
 *   batchmatMul
 */
TEST_F(UTEST_BATCH_MATMULV2_QUANT_PROCESS_FUSION, GET_CO_VALUE_SUCCESS) {
 ...
 *   batchmatMul
 */
TEST_F(UTEST_BATCH_MATMULV2_QUANT_PROCESS_FUSION, GET_QUANT_PROCESS_MODE_SUCCESS) {
  //(void)setenv("DUMP_GE_GRAPH", "2", 2);
  Configuration::Instance(AI_CORE_NAME).isa_arch_ver_ = ISAArchVersion::EN_ISA_ARCH_V100;
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
