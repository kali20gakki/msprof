...
class fusion_concat_quant_fusion_cast_ut : public testing::Test {
public:
...
protected:
  ...
  void InitGraph1(ComputeGraphPtr &graph, ge::DataType quant_data_type) {
    OpDescPtr dequant1 = std::make_shared<OpDesc>("dequant1", DEQUANT);
    OpDescPtr quant1 = std::make_shared<OpDesc>("quant1", QUANT);
    OpDescPtr quant2 = std::make_shared<OpDesc>("quant2", QUANT);
    OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
    ...
    tensor_desc_c.SetOriginShape(shape_c);
    ...
    GeTensorDesc out_desc3(shape0, ge::FORMAT_NCHW, ge::DT_FLOAT);
    dequant1->AddOutputDesc(tensor_desc_c);
    concat->AddInputDesc(tensor_desc_c);

    ge::GeShape shape1({1, 128, 7, 7});
    GeTensorDesc out2_desc0(shape1, ge::FORMAT_NCHW, ge::DT_FLOAT);
    concat->AddOutputDesc(out2_desc0);
    quant1->AddInputDesc(out2_desc0);
    quant2->AddInputDesc(out2_desc0);
    GeTensorDesc quant2_desc0(shape1, ge::FORMAT_NCHW, quant_data_type);
    quant2->AddOutputDesc(quant2_desc0);
    quant1->AddOutputDesc(quant2_desc0);
    end->AddInputDesc(quant2_desc0);
    ...
    ge::GraphUtils::AddEdge(dequant1_node->GetOutDataAnchor(0), quant1_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(concat_node->GetOutDataAnchor(0), quant2_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(quant2_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
  void InitGraph2(ComputeGraphPtr &graph, ge::DataType quant_data_type) {
    OpDescPtr dequant1 = std::make_shared<OpDesc>("dequant1", DEQUANT);
    OpDescPtr quant1 = std::make_shared<OpDesc>("quant1", QUANT);
    OpDescPtr quant2 = std::make_shared<OpDesc>("quant2", QUANT);
    OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
    ...
    ge::GraphUtils::AddEdge(dequant1_node->GetOutDataAnchor(0), quant1_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(concat_node->GetOutDataAnchor(0), quant2_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(concat_node->GetOutDataAnchor(1), cast_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(quant2_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
  void InitGraph3(ComputeGraphPtr &graph, ge::DataType quant_data_type) {
    OpDescPtr dequant1 = std::make_shared<OpDesc>("dequant1", DEQUANT);
    OpDescPtr quant1 = std::make_shared<OpDesc>("quant1", QUANT);
    OpDescPtr quant2 = std::make_shared<OpDesc>("quant2", QUANT);
    OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
    ...
    ge::GraphUtils::AddEdge(dequant1_node->GetOutDataAnchor(0), concat_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(dequant1_node->GetOutDataAnchor(0), quant1_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(quant1_node->GetOutDataAnchor(0), cast_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(concat_node->GetOutDataAnchor(0), quant2_node->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(quant2_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
...
TEST_F(fusion_concat_quant_fusion_cast_ut, concat_quant01) {
vector <ge::DataType> quant_data_types = {ge::DT_INT8};
for (auto quant_data_type: quant_data_types) {
ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
InitGraph1(graph, quant_data_type);
...
TEST_F(fusion_concat_quant_fusion_cast_ut, concat_quant02) {
vector <ge::DataType> quant_data_types = {ge::DT_INT8};
for (auto quant_data_type: quant_data_types) {
ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
InitGraph2(graph, quant_data_type);
ConcatQuantFusionPass pass;
vector<GraphPass *> passes = {&pass};
Status status = PassManager::Run(*graph, passes);
DumpGraph(graph, "test");
...
TEST_F(fusion_concat_quant_fusion_cast_ut, concat_quant03) {
vector <ge::DataType> quant_data_types = {ge::DT_INT8};
for (auto quant_data_type: quant_data_types) {
ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
InitGraph3(graph, quant_data_type);
ConcatQuantFusionPass pass;
vector<GraphPass *> passes = {&pass};
Status status = PassManager::Run(*graph, passes);
DumpGraph(graph, "test");
...
TEST_F(fusion_concat_quant_fusion_cast_ut, no_same_input) {
...
TEST_F(fusion_concat_quant_fusion_cast_ut, same_input) {
vector <ge::DataType> quant_data_types = {ge::DT_INT4, ge::DT_INT8};
for (auto quant_data_type: quant_data_types) {
ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
...
TEST_F(fusion_concat_quant_fusion_cast_ut, same_input_diff_quant) {
vector <ge::DataType> quant_data_types = {ge::DT_INT4, ge::DT_INT8};
for (auto quant_data_type: quant_data_types) {
ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
