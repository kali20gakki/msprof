...
class UTEST_fusion_engine_tf_quant_fusion_pass : public testing::Test {
protected:
  void SetUp() {}
  void TearDown() {}
  ...
  ge::NodePtr AddConst2Node(ge::ComputeGraphPtr graph, ge::NodePtr &node, int32_t in_anchor_index, float value = 1) {
  ...
  ge::NodePtr AddConst2Node2(ge::ComputeGraphPtr graph, ge::NodePtr &node, int32_t in_anchor_index, float value = 1) {
    static uint32_t newname_id = 0;
    float data[] = {value};
  ...
  ge::ComputeGraphPtr CreateSucessGraphWithPad() {
    ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
    ge::NodePtr quant      = AddNode(graph, "quant", OP_QUANT);
    ge::NodePtr sub        = AddNode(graph, "ascendweightquant", OP_ASCENDWEIGHTQUANT, 1, 2);
    ge::NodePtr conv       = AddNode(graph, "conv", OP_CONV, 1, 2);
    ...
    ge::NodePtr cast       = AddNode(graph, "cast", OP_CAST);
    ge::NodePtr dequant    = AddNode(graph, "dequant", OP_DEQUANT);
    ge::NodePtr pad        = AddNode(graph, "pad", OP_PAD);

    ge::GraphUtils::AddEdge(quant->GetOutDataAnchor(0), pad->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(pad->GetOutDataAnchor(0), conv->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(sub->GetOutDataAnchor(0), conv->GetInDataAnchor(1));
    ge::GraphUtils::AddEdge(conv->GetOutDataAnchor(0), biasadd->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(cast->GetOutDataAnchor(0), biasadd->GetInDataAnchor(1));
    ge::GraphUtils::AddEdge(biasadd->GetOutDataAnchor(0), dequant->GetInDataAnchor(0));
  ...
  ge::ComputeGraphPtr CreateMergeOnnxBatchMatMulV2(vector<ge::NodePtr> &node_list) {
    ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
    node_list.clear();
    ge::NodePtr quant      = AddNode(graph, "quant", OP_QUANT);
    ge::NodePtr matmulv2   = AddNode(graph, "matmul", OP_BATCHMATMULV2, 1, 2);
    ge::NodePtr biasadd    = AddNode(graph, "add", OP_ADD, 1, 2);
    ge::NodePtr dequant    = AddNode(graph, "dequant", OP_DEQUANT, 1, 2);

    AddConst2NodeInt8(graph, matmulv2, 1);
    AddConst2Node(graph, biasadd, 1);
    AddConst2Node(graph, dequant, 1);

    ge::GraphUtils::AddEdge(quant->GetOutDataAnchor(0), matmulv2->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(matmulv2->GetOutDataAnchor(0), biasadd->GetInDataAnchor(0));
    ge::GraphUtils::AddEdge(biasadd->GetOutDataAnchor(0), dequant->GetInDataAnchor(0));

    return graph;
  }
  ge::ComputeGraphPtr CreateMergeSubSucessGraphNormalNoCast(vector<ge::NodePtr> &node_list) {
    ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
    node_list.clear();
    ge::NodePtr quant      = AddNode(graph, "mul0", OP_QUANT);
    ge::NodePtr sub        = AddNode(graph, "ascendweightquant", OP_ASCENDWEIGHTQUANT, 1, 2);
  ...
  ge::ComputeGraphPtr CreateMergeSubSucessGraphNormalWithPad(vector<ge::NodePtr> &node_list) {
    ge::ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test_graph");
    node_list.clear();
    ge::NodePtr quant      = AddNode(graph, "mul0", OP_QUANT);
    ge::NodePtr sub        = AddNode(graph, "ascendweightquant", OP_ASCENDWEIGHTQUANT, 1, 2);
  ...
  static void check_result(const ge::ComputeGraphPtr graph, const string attr_name, int value)
  {
    ...
    for(ge::NodePtr node : graph->GetAllNodes()) {
      ...
      if (node->GetType() == RELU || node->GetType() == CAST)
        continue;
  ...
  static void check_ascend_weight_quant_dont_have_spec_attr(const ge::ComputeGraphPtr graph, const string attr_name)
  {
    printf("start to check ascend weight quant dont have spec_attr[%s]...\n", attr_name.c_str());
    bool attr_value = false;
    for (ge::NodePtr node : graph->GetAllNodes()) {
      printf("nodeName: %s ", node->GetType().c_str());
      if (node->GetType() == "AscendWeightQuant") {
        bool ret = ge::AttrUtils::GetBool(node->GetOpDesc(), attr_name, attr_value);
        EXPECT_EQ(false, ret);
      }
    }
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, pattern_success) {
  ...
  // delete no_need_constant_folding
  fe::DeleteNoConstFolding pass1;
  vector<fe::GraphPass *> passes1 = {&pass1};
  status = fe::PassManager::Run(*graph, passes1);

  DumpGraph(graph, "after delete no_need_const_folding pattern0 fusion");

  EXPECT_EQ(fe::SUCCESS, status);
  check_ascend_weight_quant_dont_have_spec_attr(graph, ATTR_NO_FOLDING);
}

TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, pattern_with_pad_success) {

  ge::ComputeGraphPtr graph = CreateSucessGraphWithPad();
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_normal_success) {
  ...
  fe::TfTagNoConstFolding pass;
  fe::TfMergeWeightQuantFusionPass pass1;
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_onnx_batchmatmulv2_success) {
  Configuration::Instance(AI_CORE_NAME).isa_arch_ver_ = ISAArchVersion::EN_ISA_ARCH_V100;
  std::vector<ge::NodePtr> node_list;
  ge::ComputeGraphPtr graph = CreateMergeOnnxBatchMatMulV2(node_list);
  DumpGraph(graph, "before merge_sub_pattern_success");
  fe::TfMergeWeightQuantFusionPass pass;
  vector<fe::GraphPass *> passes = {&pass};
  Status status = fe::PassManager::Run(*graph, passes);
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_normal_success_no_cast) {
  Configuration::Instance(AI_CORE_NAME).isa_arch_ver_ = ISAArchVersion::EN_ISA_ARCH_V100;

  std::vector<ge::NodePtr> node_list;
  ge::ComputeGraphPtr graph = CreateMergeSubSucessGraphNormalNoCast(node_list);
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_normal_success_1) {
  Configuration::Instance(AI_CORE_NAME).isa_arch_ver_ = ISAArchVersion::EN_ISA_ARCH_V100;

  std::vector<ge::NodePtr> node_list;
  ge::ComputeGraphPtr graph = CreateMergeSubSucessGraphNormalWithPad(node_list);
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_normal_success_v200) {
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_sharesub_success) {
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_nobias_success) {
...
TEST_F(UTEST_fusion_engine_tf_quant_fusion_pass, merge_sub_pattern_sharesub_nobias_success) {
