...
FILE_PERM=640
CERT_PERM=400
PROFILING_CONF_DIR=/var/log/npu/conf/profiling
PROFILING_DIR=/var/log/npu/profiling
PROFILING_CONF_FILE=${PROFILING_CONF_DIR}/profile.cfg
...
startIdeDaemon(){
    isStarted=$(ps -A |grep -w ada | grep -v grep | wc -l)
...
stopIdeDaemon(){
    pkill "ada"
    isStopped=$(ps -A |grep -w ada | grep -v grep | wc -l)
