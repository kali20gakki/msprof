 ...
 */
int32_t IdeGetDevList(IdeU32Pt devNum, IdeU32Pt devs, uint32_t len)
{
    IDE_CTRL_VALUE_FAILED(devNum != nullptr, return IDE_DAEMON_ERROR, "devNum is nullptr");
    IDE_CTRL_VALUE_FAILED(devs != nullptr, return IDE_DAEMON_ERROR, "devs is nullptr");
    ...
    if (err != DRV_ERROR_NONE) {
        IDE_LOGE("get device number Failed, err: %d", err);
        return IDE_DAEMON_ERROR;
    }

    if (*devNum > 0 && *devNum <= DEVICE_NUM_MAX) {
#if (defined IDE_DAEMON_HOST) || (defined ADX_LIB_HOST) || (defined ADX_LIB_HOST_DRV)
        for (uint32_t i = 0; i < *devNum && i < DEVICE_NUM_MAX; i++) {
            devs[i] = i;
        }
#else
        err = drvGetDeviceLocalIDs(devs, len);
        if (err != DRV_ERROR_NONE) {
...
int32_t IdeGetLogIdByPhyId(uint32_t desPhyId, IdeU32Pt logId)
{
    ...
    uint32_t devNum = 0;
    uint32_t devIds[DEVICE_NUM_MAX] = {0};
