...
group_hccl = dist.new_group(backend="hccl")
group_lccl = dist.new_group(backend="hccl")
...
prof = torch_npu.profiler.profile(
    ...
    schedule=torch_npu.profiler.schedule(wait=0, warmup=0, active=3, repeat=2, skip_first=0),
...
def run_all_gather():
    # allGather
    for _ in range(5):
        dist.all_gather(output_list, input_tensor, group=group_hccl)
    for _ in range(5):
        dist.all_gather(output_list, input_tensor, group=group_lccl)
...
def run_all_reduce():
    ...
    for _ in range(5):
        dist.all_reduce(out, group=group_hccl)
    for _ in range(5):
        dist.all_reduce(out, group=group_lccl)
