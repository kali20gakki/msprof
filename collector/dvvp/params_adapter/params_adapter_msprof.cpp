...
int ParamsAdapterMsprof::Init()
{
    ...
    std::vector<InputCfg>({
        ...
        INPUT_CFG_COM_SYS_INTERCONNECTION, INPUT_CFG_COM_DVPP, INPUT_CFG_COM_INSTR_PROFILING,
...
int ParamsAdapterMsprof::ParamsCheckMsprof()
{
    ...
    std::map<int, std::string> switchNameMap = {
        ...
        {INPUT_CFG_COM_INSTR_PROFILING, "instr-profiling"},
...
void ParamsAdapterMsprof::SetDefaultParamsApp()
{
    ...
    if (paramContainer_[INPUT_CFG_COM_GE_API].empty()) {
        paramContainer_[INPUT_CFG_COM_GE_API] = MSVP_PROF_OFF;
    }
