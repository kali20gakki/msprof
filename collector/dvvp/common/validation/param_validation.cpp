...
bool ParamValidation::CovertMetricToHex(std::string &events, const size_t mode) const
{
    ...
    if (mode == DEC_MODE) {
        ...
        int eventsInt;
        if (Utils::StrToInt(eventsInt, events) == PROFILING_FAILED) {
            return false;
        }
        hexStream << std::hex << eventsInt;
        hexStream >> events;
    }
...
bool ParamValidation::CheckFreqIsValid(const std::string &cfgName, const std::string &freq, const int rangeMin,
    const int rangeMax) const
{
    ...
    if (Utils::CheckStringIsNonNegativeIntNum(freq)) {
        int optRet;
        if (Utils::StrToInt(optRet, freq) == PROFILING_FAILED) {
            return false;
        }
        if ((optRet >= rangeMin) && (optRet <= rangeMax)) {
            ...
            MSPROF_LOGE("%s", errReason.c_str());
            CMD_LOGE("Argument --%s=%s is out of range [%d, %d].", cfgName.c_str(), freq.c_str(), rangeMin, rangeMax);
            return false;
        }
    } else {
        MSPROF_LOGE("%s", errReason.c_str());
        CMD_LOGE("Argument --%s=%s is invalid. It should be nonnegative integer.", cfgName.c_str(), freq.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"freq", freq, errReason}));
        return false;
    }
...
bool ParamValidation::CheckHostSysPidValid(const std::string &hostSysPid) const
{
    ...
    if (Utils::CheckStringIsNonNegativeIntNum(hostSysPid)) {
        int hostSysRet;
        if (Utils::StrToInt(hostSysRet, hostSysPid) == PROFILING_FAILED) {
            return false;
        }
        if (!CheckHostSysPidIsValid(hostSysRet)) {
            ...
            CMD_LOGE("Argument --host-sys-pid=%d is invalid. The process cannot be found, "
                "please enter a correct host-sys-pid.", hostSysRet);
            MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
        ...
        MSPROF_LOGE("Argument --host-sys-pid=%s is invalid."
            "Please input an integer value.The min value is 0.", hostSysPid.c_str());
        CMD_LOGE("Argument --host-sys-pid=%s is invalid. Please input an integer value."
            "The min value is 0.", hostSysPid.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"host_sys_pid", hostSysPid, errReason}));
        return false;
    }
...
bool ParamValidation::CheckPythonPathIsValid(const std::string &pythonPath) const
{
    ...
    if (MmAccess2(absolutePythonPath, M_X_OK) != PROFILING_SUCCESS) {
        ...
        CMD_LOGE("Argument --python-path=%s permission denied.", pythonPath.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"python-path", pythonPath, errReason}));
        return false;
    }
    if (Utils::IsDir(absolutePythonPath)) {
        ...
        CMD_LOGE("Argument --python-path=%s is a directory, "
            "please enter the executable file path.", pythonPath.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"python-path", pythonPath, errReason}));
        return false;
...
bool ParamValidation::CheckExportSummaryFormatIsValid(const std::string &summaryFormat) const
{
    ...
    if (summaryFormat != "json" && summaryFormat!= "csv") {
        MSPROF_LOGE("Argument --summary-format=%s is invalid. "
            "Please input 'json' or 'csv'.", summaryFormat.c_str());
        CMD_LOGE("Argument --summary-format=%s is invalid. "
            "Please input 'json' or 'csv'.", summaryFormat.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"summary-format", summaryFormat, errReason}));
        return false;
...
bool ParamValidation::CheckL2CacheEventsValid(const std::vector<std::string> &events)
{
    ...
    const std::vector<std::string> l2CacheTaskEventsWhiteList = {
        ...
        "0x90", "0x91", "0x9c", "0x9d", "0xbf", "0xf6", "0xfb",
        "0xfc"
    };
    for (uint32_t i = 0; i < events.size(); i++) {
        ...
        for (std::string temp : l2CacheTaskEventsWhiteList) {
            if (temp == event) {
                MSPROF_LOGD("l2Cache event:%s", event.c_str());
                findFlag = true;
                break;
            }
        }
        if (!findFlag) {
            MSPROF_LOGE("l2Cache event:%s is invalid", event.c_str());
            return false;
        }
    }
    return true;
...
bool ParamValidation::IsValidInputCfgSwitch(const std::string &switchName, const std::string &switchVal) const
{
    if (!IsValidSwitch(switchVal)) {
        MSPROF_LOGE("The switch %s should be set to on or off.", switchName.c_str());
...
bool ParamValidation::CheckDeviceIdIsValid(const std::string &devId) const
{
    ...
    int dev;
    if (Utils::StrToInt(dev, devId) == PROFILING_FAILED) {
        return false;
    }
    if (dev >= 64) { // 64 : devIdMaxNum
                                         ...
                                         const std::string &paramsHostSys) const
{
    ...
    for (size_t i = 0; i < devices.size(); ++i) {
        if (!ParamValidation::instance()->CheckDeviceIdIsValid(devices[i])) {
            ...
            CMD_LOGE("device id(%s) in --sys-devices is invalid.", devices[i].c_str());
            result = false;
...
bool ParamValidation::CheckProfilingParams(SHARED_PTR_ALIA<analysis::dvvp::message::ProfileParams> params)
{
    ...
    *l2CacheTaskProfilingEvents = analysis::dvvp::common::utils::Utils::Split(
    ...
    if (!CheckProfilingMetricsIsValid("ai_core_metrics", params->ai_core_metrics)) {
...
bool ParamValidation::IsValidSleepPeriod(const int period) const
{
    if (period < MIN_INTERVAL || period > MAX_PERIOD) {
        MSPROF_LOGE("Argument --sys-period=%d is invalid.", period);
        CMD_LOGE("Argument --sys-period=%d is invalid.", period);
        return false;
    }
    return true;
}
...
bool ParamValidation::CheckHostSysOptionsIsValid(const std::string &hostSysOptions) const
{
    ...
    CMD_LOGE("Argument --host-sys[%s] is invalid.", hostSysOptions.c_str());
    return false;
}
...
bool ParamValidation::CheckHostSysPidIsValid(const int hostSysPid) const
{
    if (hostSysPid < 0) {
        ...
        CMD_LOGE("Argument --host-sys-pid=%d is invalid.", hostSysPid);
        return false;
    }
...
bool ParamValidation::StorageLimitUnitValid(const std::string &storageLimit) const
{
    ...
    size_t unitLen = strlen(STORAGE_LIMIT_UNIT);
    ...
    if (unitStr != STORAGE_LIMIT_UNIT) {
        ...
        CMD_LOGE("Argument --storage-limit=%s, not end with MB", storageLimit.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"storage_limit", storageLimit, errReason}));
        return false;
    }
...
bool ParamValidation::CheckStorageLimit(const std::string &storageLimit) const
{
    ...
    std::string errReason = "storage-limit should be in range [" + std::to_string(STORAGE_LIMIT_DOWN_THD) +
        "," + std::to_string(UINT32_MAX) + "], end with MB";
    if (!StorageLimitUnitValid(storageLimit)) {
        return false;
    }
    ...
    if (!Utils::IsAllDigit(digitStr)) {
        ...
        CMD_LOGE("Argument --storage-limit=%s, invalid numbers", storageLimit.c_str());
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"storage_limit", storageLimit, errReason}));
        return false;
    ...
    if (limit < STORAGE_LIMIT_DOWN_THD) {
        ...
        CMD_LOGE("Argument --storage-limit=%s, min value is %uMB", storageLimit.c_str(), STORAGE_LIMIT_DOWN_THD);
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"storage_limit", storageLimit, errReason}));
        return false;
    } else if (limit > UINT32_MAX) {
        MSPROF_LOGE("storage_limit:%s, max value is %uMB", storageLimit.c_str(), UINT32_MAX);
        CMD_LOGE("Argument --storage-limit=%s, max value is %uMB", storageLimit.c_str(), UINT32_MAX);
        MSPROF_INPUT_ERROR("EK0003", std::vector<std::string>({"config", "value", "reason"}),
            std::vector<std::string>({"storage_limit", storageLimit, errReason}));
        return false;
    }
    ...
    std::string &resultAppParam) const
{
    ...
    if (cmdParam.find("/") != std::string::npos) {
        ...
        if (absolutePathCmdParam.empty()) {
            MSPROF_LOGE("command file (%s) does not exist or permission denied.",
                absolutePathCmdParam.c_str());
            CMD_LOGE("command file (%s) does not exist or permission denied.",
                absolutePathCmdParam.c_str());
            return PROFILING_FAILED;
        }
        if (MmAccess2(absolutePathCmdParam, M_X_OK) != PROFILING_SUCCESS) {
            MSPROF_LOGE("command file (%s) has no executable permission.",
                absolutePathCmdParam.c_str());
            CMD_LOGE("command file (%s) has no executable permission.",
                absolutePathCmdParam.c_str());
            return PROFILING_FAILED;
        }
...
int ParamValidation::MsprofCheckAppParamValid(const std::string &appParam) const
{
    if (Utils::CanonicalizePath(appParam).empty()) {
        MSPROF_LOGE("Argument --application=%s does not exist or permission denied.", appParam.c_str());
        CMD_LOGE("Argument --application=%s does not exist or permission denied.", appParam.c_str());
        return PROFILING_FAILED;
    }
    if (Utils::IsSoftLink(appParam)) {
        MSPROF_LOGE("Argument --application=%s is soft link, not support!", appParam.c_str());
        CMD_LOGE("Argument --application=%s is soft link, not support!", appParam.c_str());
        return PROFILING_FAILED;
    }
    if (Utils::IsDir(appParam)) {
        MSPROF_LOGE("Argument --application=%s is a directory, "
            "please enter the executable file path.", Utils::RealPath(appParam).c_str());
        CMD_LOGE("Argument --application=%s is a directory, "
            "please enter the executable file path.", Utils::RealPath(appParam).c_str());
        return PROFILING_FAILED;
    }
    if (MmAccess2(appParam, M_X_OK) != PROFILING_SUCCESS) {
        MSPROF_LOGE("Argument --application=%s has no executable permission.", Utils::RealPath(appParam).c_str());
    ...
    if (ret != PROFILING_SUCCESS) {
        MSPROF_LOGE("Failed to get app name from argument --application=%s", Utils::RealPath(appParam).c_str());
        CMD_LOGE("Failed to get app name from argument --application=%s", Utils::RealPath(appParam).c_str());
        return PROFILING_FAILED;
    }
    if (!ParamValidation::instance()->CheckAppNameIsValid(appName)) {
        MSPROF_LOGE("Argument --application=%s is invalid.", appName.c_str());
        CMD_LOGE("Argument --application=%s is invalid.", appName.c_str());
        return PROFILING_FAILED;
    }
    return PROFILING_SUCCESS;
}
...
bool ParamValidation::MsprofCheckAiModeValid(const std::string &aiModeParam, const std::string &aiModeType) const
{
    ...
    if (aiModeParam != "task-based" && aiModeParam != "sample-based") {
        ...
        CMD_LOGE("Argument %s=%s is invalid. Please input 'task-based' or "
            "'sample-based'.", aiModeType.c_str(), aiModeParam.c_str());
        return false;
    }
    return true;
}
...
bool ParamValidation::MsprofCheckSysDeviceValid(const std::string &devListParam) const
{
    ...
    for (size_t i = 0; i < devices.size(); ++i) {
        if (!CheckDeviceIdIsValid(devices[i])) {
            ...
            CMD_LOGE("Argument --sys-devices=%s is invalid."
                "Please input data is a id num.", devices[i].c_str());
            return false;
        }
    }
...
bool ParamValidation::MsprofCheckHostSysValid(const std::string &hostSysParam) const
{
    ...
    CMD_LOGE("Currently, --host-sys can be used only in the Linux environment.");
#endif
    ...
    if (hostSysParam.empty()) {
        ...
        CMD_LOGE("Argument --host-sys is empty. Please input in the range of "
            "'cpu|mem|disk|network|osrt'");
        return false;
    }
    ...
    const std::string &appDir) const
{
    ...
    if (std::find(hostSysArray.begin(), hostSysArray.end(), HOST_SYS_OSRT) != hostSysArray.end()) {
        if (CheckHostSysToolsIsExist(TOOL_NAME_PERF, resultDir, appDir) != PROFILING_SUCCESS) {
            ...
            CMD_LOGE("The tool perf is invalid, please check"
                " if the tool and sudo are available.");
            return false;
        }
        if (CheckHostSysToolsIsExist(TOOL_NAME_LTRACE, resultDir, appDir) != PROFILING_SUCCESS) {
            MSPROF_LOGE("The tool ltrace is invalid, please check"
                " if the tool and sudo are available.");
            CMD_LOGE("The tool ltrace is invalid, please check"
                " if the tool and sudo are available.");
            return false;
        }
    }
    if (std::find(hostSysArray.begin(), hostSysArray.end(), HOST_SYS_DISK) != hostSysArray.end()) {
        if (CheckHostSysToolsIsExist(TOOL_NAME_IOTOP, resultDir, appDir) != PROFILING_SUCCESS) {
            ...
            CMD_LOGE("The tool iotop is invalid, please check if"
                " the tool and sudo are available.");
            return false;
        }
    }
    ...
    const std::string &appDir) const
{
    if (resultDir.empty() && appDir.empty()) {
        ...
        CMD_LOGE("If you want to use this parameter:--host-sys,"
            "please put it behind the --output or --application. ");
        return PROFILING_FAILED;
    }
